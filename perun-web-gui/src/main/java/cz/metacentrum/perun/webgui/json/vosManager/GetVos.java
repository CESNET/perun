...
public class GetVos
    implements JsonCallback, JsonCallbackTable<VirtualOrganization>, JsonCallbackOracle<VirtualOrganization> {
    ...
    if (filter.equalsIgnoreCase("")) {
      ...
      for (VirtualOrganization vo : fullBackup) {
        ...
        if (vo.getName().toLowerCase().contains(filter.toLowerCase()) ||
            vo.getShortName().toLowerCase().contains(filter.toLowerCase())) {
          list.add(vo);
        }
      }
    }
