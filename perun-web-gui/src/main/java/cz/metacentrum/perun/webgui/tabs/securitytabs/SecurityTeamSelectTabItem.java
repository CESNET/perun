...
public class SecurityTeamSelectTabItem implements TabItem, TabItemWithUrl {
  ...
  public Widget draw() {
    ...
    tabMenu.addWidget(
        TabMenu.getPredefinedButton(ButtonType.CREATE, true, ButtonTranslation.INSTANCE.createSecurityTeam(),
            new ClickHandler() {
              @Override
    ...
    final CustomButton removeButton =
        TabMenu.getPredefinedButton(ButtonType.DELETE, buttonTranslation.deleteSecurityTeam());
  ...
  public void open() {
    ...
    session.getUiElements().getBreadcrumbs()
        .setLocation(MainMenu.SECURITY_ADMIN, "Select SecurityTeam", getUrlWithParameters());
  }
