...
public class GroupDetailTabItem implements TabItem, TabItemWithUrl {
  ...
  public Widget draw() {
    ...
    if (group.isCoreGroup()) {
      change = new CustomButton("", "Core group's name and description can't be changed",
          SmallIcons.INSTANCE.applicationFormEditIcon());
      change.setEnabled(false);
    } else {
      change = new CustomButton("", ButtonTranslation.INSTANCE.editGroupDetails(),
          SmallIcons.INSTANCE.applicationFormEditIcon());
    }
    ...
    if (JsonUtils.isExtendedInfoVisible()) {
      menu.setHTML(0, column,
          "<strong>ID:</strong><br/><span class=\"inputFormInlineComment\">" + group.getId() + "</span>");
      column++;
      menu.setHTML(0, column, "&nbsp;");
      menu.getFlexCellFormatter().setWidth(0, column, "25px");
      column++;
    ...
    menu.setHTML(0, column, "<strong>Description:</strong><br/><span class=\"inputFormInlineComment\">" +
        SafeHtmlUtils.fromString((group.getDescription() != null) ? group.getDescription() : "").asString() +
        "&nbsp;</span>");
  ...
  public void open() {
    ...
    if (vo == null) {
      new GetEntityById(PerunEntity.VIRTUAL_ORGANIZATION, group.getVoId(), new JsonCallbackEvents() {
        ...
        public void onFinished(JavaScriptObject jso) {
          vo = jso.cast();
          session.getUiElements().getBreadcrumbs().setLocation(MainMenu.GROUP_ADMIN, "VO: " + vo.getName(),
              VosTabs.URL + UrlMapper.TAB_NAME_SEPARATOR + "detail?id=" + vo.getId(), "Group: " + group.getName(),
        ...
        public void onError(PerunError error) {
          session.getUiElements().getBreadcrumbs()
              .setLocation(MainMenu.GROUP_ADMIN, "Group: " + group.getName(), getUrlWithParameters());
        }
      }
      ).retrieveData();
    } else {
