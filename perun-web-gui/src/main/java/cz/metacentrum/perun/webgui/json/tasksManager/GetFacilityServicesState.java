...
public class GetFacilityServicesState implements JsonCallback, JsonCallbackTable<ServiceState> {
  ...
  // Selection model
  final MultiSelectionModel<ServiceState> selectionModel =
      new MultiSelectionModel<ServiceState>(new ServiceStateKeyProvider());
  // Session
  ...
  public CellTable<ServiceState> getEmptyTable() {
    ...
    columnSortHandler.setComparator(blockedColumn, new Comparator<ServiceState>() {
      ...
      public int compare(ServiceState o1, ServiceState o2) {
        String val1 =
            (o1.isBlockedOnFacility()) ? "BLOCKED" : ((o1.isBlockedGlobally()) ? "BLOCKED GLOBALLY" : "ALLOWED");
        String val2 =
            (o2.isBlockedOnFacility()) ? "BLOCKED" : ((o2.isBlockedGlobally()) ? "BLOCKED GLOBALLY" : "ALLOWED");
