  ...
  public Widget draw() {
    ...
    if (user.isServiceUser()) {
      ...
      this.titleWidget.setText(
          Utils.getStrippedStringWithEllipsis(user.getFullNameWithTitles().trim()) + ": Associated users");
        ...
        public void onClick(ClickEvent clickEvent) {
          ...
          if (fullList.size() == list.size()) {
                            ...
                            for (int i = 0; i < list.size(); i++) {
                              ...
                              if (i == list.size() - 1) {
                                req = new RemoveSpecificUserOwner(
                                    JsonCallbackEvents.disableButtonEvents(removeUserButton, refreshEvents));
                              } else {
                                req = new RemoveSpecificUserOwner(
            ...
            if (containsMyself) {
              ...
              UiElements.showDeleteConfirm(list,
                  "<p style=\"color:red;\"><b>You are about to remove yourself from service identity! If you do that, you won't be able to add yourself again. Only users associated with service identity can add other users again.</b></p>" +
                      "<p>Following users will be removed from service identity and they will lose any access to it.</p>",
                  new ClickHandler() {
                    @Override
                    public void onClick(ClickEvent event) {
                      for (int i = 0; i < list.size(); i++) {
              ...
              // if not selected myself, continue same way
              UiElements.showDeleteConfirm(list,
                  "Following users will be removed from service identity and they will lose any access to it. Only users associated with service identity can add other users again.",
                  new ClickHandler() {
                    @Override
                    public void onClick(ClickEvent event) {
                      for (int i = 0; i < list.size(); i++) {
      ...
      CellTable<User> table = request.getTable(new FieldUpdater<User, String>() {
        public void update(int i, User user, String s) {
          if (session.isSelf(user.getId())) {
            ...
            UiElements.generateAlert("You are not authorized",
                "You are not authorized to view personal details of user " +
                    SafeHtmlUtils.fromString(user.getFullNameWithTitles()).asString() + ".");
          }
        }
      });
      ...
      // PERSON TYPE user
      this.titleWidget.setText(
          Utils.getStrippedStringWithEllipsis(user.getFullNameWithTitles().trim()) + ": Service identities");
      ...
      removeUserButton.addClickHandler(new ClickHandler() {
        ...
        public void onClick(ClickEvent clickEvent) {
          final ArrayList<User> list = request.getTableSelectedList();
          UiElements.showDeleteConfirm(list,
              "Following service identities will be removed from you and you will lose any access to them. Only users associated with service identity can add you again.",
              new ClickHandler() {
                @Override
                public void onClick(ClickEvent event) {
                  // TODO - SHOULD HAVE ONLY ONE CALLBACK TO CORE
    ...
    if (!user.isServiceUser()) {
      session.getUiElements().getBreadcrumbs()
          .setLocation(MainMenu.USER, Utils.getStrippedStringWithEllipsis(user.getFullNameWithTitles().trim()),
              UsersTabs.URL + UrlMapper.TAB_NAME_SEPARATOR + getUrl() + "?id=" + userId, "Service identities",
              getUrlWithParameters());
    } else {
      session.getUiElements().getBreadcrumbs()
          .setLocation(MainMenu.USER, Utils.getStrippedStringWithEllipsis(user.getFullNameWithTitles().trim()),
              UsersTabs.URL + UrlMapper.TAB_NAME_SEPARATOR + getUrl() + "?id=" + userId, "Associated users",
              getUrlWithParameters());
    }
  }
