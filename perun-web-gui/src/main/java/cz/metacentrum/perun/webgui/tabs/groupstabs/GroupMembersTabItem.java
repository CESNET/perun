...
public class GroupMembersTabItem implements TabItem, TabItemWithUrl {
  ...
  public Widget draw() {
    ...
    final CustomButton searchButton =
        TabMenu.getPredefinedButton(ButtonType.SEARCH, ButtonTranslation.INSTANCE.searchMemberInGroup());
    final CustomButton listAllButton =
        TabMenu.getPredefinedButton(ButtonType.LIST_ALL_MEMBERS, ButtonTranslation.INSTANCE.listAllMembersInGroup());
    ...
    CustomButton addButton =
        TabMenu.getPredefinedButton(ButtonType.ADD, true, ButtonTranslation.INSTANCE.addMemberToGroup(),
            new ClickHandler() {
    ...
    if (rg.isBlockManualMemberAdding() && !session.isPerunAdmin()) {
      addButton.setEnabled(false);
      addButton.setTitle(
          "This Group prevents manual addition of members. Users can use self-registration process. You can send them an invitation with registration link.");
    }
    if (rg.isSyncEnabled()) {
      addButton.setEnabled(false);
    ...
    CustomButton inviteButton =
        new CustomButton("Invite member…", SmallIcons.INSTANCE.emailAddIcon(), new ClickHandler() {
          @Override
          public void onClick(ClickEvent event) {
    ...
    final CustomButton removeButton =
        TabMenu.getPredefinedButton(ButtonType.REMOVE, ButtonTranslation.INSTANCE.removeMemberFromGroup());
    if (!session.isGroupAdmin(groupId) && !session.isVoAdmin(group.getVoId())) {
    ...
    removeButton.addClickHandler(new ClickHandler() {
      ...
      public void onClick(ClickEvent event) {
        ...
        String text =
            "Following members will be removed from group. They will lose access to resources provided by this group.";
        UiElements.showDeleteConfirm(membersForRemoving, text, new ClickHandler() {
          @Override
          public void onClick(ClickEvent clickEvent) {
            RemoveMembers request =
                new RemoveMembers(JsonCallbackEvents.disableButtonEvents(removeButton, refreshEvent));
    ...
    findMembers.setEvents(JsonCallbackEvents.mergeEvents(
        JsonCallbackEvents.disableButtonEvents(searchButton, JsonCallbackEvents.disableCheckboxEvents(disabled)),
    ...
    members.setEvents(JsonCallbackEvents.mergeEvents(
      ...
      GetGroupMembersCount count = new GetGroupMembersCount(groupId, new JsonCallbackEvents() {
        ...
        public void onFinished(JavaScriptObject jso) {
          ...
          if (membersCount > 1000) {
            ...
            HTML label = new HTML();
            label.setHTML("<h2>Group has " + membersCount +
                " members. We suggest you to use search instead.</h2><h2>You can search member by name, login, email or click List all to see all members.</h2>");
