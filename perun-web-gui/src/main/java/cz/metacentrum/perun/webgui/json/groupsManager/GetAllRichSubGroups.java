...
public class GetAllRichSubGroups implements JsonCallback, JsonCallbackTable<RichGroup>, JsonCallbackOracle<RichGroup> {
  ...
  // Selection model
  final MultiSelectionModel<RichGroup> selectionModel =
      new MultiSelectionModel<RichGroup>(new GeneralKeyProvider<RichGroup>());
  // Session
      ...
      public void update(int index, final RichGroup object, RichGroup value) {
        ...
        GetEntityById get = new GetEntityById(PerunEntity.RICH_GROUP, object.getId(), new JsonCallbackEvents() {
          ...
          public void onFinished(JavaScriptObject jso) {
            ...
            String name, syncEnabled, syncInterval, syncStartTimestamp, syncTimestamp, syncSuccessTimestamp, syncState,
                authGroup, syncTimes, syncSuccessStartTimestamp;
            name = object.getName();
            ...
            html +=
                "Sync. Times: <b>" + SafeHtmlUtils.fromString(syncTimes).asString() + "</b><br>-----------------<br>";

            if (object.isSyncEnabled()) {
              html += "Last sync. state: <b>" + SafeHtmlUtils.fromString(syncState).asString() + "</b><br>";
              html += "Last sync. timestamp (start): <b>" + SafeHtmlUtils.fromString(syncStartTimestamp).asString() +
                  "</b><br>";
              html +=
                  "Last sync. timestamp (end): <b>" + SafeHtmlUtils.fromString(syncTimestamp).asString() + "</b><br>";
              html += "Last successful sync. timestamp (start): <b>" +
                  SafeHtmlUtils.fromString(syncSuccessStartTimestamp).asString() + "</b><br>";
              html += "Last successful sync. timestamp (end): <b>" +
                  SafeHtmlUtils.fromString(syncSuccessTimestamp).asString() + "</b><br>";
            }
            ...
            final CustomButton okButton =
                new CustomButton("Force synchronization", SmallIcons.INSTANCE.arrowRefreshIcon());
            okButton.addClickHandler(new ClickHandler() {
              @Override
              public void onClick(ClickEvent event) {
                ForceGroupSynchronization call =
                    new ForceGroupSynchronization(JsonCallbackEvents.disableButtonEvents(okButton));
