...
public class GetApplicationForm implements JsonCallback {
  ...
  private void createWidget(final ApplicationForm form) {
    ...
    final CustomButton button =
        TabMenu.getPredefinedButton(ButtonType.SETTINGS, ButtonTranslation.INSTANCE.changeAppFormSettings());
    ...
    if (form != null) {
      ...
      String module =
          "</br><strong>Module names: </strong>" + SafeHtmlUtils.fromString(form.getModuleClassNames()).asString();
        ...
        ClickHandler clickCHB = new ClickHandler() {
          public void onClick(ClickEvent event) {
            ...
            if (((CheckBox) event.getSource()).getValue()) {
              AddGroupsToAutoRegistration request =
                  new AddGroupsToAutoRegistration(JsonCallbackEvents.disableCheckboxEvents(checkBox));
              request.setAutoRegGroups(list, null, null);
            } else {
              RemoveGroupsFromAutoRegistration request =
                  new RemoveGroupsFromAutoRegistration(JsonCallbackEvents.disableCheckboxEvents(checkBox));
