...
public class FacilityDestinationsTabItem implements TabItem, TabItemWithUrl {
  ...
  public static final String URL = "destinations";
  /**
   * Perun web session
   */
  ...
  public Widget draw() {
    ...
    // buttons
    menu.addWidget(TabMenu.getPredefinedButton(ButtonType.ADD, true, ButtonTranslation.INSTANCE.addDestination(),
        new ClickHandler() {
    ...
    removeButton.addClickHandler(new ClickHandler() {
      public void onClick(ClickEvent event) {
        ...
        String text =
            "<span class=\"serverResponseLabelError\"><strong>Removing destination will stop propagation of service's configuration for this destination/service.</strong></span><p>Following destinations will be removed.";
      ...
      public void onClick(ClickEvent event) {
        ...
        for (int i = 0; i < destForBlockUnblock.size(); i++) {
          if (i == destForBlockUnblock.size() - 1) {
            BlockUnblockServiceOnDestination request =
                new BlockUnblockServiceOnDestination(JsonCallbackEvents.disableButtonEvents(allowButton, events));
            request.unblockServiceOnDestination(destForBlockUnblock.get(i).getService().getId(),
                destForBlockUnblock.get(i).getId());
          } else {
            BlockUnblockServiceOnDestination request =
                new BlockUnblockServiceOnDestination(JsonCallbackEvents.disableButtonEvents(allowButton));
            request.unblockServiceOnDestination(destForBlockUnblock.get(i).getService().getId(),
                destForBlockUnblock.get(i).getId());
          }
        }
    ...
    blockButton.addClickHandler(new ClickHandler() {
      ...
      public void onClick(ClickEvent event) {
        ...
        for (int i = 0; i < destForBlockUnblock.size(); i++) {
          if (i == destForBlockUnblock.size() - 1) {
            BlockUnblockServiceOnDestination request =
                new BlockUnblockServiceOnDestination(JsonCallbackEvents.disableButtonEvents(blockButton, events));
            request.blockServiceOnDestination(destForBlockUnblock.get(i).getService().getId(),
                destForBlockUnblock.get(i).getId());
          } else {
            BlockUnblockServiceOnDestination request =
                new BlockUnblockServiceOnDestination(JsonCallbackEvents.disableButtonEvents(blockButton));
            request.blockServiceOnDestination(destForBlockUnblock.get(i).getService().getId(),
                destForBlockUnblock.get(i).getId());
          }
        }
