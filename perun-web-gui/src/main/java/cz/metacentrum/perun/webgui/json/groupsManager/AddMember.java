    ...
    JsonCallbackEvents newEvents = new JsonCallbackEvents() {
      ...
      public void onError(PerunError error) {
        session.getUiElements().setLogErrorText(
            "Adding member: " + member.getUser().getFullName() + " to group: " + group.getShortName() + " failed.");
      ...
      public void onFinished(JavaScriptObject jso) {
        session.getUiElements().setLogSuccessText(
            "Member: " + member.getUser().getFullName() + " added to group: " + group.getShortName());
  ...
  private void handleCommonExceptions(PerunError error, RichMember member, Group group) {
    ...
    if (error != null) {
      ...
      if ("WrongAttributeValueException".equals(error.getName())) {
        ...
        String text =
            member.getUser().getFullNameWithTitles() + " can't be added to group " + group.getShortName() + ".";
        ...
        if (a != null) {
          ...
          text +=
              "<p>Please fix the issue before adding member to group on member's settings page. If you are not allowed to do so, ask user to fill application form or edit own entries on user detail.";
        ...
        UiElements.generateError(error, "Already member",
            member.getUser().getFullNameWithTitles() + " is already member of group " + group.getShortName() + ".");
