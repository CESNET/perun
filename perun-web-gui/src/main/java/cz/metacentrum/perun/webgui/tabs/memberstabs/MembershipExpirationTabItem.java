...
public class MembershipExpirationTabItem implements TabItem {
  ...
  public Widget draw() {
    ...
    if (expire != null && !"null".equalsIgnoreCase(expire.getValue())) {
      ...
      expirationValue =
          expire.getValue().substring(0, ((expire.getValue().length() > 10) ? 9 : expire.getValue().length() - 1));
    } else {
    ...
    final CustomButton changeButton =
        new CustomButton("Save", "Save changes in membership expiration date", SmallIcons.INSTANCE.diskIcon());
      ...
      public void onClick(ClickEvent clickEvent) {
        if (changeOrNever) {
          ...
          SetAttributes request =
              new SetAttributes(JsonCallbackEvents.closeTabDisableButtonEvents(changeButton, tab, false, events));
          ...
          RemoveAttributes request =
              new RemoveAttributes(JsonCallbackEvents.closeTabDisableButtonEvents(changeButton, tab, false, events));
          ArrayList<Attribute> list = new ArrayList<Attribute>();
          list.add(expire);
