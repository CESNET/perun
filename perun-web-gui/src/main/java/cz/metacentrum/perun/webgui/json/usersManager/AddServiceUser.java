...
public class AddServiceUser {
    ...
    // URL to call
    final String JSON_URL = "usersManager/addServiceUser";
    // external events
     ...
     */
    public AddServiceUser(final JsonCallbackEvents events) {
        this.events = events;
    }
    ...
    private boolean testAdding()
    {
        ...
        if(user.isServiceUser()){
            errorMsg += "Can't connect two service identities.\n";
            result = false;
        }
        ...
        if(errorMsg.length()>0){
            Confirm c = new Confirm("Error while connecting identities", new HTML(errorMsg), true);
            c.show();
        }
