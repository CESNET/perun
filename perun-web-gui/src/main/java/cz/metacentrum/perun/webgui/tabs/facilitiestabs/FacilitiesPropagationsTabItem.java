...
import cz.metacentrum.perun.webgui.json.tasksManager.GetFacilityState;
...
public class FacilitiesPropagationsTabItem implements TabItem, TabItemWithUrl {
      ...
      public void onFinished(JavaScriptObject jso) {
        ...
        if (list != null && !list.isEmpty()) {
          ...
          for (final FacilityState state : clusters) {
            ...
            content.setHTML(mainrow, 0,
                "<strong>" + SafeHtmlUtils.fromString(state.getFacility().getName()).asString() + "</strong>");
            ...
            for (final String dest : destList) {
              ...
              hyp.setHTML("<span style=\"display: inline-block; width: " + width + "px; text-align: center;\">" +
                  SafeHtmlUtils.fromString((show != null) ? show : "").asString() + "</span>");
  ...
  public void open() {
    ...
    session.getUiElements().getBreadcrumbs()
        .setLocation(MainMenu.FACILITY_ADMIN, "All facilities states", getUrlWithParameters());
  }
