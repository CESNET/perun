...
import cz.metacentrum.perun.webgui.widgets.AddRemoveItemsTable;
  ...
  public Widget draw() {
        ...
        cb.addClickHandler(new ClickHandler() {
          public void onClick(ClickEvent clickEvent) {
            ...
            if (itemsTable.getList().isEmpty()) {
              new Confirm("No user associated",
                  new HTML("You must associate at least one real user to service member."), true).show();
              return;
            }

            // create member + user
            CreateSpecificMember request =
                new CreateSpecificMember(JsonCallbackEvents.disableButtonEvents(cb, new JsonCallbackEvents() {
                  public void onFinished(JavaScriptObject jso) {
                    ...
                    secondTabPanel.add(new HTML(
                        "<h3>3.Set password for: " + serviceUserLogin.getTextBox().getValue().trim() + "</h3>"));
                    ...
                    button.addClickHandler(new ClickHandler() {
                      public void onClick(ClickEvent clickEvent) {
                        ...
                        if ("mu".equals(namespaceValue)) {
                          ...
                          final GenerateAccount req = new GenerateAccount(
                              JsonCallbackEvents.disableButtonEvents(button, new JsonCallbackEvents() {
                                public void onFinished(JavaScriptObject jso) {

                                  BasicOverlayType basic = jso.cast();
                                  final String login =
                                      basic.getCustomProperty("urn:perun:user:attribute-def:def:login-namespace:mu");
                                  ...
                                  SetLogin setLogin = new SetLogin(
                                      JsonCallbackEvents.disableButtonEvents(button, new JsonCallbackEvents() {
                                          ...
                                          CreatePassword req = new CreatePassword(
                                              JsonCallbackEvents.disableButtonEvents(button, new JsonCallbackEvents() {
                                                ...
                                                public void onFinished(JavaScriptObject jso) {
                                                  ...
                                                  ValidateMemberAsync req2 = new ValidateMemberAsync(
                                                      JsonCallbackEvents.closeTabDisableButtonEvents(button, tab,
                                                          true));
                                        ...
                                        public void onError(PerunError error) {
                                          ...
                                          UiElements.generateError(error, "Saving login failed",
                                              "You were assigned with login <b>" + login +
                                                  "</b> in namespace MU but saving it to user failed. <b>Please copy your login and contact support at <a href=\"mailto:" +
                                                  Utils.perunReportEmailAddress() + "\">" +
                                                  Utils.perunReportEmailAddress() + "</a>.</b>");

                                          // validate member when all logins are set
                                          ValidateMemberAsync req2 = new ValidateMemberAsync(
                                              JsonCallbackEvents.closeTabDisableButtonEvents(button, tab, true));
                                          req2.validateMemberAsync(member);
                          ...
                          GetEntityById get = new GetEntityById(PerunEntity.RICH_MEMBER, member.getId(),
                              JsonCallbackEvents.disableButtonEvents(button, new JsonCallbackEvents() {
                                @Override
                                public void onFinished(JavaScriptObject jso) {
                                  RichMember rm = jso.cast();

                                  params.put("urn:perun:user:attribute-def:core:firstName",
                                      rm.getUser().getFirstName());
                                  ...
                                  req.generateAccount(namespaceValue, serviceUserPassword.getTextBox().getValue(),
                                      params);
                          ...
                          req.createPassword(member.getUserId(), serviceUserLogin.getTextBox().getValue().trim(),
                              namespaceValue, serviceUserPassword.getTextBox().getValue());
                    ...
                    skipButton.addClickHandler(new ClickHandler() {
                      public void onClick(ClickEvent clickEvent) {
                        ...
                        if ("mu".equals(namespaceValue)) {
                          ...
                          CreatePassword req = new CreatePassword(
                              JsonCallbackEvents.disableButtonEvents(skipButton, new JsonCallbackEvents() {
                                public void onFinished(JavaScriptObject jso) {
                                  // validate member when all kerberos logins are set
                                  ValidateMemberAsync req2 = new ValidateMemberAsync(
                                      JsonCallbackEvents.closeTabDisableButtonEvents(skipButton, tab, true));
                                  req2.validateMemberAsync(member);
                                }
                              }));
                          // set empty password for service member if "skipped"
                          req.createRandomPassword(member.getUserId(), serviceUserLogin.getTextBox().getValue().trim(),
                              namespaceValue);
        ...
        itemsTable.setEvents(new AddRemoveItemsTable.HandleItemsAction<User>() {
          ...
          public void onRemove(User object) {
            if (object.equals(session.getUser())) {
              ...
              UiElements.generateInfo("Can't remove yourself",
                  "<p>You can't remove yourself yet. You wouldn't be able to finish service member configuration. Please remove yourself afterwards.");
            }
            if (itemsTable.getList().isEmpty()) {
              cb.setEnabled(false);
            }
    ...
    serviceUserLogin.getTextBox().addKeyUpHandler(new KeyUpHandler() {
      ...
      public void onKeyUp(KeyUpEvent keyUpEvent) {
        ...
        if ((!login.isEmpty() && RegExp.compile(Utils.LOGIN_VALUE_MATCHER).test(login)) ||
            serviceUserLogin.isHardError()) {
