...
public class ServiceDestinationsTabItem implements TabItem, TabItemWithUrl {
  ...
  public Widget draw() {
    ...
    // buttons
    final CustomButton addDestButton =
        TabMenu.getPredefinedButton(ButtonType.ADD, true, ButtonTranslation.INSTANCE.addDestination());
    final CustomButton removeDestButton =
            ...
            for (int i = 0; i < destsToRemove.size(); i++) {
              if (i == destsToRemove.size() - 1) {
                RemoveDestination request =
                    new RemoveDestination(destsToRemove.get(i).getFacility().getId(), service.getId(),
                        JsonCallbackEvents.disableButtonEvents(removeDestButton, refreshEvents));
                request.removeDestination(destsToRemove.get(i).getDestination(), destsToRemove.get(i).getType());
              } else {
                RemoveDestination request =
                    new RemoveDestination(destsToRemove.get(i).getFacility().getId(), service.getId(),
                        JsonCallbackEvents.disableButtonEvents(removeDestButton));
                request.removeDestination(destsToRemove.get(i).getDestination(), destsToRemove.get(i).getType());
              }
            }
          }
      ...
      public void onClick(ClickEvent event) {
        ...
        for (int i = 0; i < destForBlockUnblock.size(); i++) {
          if (i == destForBlockUnblock.size() - 1) {
            BlockUnblockServiceOnDestination request = new BlockUnblockServiceOnDestination(
                JsonCallbackEvents.disableButtonEvents(allowButton, refreshEvents));
    ...
    blockButton.addClickHandler(new ClickHandler() {
      ...
      public void onClick(ClickEvent event) {
        ...
        for (int i = 0; i < destForBlockUnblock.size(); i++) {
          if (i == destForBlockUnblock.size() - 1) {
            BlockUnblockServiceOnDestination request = new BlockUnblockServiceOnDestination(
                JsonCallbackEvents.disableButtonEvents(blockButton, refreshEvents));
