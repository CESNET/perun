...
public class SecurityTeamBlacklistTabItem implements TabItem, TabItemWithUrl {
  ...
  public Widget draw() {
    ...
    // Get vos request
    final GetBlacklistWithDescription teams =
        new GetBlacklistWithDescription(PerunEntity.SECURITY_TEAM, securityTeamId);
    ...
    tabMenu.addWidget(
        TabMenu.getPredefinedButton(ButtonType.ADD, true, ButtonTranslation.INSTANCE.addUsersToBlacklist(),
            new ClickHandler() {
              @Override
    ...
    final CustomButton removeButton =
        TabMenu.getPredefinedButton(ButtonType.REMOVE, buttonTranslation.removeSelectedUsersFromBlacklist());
          ...
          public void onClick(ClickEvent clickEvent) {
            ...
            for (int i = 0; i < itemsToRemove.size(); i++) {
              ...
              if (i == itemsToRemove.size() - 1) {
                request = new RemoveUserFromBlacklist(securityTeamId,
                    JsonCallbackEvents.disableButtonEvents(removeButton, events));
              } else {
                request =
                    new RemoveUserFromBlacklist(securityTeamId, JsonCallbackEvents.disableButtonEvents(removeButton));
              }
