...
public class UserExtSourceDetailTabItem implements TabItem, TabItemWithUrl {
  ...
  public Widget draw() {
    ...
    if (JsonUtils.isExtendedInfoVisible()) {
      menu.setHTML(0, column,
          "<strong>UES ID:</strong><br/><span class=\"inputFormInlineComment\">" + userExtSource.getId() + "</span>");
      column++;
      menu.setHTML(0, column, "&nbsp;");
      menu.getFlexCellFormatter().setWidth(0, column, "25px");
      column++;
      menu.setHTML(0, column, "<strong>ES name:</strong><br/><span class=\"inputFormInlineComment\">" +
          SafeHtmlUtils.fromString(
                  (userExtSource.getExtSource().getName() != null) ? userExtSource.getExtSource().getName() : "")
              .asString() + "</span>");
      column++;
      menu.setHTML(0, column, "&nbsp;");
      menu.getFlexCellFormatter().setWidth(0, column, "25px");
      column++;
      menu.setHTML(0, column, "<strong>ES type:</strong><br/><span class=\"inputFormInlineComment\">" +
          SafeHtmlUtils.fromString(
                  (userExtSource.getExtSource().getType() != null) ? userExtSource.getExtSource().getType() : "")
              .asString() + "</span>");
    }
  ...
  public void open() {
    ...
    session.getUiElements().getBreadcrumbs()
        .setLocation(MainMenu.PERUN_ADMIN, "Users", UsersTabs.URL + UrlMapper.TAB_NAME_SEPARATOR + getUrl(),
            "UserExtSource detail", getUrlWithParameters());
  }
