...
public class MemberDetailTabItem implements TabItem, TabItemWithUrl {
  ...
  public Widget draw() {
    ...
    if (JsonUtils.isExtendedInfoVisible()) {
      menu.setHTML(0, column,
          "<strong>Member ID:</strong><br/><span class=\"inputFormInlineComment\">" + member.getId() + "</span>");
      column++;
      menu.setHTML(0, column, "&nbsp;");
      menu.getFlexCellFormatter().setWidth(0, column, "25px");
      column++;
      menu.setHTML(0, column,
          "<strong>User ID:</strong><br/><span class=\"inputFormInlineComment\">" + member.getUser().getId() +
              "</span>");
      column++;
      menu.setHTML(0, column, "&nbsp;");
      menu.getFlexCellFormatter().setWidth(0, column, "25px");
          ...
          new GetEntityById(PerunEntity.VIRTUAL_ORGANIZATION, member.getVoId(), new JsonCallbackEvents() {
            ...
            public void onFinished(JavaScriptObject jso) {
              ...
              session.getUiElements().getBreadcrumbs()
                  .setLocation(vo, "Members", VosTabs.URL + UrlMapper.TAB_NAME_SEPARATOR + "members?id=" + vo.getId());
            }
          }).retrieveData();
        } else {
          // display
        ...
        if (group == null) {
          ...
          new GetEntityById(PerunEntity.GROUP, groupId, new JsonCallbackEvents() {
            ...
            public void onFinished(JavaScriptObject jso) {
              ...
              session.getUiElements().getBreadcrumbs().setLocation(group, "Members",
                  GroupsTabs.URL + UrlMapper.TAB_NAME_SEPARATOR + "members?id=" + group.getId());
            }
          }).retrieveData();
        } else {
          // display
