...
public class AddUserToBlacklistTabItem implements TabItem {
  ...
  public Widget draw() {
    ...
    final CustomButton addButton =
        TabMenu.getPredefinedButton(ButtonType.ADD, ButtonTranslation.INSTANCE.addSelectedUsersToBlacklist());
          ...
          Confirm c = new Confirm("Add user(s) to blacklist", layout, new ClickHandler() {
            ...
            public void onClick(ClickEvent event) {
              for (int i = 0; i < list.size(); i++) {
                ...
                final int n = i;
                AddUserToBlacklist request = new AddUserToBlacklist(securityTeamId,
                    JsonCallbackEvents.disableButtonEvents(addButton, new JsonCallbackEvents() {
                      @Override
                      public void onFinished(JavaScriptObject jso) {
