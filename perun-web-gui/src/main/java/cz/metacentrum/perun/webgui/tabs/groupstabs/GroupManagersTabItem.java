...
public class GroupManagersTabItem implements TabItem, TabItemWithUrl {
  ...
  private Widget fillContentUsers(final GetRichAdminsWithAttributes admins, TabMenu menu) {
    ...
    final CustomButton removeButton =
        TabMenu.getPredefinedButton(ButtonType.REMOVE, ButtonTranslation.INSTANCE.removeManagerFromGroup());

    if (!isMembersGroup) {

      CustomButton addButton =
          TabMenu.getPredefinedButton(ButtonType.ADD, true, ButtonTranslation.INSTANCE.addManagerToGroup(),
              new ClickHandler() {
                      ...
                      public void onFinished(JavaScriptObject jso) {
                        ...
                        if ("vo.esc.pithia.eu".equals(vo.getShortName())) {
                          session.getTabManager()
                              .addTabToCurrentTab(new AddGroupManagerFromMembersTabItem(group), true);
                        } else {
            ...
            public void onClick(ClickEvent clickEvent) {
              ...
              for (int i = 0; i < itemsToRemove.size(); i++) {
                ...
                if (i == itemsToRemove.size() - 1) {
                  request =
                      new RemoveAdmin(PerunEntity.GROUP, JsonCallbackEvents.disableButtonEvents(removeButton, events));
                } else {
  ...
  private Widget fillContentGroups(final GetAdminGroups admins, TabMenu menu) {
    ...
    final CustomButton removeButton =
        TabMenu.getPredefinedButton(ButtonType.REMOVE, ButtonTranslation.INSTANCE.removeManagerGroupFromGroup());

    if (!isMembersGroup) {

      CustomButton addButton =
          TabMenu.getPredefinedButton(ButtonType.ADD, true, ButtonTranslation.INSTANCE.addManagerGroupToGroup(),
              new ClickHandler() {
                public void onClick(ClickEvent event) {
