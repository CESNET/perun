...
public class SelfAuthenticationsTabItem implements TabItem, TabItemWithUrl {
  ...
  public Widget draw() {
    ...
    this.titleWidget.setText(
        Utils.getStrippedStringWithEllipsis(user.getFullNameWithTitles().trim()) + ": Authentication");
    ...
    layout.setHTML(4, 0,
        "To <strong>add certificate</strong> please visit <a href=\"" + Utils.getIdentityConsolidatorLink(false) +
            "\" target=\"_blank\">identity consolidator &gt;&gt;</a> and select \"Using personal certificate\" option.<br />&nbsp;");
    ...
    final GetLogins logins = new GetLogins(userId, new JsonCallbackEvents() {
      ...
      public void onFinished(JavaScriptObject jso) {
        ...
        if (user.isServiceUser() || user.isSponsoredUser()) {
          CustomButton addLogin =
              TabMenu.getPredefinedButton(ButtonType.ADD, "Add new login (only for supported namespaces)");
        ...
        if (list != null && !list.isEmpty()) {
          ...
          for (final UserExtSource a : list) {
            if (a.getExtSource().getType().equals("cz.metacentrum.perun.core.impl.ExtSourceX509")) {
              found = true;
              tab.setHTML(i++, 0,
                  "<strong>" + SafeHtmlUtils.fromString((a.getLogin() != null) ? a.getLogin() : "").asString() +
                      "</strong>");
              tab.setHTML(i++, 0, "Issuer: " +
                  SafeHtmlUtils.fromString((a.getExtSource().getName() != null) ? a.getExtSource().getName() : "")
                      .asString());
              if (!a.isPersistent()) {
                CustomButton removeButton =
                    new CustomButton("Remove", SmallIcons.INSTANCE.deleteIcon(), new ClickHandler() {
                      @Override
                      public void onClick(ClickEvent event) {
  ...
  public void open() {
    ...
    session.getUiElements().getBreadcrumbs()
        .setLocation(MainMenu.USER, Utils.getStrippedStringWithEllipsis(user.getFullNameWithTitles().trim()),
            UsersTabs.URL + UrlMapper.TAB_NAME_SEPARATOR + getUrl() + "?id=" + userId, "Authentication",
