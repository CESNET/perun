...
public class ListBoxWithObjects<T> extends ListBox {
  ...
  public void addAllOption() {
    ...
    if (all == false && !(isEmpty())) {
      if (notSelected == true) {
        super.insertItem(WidgetTranslation.INSTANCE.listboxAll(), 1);
      } else {
        super.insertItem(WidgetTranslation.INSTANCE.listboxAll(), 0);
      }
      all = true;
    }
  }
  ...
  public void removeSelectedItem() {
    ...
    if (index == 0 && (all == true || notSelected == true)) {
      return;
    }
    if (index <= 1 && (all == true && notSelected == true)) {
      return;
    }
    super.removeItem(index);
