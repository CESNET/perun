...
import cz.metacentrum.perun.webgui.json.cabinetManager.CreatePublication;
import cz.metacentrum.perun.webgui.json.cabinetManager.DeleteAuthorship;
import cz.metacentrum.perun.webgui.json.cabinetManager.DeleteThanks;
import cz.metacentrum.perun.webgui.json.cabinetManager.FindAuthorsByPublicationId;
import cz.metacentrum.perun.webgui.json.cabinetManager.FindExternalPublications;
import cz.metacentrum.perun.webgui.json.cabinetManager.FindSimilarPublications;
...
import cz.metacentrum.perun.webgui.json.cabinetManager.GetRichThanksByPublicationId;
import cz.metacentrum.perun.webgui.json.cabinetManager.UpdatePublication;
...
public class AddPublicationsTabItem implements TabItem, TabItemWithUrl, TabItemWithHelp {
  ...
  public Widget draw() {
    ...
    if (state == State.START) {
      ...
      helpWidget.add(new HTML(
          "<ol style=\"line-height:1.5;\"><li><strong>Please select either \"Create\" or \"Import\" for new publication.</strong>" +
      ...
      helpWidget.add(new HTML(
          "<ol style=\"line-height:1.5;\"><li><strong>Enter publication title and full citation.</strong></li>" +
      ...
      helpWidget.add(new HTML(
          "<ol style=\"line-height:1.5;\"><li><strong>Select ext. publication system for searching</strong>. Currently we can import publications reported in IS MU and OBD ZČU.</li>" +
      ...
      // help widget
      helpWidget.add(new HTML(
          "<ol style=\"line-height:1.5;\"><li><strong>Click on publication title</strong> to edit it's details." +
  ...
  private void loadStartScreen() {
    ...
    importLayout.setHTML(1, 0,
        "<span class=\"inputFormInlineComment\">Are you from MU, ZCU? You can import publications you have already reported in university publication systems.</span>");
    ...
    createLayout.setHTML(1, 0,
        "<span class=\"inputFormInlineComment\">Use when you want to add custom publication or report publication created by other user.</span>");
  ...
  private void loadCreateScreen() {
    ...
    // Widgets
    final CustomButton backButton =
        TabMenu.getPredefinedButton(ButtonType.BACK, "Go back to start page - !! ALL UNSAVED CHANGES WILL BE LOST !!",
            new ClickHandler() {
              public void onClick(ClickEvent event) {
      ...
      public void onClick(ClickEvent event) {
        ...
        if (titleValidator.validateTextBox() && citeValidator.validateTextArea()) {
          ...
          CreatePublication request =
              new CreatePublication(JsonCallbackEvents.disableButtonEvents(finishButton, new JsonCallbackEvents() {
                public void onFinished(JavaScriptObject jso) {
          ...
          request.createPublication(title.getTextBox().getText().trim(), categoryId,
              Integer.parseInt(year.getValue(year.getSelectedIndex())), isbn.getText().trim(), doi.getText().trim(),
              cite.getTextArea().getText().trim());
          counter++;
    ...
    final CustomButton checkButton =
        new CustomButton("Check", "Check if same publication exist in Perun", SmallIcons.INSTANCE.booksIcon());
    filterCall.setEvents(JsonCallbackEvents.disableButtonEvents(checkButton, new JsonCallbackEvents() {
      ...
      public void onFinished(JavaScriptObject jso) {
        ...
        if (pubs == null || pubs.isEmpty()) {
          UiElements.generateInfo("No similar publications",
              "No similar publications were found. You can continue reporting new publication.");
        } else {
    ...
    checkLayout.setWidget(0, 0,
        new CustomButton("Back to input form", "Back to Create publication form.", SmallIcons.INSTANCE.arrowLeftIcon(),
            new ClickHandler() {
              @Override
              public void onClick(ClickEvent event) {
  ...
  private void loadImportScreen() {
    ...
    // CALLBACK
    final FindExternalPublications find =
        new FindExternalPublications(userId, JsonCallbackEvents.disableButtonEvents(searchButton));
  ...
  private void loadReviewScreen() {
    ...
    // WIDGETS
    CustomButton backButton =
        TabMenu.getPredefinedButton(ButtonType.BACK, "Go back to import page - !! ALL UNSAVED CHANGES WILL BE LOST !!",
            new ClickHandler() {
              public void onClick(ClickEvent event) {
                importedPublications.clear();
                hadError = false;
    ...
    // finish button
    final CustomButton finishButton = TabMenu.getPredefinedButton(ButtonType.FINISH,
        "Finish adding and close tab - !! ALL UNSAVED CHANGES WILL BE LOST !!", new ClickHandler() {
          public void onClick(ClickEvent event) {
    ...
    final HTML errorText = new HTML(
        "<h4 style=\"font-size: 12pt; color:red\">Some publications were not imported! Please review imported publication(s) first. Click on publication title to see it's details.</h4>");
    final HTML successText = new HTML(
        "<h4 style=\"font-size: 12pt; color:red\">Please review created publication(s). Click on publication title to see it's details.</h4>");

    if (importedPublications.isEmpty()) {
      scrollContent.add(new HTML(
          "<p style=\"font-size: 12pt; font-weight:bold; text-align:center; color:red\">No publication(s) created! Please go back and retry!</p>"));
    } else {
      if (hadError) {
    ...
    for (final Publication pub : importedPublications) {
      ...
      // button
      thanksMenu.addWidget(
          TabMenu.getPredefinedButton(ButtonType.ADD, true, "Add acknowledgement to publication", new ClickHandler() {
            public void onClick(ClickEvent event) {
      ...
      final CustomButton removeThanksButton =
      ...
      removeThanksButton.addClickHandler(new ClickHandler() {
        public void onClick(ClickEvent event) {
          ...
          String text =
              "Following acknowledgements will be removed from publication. Publication authors may loose benefits granted by them.";
      ...
      final CustomButton addMyselfButton =
          new CustomButton("Add myself", "Add you as author", SmallIcons.INSTANCE.addIcon());
      addMyselfButton.addClickHandler(new ClickHandler() {
        public void onClick(ClickEvent event) {
          if (session.isPerunAdmin() && session.getUser() == null) {
            ...
            JsonCallbackEvents events = JsonCallbackEvents.disableButtonEvents(addMyselfButton,
                JsonCallbackEvents.refreshTableEvents(authorCall));
      ...
      CustomButton addOthersButton =
          new CustomButton("Add others…", "Add more authors", SmallIcons.INSTANCE.addIcon(), new ClickHandler() {
            public void onClick(ClickEvent event) {
      ...
      final CustomButton removeAuthorButton =
          TabMenu.getPredefinedButton(ButtonType.REMOVE, "Remove selected author(s) from publication");
      removeAuthorButton.addClickHandler(new ClickHandler() {
        public void onClick(ClickEvent event) {
          ...
          String text =
              "Following authors will be removed from publication. They will lose all benefits granted by reporting this publication.";
          UiElements.showDeleteConfirm(list, text, new ClickHandler() {
            @Override
            public void onClick(ClickEvent event) {
              // TODO - SHOULD HAVE ONLY ONE CALLBACK TO CORE
              for (int i = 0; i < list.size(); i++) {
                if (i == list.size() - 1) {
                  DeleteAuthorship request =
                      new DeleteAuthorship(JsonCallbackEvents.disableButtonEvents(removeAuthorButton, events));
                  request.deleteAuthorship(pub.getId(), list.get(i).getId());
                } else {
                  DeleteAuthorship request =
                      new DeleteAuthorship(JsonCallbackEvents.disableButtonEvents(removeAuthorButton));
                  request.deleteAuthorship(pub.getId(), list.get(i).getId());
                }
              }
            }
        ...
        public void onClick(ClickEvent event) {
          ...
          if (pub.isLocked()) {
            ...
            if (importedPublications.isEmpty()) {
              ...
              scrollContent.add(new HTML(
                  "<p style=\"font-size: 12pt; font-weight:bold; text-align:center; color:darkgreen\">You are now done. <br/><br/>Please click on Finish or Back button (close tab / repeat create or import publication).</p>"));
            }
          } else {
            ...
            if (!authorFound) {
              UiElements.generateAlert("You are not set as an author",
                  "<p>You are not set as an author of this publication. If you submit it like this, you won't be able to edit it afterwards.",
                  new ClickHandler() {
                    @Override
                    public void onClick(ClickEvent event) {
                      saveSinglePublication(pub, categoryBox.getSelectedObject().getId(), cite.getText(), pubHeader,
                          splitter, pubTable, errorIcon, errorText, successText, finishButton, scrollContent);
                    }
                  });
            } else {
              saveSinglePublication(pub, categoryBox.getSelectedObject().getId(), cite.getText(), pubHeader, splitter,
                  pubTable, errorIcon, errorText, successText, finishButton, scrollContent);
            }
          }
        }
      });
  ...
  private void saveSinglePublication(Publication pub, int categoryId, String cite, TabMenu pubHeader, HTML splitter,
                                     FlexTable pubTable,
                                     Image errorIcon, HTML errorText, HTML successText, CustomButton finishButton,
                                     VerticalPanel scrollContent) {
  ...
  public void open() {
    ...
    session.getUiElements().getBreadcrumbs().setLocation(MainMenu.USER, "My publications",
        CabinetTabs.URL + UrlMapper.TAB_NAME_SEPARATOR + "userpubs?user=" + userId, "Add publication",
