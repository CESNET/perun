...
public class SecurityTeamMembersTabItem implements TabItem, TabItemWithUrl {
  ...
  public static final String URL = "mem";
  /**
   * Perun web session
   */
  ...
  public Widget draw() {
    ...
    tabMenu.addWidget(
        TabMenu.getPredefinedButton(ButtonType.ADD, true, ButtonTranslation.INSTANCE.addMemberToSecurityTeam(),
            new ClickHandler() {
              @Override
    ...
    final CustomButton removeButton =
        TabMenu.getPredefinedButton(ButtonType.REMOVE, buttonTranslation.removeMemberOfSecurityTeam());
          ...
          public void onClick(ClickEvent clickEvent) {
            ...
            for (int i = 0; i < itemsToRemove.size(); i++) {
              ...
              if (i == itemsToRemove.size() - 1) {
                request = new RemoveAdmin(PerunEntity.SECURITY_TEAM,
                    JsonCallbackEvents.disableButtonEvents(removeButton, events));
              } else {
                request =
                    new RemoveAdmin(PerunEntity.SECURITY_TEAM, JsonCallbackEvents.disableButtonEvents(removeButton));
              }
