...
public class ManageGroupsBeforeAssigning implements TabItem, TabItemWithUrl {
  ...
  public boolean isPrepared() {
    ...
    if (resource == null || allGroups == null) {
      return false;
    }
    return true;
  }
  ...
  public Widget draw() {
    ...
    final CustomButton finishAssigningButton =
        TabMenu.getPredefinedButton(ButtonType.FINISH, ButtonTranslation.INSTANCE.finishGroupAssigning());

    final JsonCallbackEvents closeTabEvents =
        JsonCallbackEvents.closeTabDisableButtonEvents(finishAssigningButton, this, true);
    ...
    finishAssigningButton.addClickHandler(new ClickHandler() {
      public void onClick(ClickEvent event) {
        ...
        if (groupsDropDown.getSelectedIndex() == 0) {
          ...
          AssignGroupToResources request =
              new AssignGroupToResources(JsonCallbackEvents.disableButtonEvents(finishAssigningButton, localEvents));
    ...
    groupsDropDown.addChangeHandler(new ChangeHandler() {
      public void onChange(ChangeEvent event) {
        ...
        if (groupsDropDown.getSelectedIndex() > 0) {
          ...
          // some group selected
          TabItem ti =
              new GroupResourceRequiredAttributesTabItem(resourceId, groupsDropDown.getSelectedObject().getId());
          ti.draw();
...
*/
  ...
  public String getUrlWithParameters() {
    return ResourcesTabs.URL + UrlMapper.TAB_NAME_SEPARATOR + getUrl() + "?resId=" + resourceId + "&" +
        UrlMapper.getUrlFromList("to-assign", groupsToAssign) + "&length=" + groupsToAssign.size();
  }
