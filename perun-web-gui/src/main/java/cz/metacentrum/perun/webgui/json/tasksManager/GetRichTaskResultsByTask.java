 ...
 */
public class GetRichTaskResultsByTask
  ...
  public CellTable<TaskResult> getEmptyTable() {
    ...
    Column<TaskResult, TaskResult> standardMessageColumn =
        JsonUtils.addCustomCellColumn(new AbstractCell<TaskResult>() {
          @Override
            ...
            if (taskResult != null) {
              if (taskResult.getStandardMessage().length() > STANDARD_MESSAGE_LIMIT) {
                safeHtmlBuilder.appendEscapedLines(
                    taskResult.getStandardMessage().substring(0, STANDARD_MESSAGE_LIMIT) + "…");
              } else {
