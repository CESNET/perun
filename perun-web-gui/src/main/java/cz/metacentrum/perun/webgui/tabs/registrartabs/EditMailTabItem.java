...
public class EditMailTabItem implements TabItem, TabItemWithUrl {
  ...
  private Widget availableTagsTab() {
    ...
    ft.setHTML(0, 0,
        "Following tags can be used in mail's subject and text and are replaced by actual data on sending. Just copy/paste tags from here to input form. When no data for tag is found, it's replaced by whitespace.");
  ...
  public Widget draw() {
    ...
    saveButton =
        TabMenu.getPredefinedButton(ButtonType.SAVE, ButtonTranslation.INSTANCE.saveEmailNotificationForApplication(),
            new ClickHandler() {
              public void onClick(ClickEvent event) {
                ...
                // request
                UpdateApplicationMail req =
                    new UpdateApplicationMail(JsonCallbackEvents.closeTabDisableButtonEvents(saveButton, tab, true));
