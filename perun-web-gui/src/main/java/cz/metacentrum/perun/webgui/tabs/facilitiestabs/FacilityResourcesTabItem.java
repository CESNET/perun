...
public class FacilityResourcesTabItem implements TabItem, TabItemWithUrl {
  ...
  public Widget draw() {
    ...
    menu.addWidget(TabMenu.getPredefinedButton(ButtonType.CREATE, true, ButtonTranslation.INSTANCE.createResource(),
        new ClickHandler() {
            ...
            for (int i = 0; i < toDelete.size(); i++) {
              if (i == toDelete.size() - 1) {
                DeleteResource request =
                    new DeleteResource(JsonCallbackEvents.disableButtonEvents(deleteButton, events));
