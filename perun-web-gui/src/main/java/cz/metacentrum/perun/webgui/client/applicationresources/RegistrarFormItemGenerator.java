...
public class RegistrarFormItemGenerator {
  ...
  public RegistrarFormItemGenerator(final ApplicationFormItem item, String strValue, String locale) {
    ...
    this.item = item;
    if ((!Utils.getNativeLanguage().isEmpty() && locale.equalsIgnoreCase(Utils.getNativeLanguage().get(0))) ||
    ...
    return new FormInputChecker() {
      ...
      public boolean isValid(boolean forceNewValidation) {
        ...
        if (!valid && !item.getDisabled().equalsIgnoreCase("ALWAYS")) {
          statusCellWrapper.setWidget(
              new FormInputStatusWidget(ApplicationMessages.INSTANCE.missingValue(), Status.ERROR));
          return false;
        }
  ...
  private Widget generateEmailTextBox() {
    ...
    inputChecker = new FormInputChecker() {
      ...
      public boolean isValid(boolean forceNewValidation) {
        ...
        if (!valid) {
          statusCellWrapper.setWidget(
              new FormInputStatusWidget(ApplicationMessages.INSTANCE.incorrectEmailFormat(), Status.ERROR));
    ...
    inputChecker = new FormInputChecker() {
      ...
      public boolean isValid(boolean forceNewValidation) {
        ...
        if (!pwdbox1.getText().equals(pwdbox2.getText())) {
          statusCellWrapper.setWidget(
              new FormInputStatusWidget(ApplicationMessages.INSTANCE.passwordsDontMatch(), Status.ERROR));
          valid = false;
          return false;
        }
  ...
  private Widget generateUsernameBox() {
    ...
    if (!tbox.getValue().equalsIgnoreCase("")) {
      ...
      if (item.getPerunDestinationAttribute() != null && !item.getPerunDestinationAttribute().isEmpty()) {
        ...
        final String loginNamespace =
            item.getPerunDestinationAttribute().substring(PERUN_ATTRIBUTE_LOGIN_NAMESPACE_POSITION);
          ...
          public boolean isValid(boolean forceNewValidation) {
            ...
            if (validMap.containsKey(str)) {
              ...
              if (valid) {
                ...
                statusCellWrapper.setWidget(
                    new FormInputStatusWidget(ApplicationMessages.INSTANCE.usernameAvailable(), Status.OK));
              } else {

                statusCellWrapper.setWidget(
                    new FormInputStatusWidget(ApplicationMessages.INSTANCE.usernameNotAvailable(), Status.ERROR));
              }
              return valid;
            }
            ...
            statusCellWrapper.setWidget(
                new FormInputStatusWidget(ApplicationMessages.INSTANCE.validating(), Status.LOADING));
            ...
            new IsLoginAvailable(loginNamespace, str, new JsonCallbackEvents() {
              ...
              public void onError(PerunError error) {
                ...
                if ("InvalidLoginException".equalsIgnoreCase(error.getName())) {
                  ...
                  statusCellWrapper.setWidget(
                      new FormInputStatusWidget("Login has invalid syntax." + error.getErrorInfo(), Status.ERROR));
                } else {
                  // generic error
                  statusCellWrapper.setWidget(
                      new FormInputStatusWidget("Unable to check if login is available!", Status.ERROR));
                }
                validating = false;
                validationTrigger.triggerValidation();
  ...
  private void setVariableWidth(TextBox box) {
    ...
    if (box.getText().length() != 0) {
      ...
      } else if (box.getText().length() * WIDTH_PER_CHAR > MIN_WIDTH &&
          box.getText().length() * WIDTH_PER_CHAR <= MAX_WIDTH) {
