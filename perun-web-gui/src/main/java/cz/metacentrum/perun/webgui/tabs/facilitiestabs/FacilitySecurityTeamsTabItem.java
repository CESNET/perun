...
public class FacilitySecurityTeamsTabItem implements TabItem, TabItemWithUrl {
  ...
  public Widget draw() {
    ...
    menu.addWidget(TabMenu.getPredefinedButton(ButtonType.ADD, true, ButtonTranslation.INSTANCE.assignSecurityTeam(),
        new ClickHandler() {
          @Override
    ...
    final CustomButton removeButton =
        TabMenu.getPredefinedButton(ButtonType.REMOVE, ButtonTranslation.INSTANCE.removeSelectedSecurityTeams());
            ...
            for (int i = 0; i < list.size(); i++) {
              if (i == list.size() - 1) {
                RemoveSecurityTeam request = new RemoveSecurityTeam(JsonCallbackEvents.disableButtonEvents(removeButton,
                    JsonCallbackEvents.refreshTableEvents(securityTeams)));
                request.removeSecurityTeam(facilityId, list.get(i).getId());
              } else {
                RemoveSecurityTeam request =
                    new RemoveSecurityTeam(JsonCallbackEvents.disableButtonEvents(removeButton));
                request.removeSecurityTeam(facilityId, list.get(i).getId());
              }
            }
          }
