...
public class CreateMailTabItem implements TabItem {
  ...
  private Widget availableTagsTab() {
    ...
    ft.setHTML(0, 0,
        "Following tags can be used in mail's subject and text and are replaced by actual data on sending. Just copy/paste tags from here to input form. When no data for tag is found, it's replaced by whitespace.<p><strong>For user invitation email there is only limited set of tags, see end of this help.</strong>");
  ...
  public Widget draw() {
    ...
    saveButton = TabMenu.getPredefinedButton(ButtonType.CREATE,
        ButtonTranslation.INSTANCE.createEmailNotificationForApplication(), new ClickHandler() {
          public void onClick(ClickEvent event) {
            ...
            // request
            AddApplicationMail req =
                new AddApplicationMail(entity, JsonCallbackEvents.closeTabDisableButtonEvents(saveButton, tab, true));
