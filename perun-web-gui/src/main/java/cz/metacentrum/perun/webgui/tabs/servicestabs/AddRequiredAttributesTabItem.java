...
public class AddRequiredAttributesTabItem implements TabItem {
  ...
  public Widget draw() {
    ...
    final CustomButton addButton =
        TabMenu.getPredefinedButton(ButtonType.ADD, ButtonTranslation.INSTANCE.addSelectedRequiredAttribute());
      ...
      public void onClick(ClickEvent event) {
        ...
        if (UiElements.cantSaveEmptyListDialogBox(attributesToAdd)) {
          ...
          for (int i = 0; i < attributesToAdd.size(); i++) {
            final int n = i;
            AddRequiredAttribute request =
                new AddRequiredAttribute(JsonCallbackEvents.disableButtonEvents(addButton, new JsonCallbackEvents() {
                  @Override
                  public void onFinished(JavaScriptObject jso) {
