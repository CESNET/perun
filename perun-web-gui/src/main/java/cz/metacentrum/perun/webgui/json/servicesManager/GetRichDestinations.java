...
public class GetRichDestinations
    implements JsonCallback, JsonCallbackTable<Destination>, JsonCallbackOracle<Destination> {

  // Selection model
  final MultiSelectionModel<Destination> selectionModel =
      new MultiSelectionModel<Destination>(new DestinationKeyProvider());
  ...
  public CellTable<Destination> getEmptyTable() {
    ...
    Column<Destination, String> facilityColumn = JsonUtils.addColumn(new JsonUtils.GetValue<Destination, String>() {
      public String getValue(Destination dest) {
        if (dest.getService() != null) {
          return dest.getFacility().getName();
        } else {
          return "";
        }
      }
