...
public class ResourceDetailTabItem implements TabItem, TabItemWithUrl {
  ...
  public Widget draw() {
    ...
    CustomButton change = new CustomButton("", ButtonTranslation.INSTANCE.editResourceDetails(),
    ...
    if (JsonUtils.isExtendedInfoVisible()) {
      menu.setHTML(0, column,
          "<strong>ID:</strong><br/><span class=\"inputFormInlineComment\">" + resource.getId() + "</span>");
      column++;
      menu.setHTML(0, column, "&nbsp;");
      menu.getFlexCellFormatter().setWidth(0, column, "25px");
      column++;
    ...
    if (facilityId > 0) {
      // facility admin view
      menu.setHTML(0, column,
          "<strong>VO:</strong><br/><span class=\"inputFormInlineComment\">" + resource.getVoId() + " / " +
              SafeHtmlUtils.fromString(resource.getVo().getShortName()).asString() + "</span>");
      column++;
      menu.setHTML(0, column, "&nbsp;");
      menu.getFlexCellFormatter().setWidth(0, column, "25px");
      column++;
    ...
    menu.setHTML(0, column, "<strong>Description:</strong><br/><span class=\"inputFormInlineComment\">" +
        SafeHtmlUtils.fromString((resource.getDescription() != null) ? resource.getDescription() : "").asString() +
        "&nbsp;</span>");
  ...
  public void open() {
    if (facilityId == 0) {
      ...
      session.getUiElements().getBreadcrumbs().setLocation(resource.getVo(), "Resources",
          VosTabs.URL + UrlMapper.TAB_NAME_SEPARATOR + VoResourcesTabItem.URL + "?id=" + resource.getVoId());
      ...
      session.getUiElements().getBreadcrumbs().setLocation(resource.getFacility(), "Resources",
          FacilitiesTabs.URL + UrlMapper.TAB_NAME_SEPARATOR + FacilityResourcesTabItem.URL + "?id=" +
  ...
  public boolean isAuthorized() {
    ...
    if (session.isVoAdmin(resource.getVoId()) || session.isVoObserver(resource.getVoId()) ||
        session.isFacilityAdmin(resource.getFacilityId())) {
      return true;
    } else {
      return false;
    }
