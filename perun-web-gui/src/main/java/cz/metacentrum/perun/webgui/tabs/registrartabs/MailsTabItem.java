...
import cz.metacentrum.perun.webgui.json.registrarManager.DeleteApplicationMail;
import cz.metacentrum.perun.webgui.json.registrarManager.GetApplicationForm;
import cz.metacentrum.perun.webgui.json.registrarManager.GetApplicationMails;
import cz.metacentrum.perun.webgui.json.registrarManager.SetSendingEnabled;
...
public class MailsTabItem implements TabItem, TabItemWithUrl {
  ...
  public static final String URL = "app-mails";
  int entityId = 0;
  /**
   * Perun web session
   */
  ...
  public Widget draw() {
    ...
    // add button
    menu.addWidget(
        TabMenu.getPredefinedButton(ButtonType.ADD, true, ButtonTranslation.INSTANCE.addMail(), new ClickHandler() {
    ...
    final CustomButton removeButton =
        TabMenu.getPredefinedButton(ButtonType.REMOVE, ButtonTranslation.INSTANCE.removeMail());
        ...
        UiElements.showDeleteConfirm(list, text, new ClickHandler() {
          ...
          public void onClick(ClickEvent event) {
            for (int i = 0; i < list.size(); i++) {
              if (i != list.size() - 1) {
                DeleteApplicationMail request =
                    new DeleteApplicationMail(entity, JsonCallbackEvents.disableButtonEvents(removeButton));
                request.deleteMail(list.get(i).getId(), entityId);
              } else {
                // refresh table on last call
                DeleteApplicationMail request = new DeleteApplicationMail(entity,
                    JsonCallbackEvents.disableButtonEvents(removeButton,
                        JsonCallbackEvents.refreshTableEvents(mailsRequest)));
                request.deleteMail(list.get(i).getId(), entityId);
              }
            }
          }
    ...
    CustomButton enableButton =
        TabMenu.getPredefinedButton(ButtonType.ENABLE, ButtonTranslation.INSTANCE.enableMail(), new ClickHandler() {
          public void onClick(ClickEvent event) {
    ...
    CustomButton disableButton =
        TabMenu.getPredefinedButton(ButtonType.DISABLE, ButtonTranslation.INSTANCE.disableMail(), new ClickHandler() {
    ...
    menu.addWidget(new CustomButton(ButtonTranslation.INSTANCE.mailFooterButton() + "…",
        ButtonTranslation.INSTANCE.editMailFooter(), SmallIcons.INSTANCE.emailIcon(), new ClickHandler() {
      public void onClick(ClickEvent event) {
        if (group == null) {
    ...
    if (group == null) {
      copy = new CustomButton(ButtonTranslation.INSTANCE.copyFromVoButton() + "…",
          ButtonTranslation.INSTANCE.copyMailsFromVo(), SmallIcons.INSTANCE.copyIcon(), new ClickHandler() {
        public void onClick(ClickEvent event) {
          session.getTabManager().addTabToCurrentTab(new CopyMailsTabItem(voId, 0));
        }
      });
    } else {
      copy = new CustomButton(ButtonTranslation.INSTANCE.copyFromGroupButton() + "…",
          ButtonTranslation.INSTANCE.copyMailsFromGroup(), SmallIcons.INSTANCE.copyIcon(), new ClickHandler() {
        public void onClick(ClickEvent event) {
