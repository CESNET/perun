...
public class PerunAttributeValueCell extends AbstractSafeHtmlCell<Attribute> {
  ...
  static public final String LIST_ITEM_TABLE_ROW = "<tr %s>"
      ...
      + "<td class=\"PerunAttributeRemoveButton\"><button title=\"" + WidgetTranslation.INSTANCE.removeValue() +
      "\" class=\"gwt-Button PerunAttributeControllButton\" onclick=\"var parent = $(this).parents('.perunAttributeCell'); $(this).parent().parent().remove(); var event = jQuery.Event('change'); event.currentTarget = event.target = parent[0]; parent.trigger(event); \">" +
	...
	*/
  static public final String MAP_ITEM_TABLE_ROW = "<tr %s>" +
      "<td><input style=\"width:200px\" type=\"text\" class=\"map-entry-key gwt-TextBox\" value=\"%s\" onkeydown=\"var event2 = jQuery.Event('keydown'); event2.keyCode = event.keyCode; jQuery(this).parents('.perunAttributeCell').keydown(); jQuery(this).parents('.perunAttributeCell').trigger(event2); \" onchange=\"$(this).parents('.perunAttributeCell').change();\" onclick=\"$(this).parents('.perunAttributeCell').click();\" onblur=\"$(this).parents('.perunAttributeCell').blur();\" onfocus=\"$(this).parents('.perunAttributeCell').focus();\" /></td>" +
      "<td>=</td>" +
      "<td><input style=\"width:200px\" type=\"text\" class=\"map-entry-value gwt-TextBox\" value=\"%s\" onkeydown=\"var event2 = jQuery.Event('keydown'); event2.keyCode = event.keyCode; jQuery(this).parents('.perunAttributeCell').keydown(); jQuery(this).parents('.perunAttributeCell').trigger(event2);\" onchange=\"$(this).parents('.perunAttributeCell').change();\" onclick=\"$(this).parents('.perunAttributeCell').click();\" onblur=\"$(this).parents('.perunAttributeCell').blur();\" onfocus=\"$(this).parents('.perunAttributeCell').focus();\" /></td>" +
      "<td class=\"PerunAttributeRemoveButton\"><button title=\"" + WidgetTranslation.INSTANCE.removeValue() +
      "\" class=\"gwt-Button PerunAttributeControllButton\" onclick=\"var parent = $(this).parents('.perunAttributeCell'); $(this).parent().parent().remove(); var event = jQuery.Event('change'); event.currentTarget = event.target = parent[0]; parent.trigger(event); \">" +
      REMOVE_ICON + "</button></td>" + "</tr>";
  static public final String LIST_ITEM_TABLE_ROW_READONLY = "<tr %s>"
      + "<td><input title=\"" + WidgetTranslation.INSTANCE.notWritable() +
      "\" readonly onkeydown=\"var event2 = jQuery.Event('keydown'); event2.keyCode = event.keyCode; jQuery(this).parents('.perunAttributeCell').keydown(); jQuery(this).parents('.perunAttributeCell').trigger(event2);\" onchange=\"$(this).parents('.perunAttributeCell').change();\" onclick=\"$(this).parents('.perunAttributeCell').click();\" onblur=\"$(this).parents('.perunAttributeCell').blur();\" onfocus=\"$(this).parents('.perunAttributeCell').focus();\" style=\"width:250px\" type=\"text\" class=\"list-item-value gwt-TextBox gwt-TextBox-readonly\" value=\"%s\" /></td>"
      + "<td class=\"PerunAttributeRemoveButton\"><button title=\"" + WidgetTranslation.INSTANCE.notWritable() +
      "\" class=\"gwt-Button PerunAttributeControllButton customButtonImageDisabled\">" + REMOVE_ICON + "</button></td>"
      + "</tr>";
  static public final String MAP_ITEM_TABLE_ROW_READONLY =
      "<tr %s>" + "<td><input style=\"width:200px\" title=\"" + WidgetTranslation.INSTANCE.notWritable() +
          "\" readonly type=\"text\" class=\"map-entry-key gwt-TextBox gwt-TextBox-readonly\" value=\"%s\" onkeydown=\"var event2 = jQuery.Event('keydown'); event2.keyCode = event.keyCode; jQuery(this).parents('.perunAttributeCell').keydown(); jQuery(this).parents('.perunAttributeCell').trigger(event2); \" onchange=\"$(this).parents('.perunAttributeCell').change();\" onclick=\"$(this).parents('.perunAttributeCell').click();\" onblur=\"$(this).parents('.perunAttributeCell').blur();\" onfocus=\"$(this).parents('.perunAttributeCell').focus();\" /></td>" +
          "<td>=</td>" + "<td><input style=\"width:200px\" title=\"" + WidgetTranslation.INSTANCE.notWritable() +
          "\" readonly type=\"text\" class=\"map-entry-value gwt-TextBox gwt-TextBox-readonly\" value=\"%s\" onkeydown=\"var event2 = jQuery.Event('keydown'); event2.keyCode = event.keyCode; jQuery(this).parents('.perunAttributeCell').keydown(); jQuery(this).parents('.perunAttributeCell').trigger(event2);\" onchange=\"$(this).parents('.perunAttributeCell').change();\" onclick=\"$(this).parents('.perunAttributeCell').click();\" onblur=\"$(this).parents('.perunAttributeCell').blur();\" onfocus=\"$(this).parents('.perunAttributeCell').focus();\" /></td>" +
          "<td class=\"PerunAttributeRemoveButton\"><button title=\"" + WidgetTranslation.INSTANCE.notWritable() +
          "\" class=\"gwt-Button PerunAttributeControllButton customButtonImageDisabled\" \">" + REMOVE_ICON +
          "</button></td>" + "</tr>";
  ...
  private static String generateTextBox(String value, boolean writable) {
    ...
    if (writable) {
      ...
      return "<input title=\"" + WidgetTranslation.INSTANCE.notWritable() +
          "\" readonly type=\"text\" style=\"width:250px\" class=\"textbox-value gwt-TextBox gwt-TextBox-readonly\" value=\"" +
          value + "\" />";
    }
  ...
  private static String generateNumberBox(String value, boolean writable) {
    ...
    if (writable) {
      ...
      return "<input title=\"" + WidgetTranslation.INSTANCE.notWritable() +
          "\" readonly type=\"text\" style=\"width:100px\" class=\"numberbox-value gwt-TextBox gwt-TextBox-readonly\" value=\"" +
          value + "\" />";
    }
  ...
  private static String generateCheckBox(String value, boolean writable) {
    ...
    if (writable) {
      ...
      return "<input title=\"" + WidgetTranslation.INSTANCE.notWritable() +
          "\" readonly disabled=\"disabled\" type=\"checkbox\" class=\"checkbox-value gwt-CheckBox gwt-CheckBox-readonly\" " +
          value + " />";
    }
  ...
  private static String generateList(JsArrayString list, boolean writable) {
    ...
    if (writable) {
      output +=
          JsonUtils.stringFormat(LIST_ITEM_TABLE_ROW, " style=\"display:none;\" class=\"attribute-list-item-source\"",
              "");
      output += "<tr>";
      output +=
          "<td colspan=\"2\" style=\"text-align:center\"><button title=\"" + WidgetTranslation.INSTANCE.addValue() +
              "\" class=\"PerunAttributeAddButton gwt-Button PerunAttributeControllButton\" onclick=\"$(this).parent().parent().before('<tr>' + $('.attribute-list-item-source').html() + '</tr>');\">" +
              ADD_ICON + "</button></td>";

    } else {
      output += "<tr>";
      output +=
          "<td colspan=\"2\" style=\"text-align:center\"><button title=\"" + WidgetTranslation.INSTANCE.notWritable() +
              "\" class=\"PerunAttributeAddButton gwt-Button PerunAttributeControllButton customButtonImageDisabled\">" +
              ADD_ICON + "</button></td>";
    }
  ...
  private static String generateMap(Map<String, JSONValue> map, boolean writable) {
    ...
    if (writable) {
      output +=
          JsonUtils.stringFormat(MAP_ITEM_TABLE_ROW, " style=\"display:none;\" class=\"attribute-map-item-source\"",
              "Enter new key first!", "Then enter new value.");
    }
    ...
    if (writable) {
      ...
      output +=
          "<td colspan=\"3\" style=\"text-align:center\"><button title=\"" + WidgetTranslation.INSTANCE.addValue() +
              "\" class=\"PerunAttributeAddButton gwt-Button PerunAttributeControllButton\" onclick=\"$(this).parent().parent().before('<tr>' + $('.attribute-map-item-source').html() + '</tr>'); $(this).parent().parent().prev().addClass('map-entry');   \">" +
              ADD_ICON + "</button></td>";
    } else {
      output +=
          "<td colspan=\"3\" style=\"text-align:center\"><button title=\"" + WidgetTranslation.INSTANCE.notWritable() +
              "\" class=\"PerunAttributeAddButton gwt-Button PerunAttributeControllButton customButtonImageDisabled\">" +
              ADD_ICON + "</button></td>";
    }
   ...
   */
  @Override
  public void onBrowserEvent(Context context, Element parent, Attribute value, NativeEvent event,
                             ValueUpdater<Attribute> valueUpdater) {
   ...
   */
  @Override
  protected void onEnterKeyDown(Context context, Element parent, Attribute value, NativeEvent event,
                                ValueUpdater<Attribute> valueUpdater) {
    editing = false;
    valueChanged(value, valueUpdater);
  }
