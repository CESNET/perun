...
public class FindPublicationsByGUIFilter implements JsonCallback, JsonCallbackTable<Publication> {
  ...
  // json url
  static private final String JSON_URL = "cabinetManager/findPublicationsByGUIFilter";
  // Selection model
  final MultiSelectionModel<Publication> selectionModel =
      new MultiSelectionModel<Publication>(new GeneralKeyProvider<Publication>());
  // session
  ...
  public CellTable<Publication> getEmptyTable() {
    ...
    Column<Publication, ImageResource> lockedColumn =
        new Column<Publication, ImageResource>(new CustomImageResourceCell("click")) {
          ...
          public void onBrowserEvent(final Context context, final Element elem, final Publication object,
                                     NativeEvent event) {
