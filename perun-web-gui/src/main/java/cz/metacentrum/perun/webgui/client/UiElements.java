...
public class UiElements {
  ...
  private HashMap<Integer, Widget> allTabs = new HashMap<Integer, Widget>();
      // key is UNIQUE ID, value is content widget
  ...
  public UiElements(AbsolutePanel cp) {
    ...
    UiElements.addResizeCommand(new Command() {
      public void execute() {
        int clientWidth =
            (Window.getClientWidth() > WebGui.MIN_CLIENT_WIDTH) ? Window.getClientWidth() : WebGui.MIN_CLIENT_WIDTH;
        // 90 = buttons width
        // 208 = main menu width
  ...
  static public void generateAlert(String header, String text, ClickHandler okClickHandler,
    ...
    Confirm c = new Confirm(header, layout, okClickHandler, cancelClickHandler, true);
    c.setNonScrollable(true);
    c.show();
  ...
  static public void generateError(final PerunError error, String header, String text) {
    ...
    Confirm conf;
    if (error != null && "0".equals(error.getErrorId())) {
      conf = new Confirm(header + ((!error.getErrorId().equals("")) ? " (" + error.getErrorId() + ")" : ""), layout,
          okClickHandler, true);
    } else {
      conf = new Confirm(header + ((!error.getErrorId().equals("")) ? " (" + error.getErrorId() + ")" : ""), layout,
          okClickHandler, reportClickHandler, okLabel, reportLabel, true);
   ...
   * @param okClickHandler OK action aka delete action itself
   */
  static public <T extends JavaScriptObject> void showDeleteConfirm(ArrayList<T> list, String text,
      ...
      for (JavaScriptObject object : list) {
        ...
        if (go.getObjectType().equalsIgnoreCase("RichMember")) {
          ...
          items = items.concat("<li>" +
              SafeHtmlUtils.fromString((rm.getUser().getFullName() != null) ? rm.getUser().getFullName() : "")
                  .asString() + "</li>");
        } else if (go.getObjectType().equalsIgnoreCase("User") || go.getObjectType().equalsIgnoreCase("RichUser")) {
          User u = go.cast();
          items = items.concat(
              "<li>" + SafeHtmlUtils.fromString((u.getFullName() != null) ? u.getFullName() : "").asString() + "</li>");
        } else if (go.getObjectType().equalsIgnoreCase("RichDestination")) {
          Destination d = go.cast();
          items = items.concat("<li>" +
              SafeHtmlUtils.fromString(d.getDestination() + " / " + d.getType() + " / " + d.getService().getName())
                  .asString() + "</li>");
        } else if (go.getObjectType().equalsIgnoreCase("Host")) {
          Host h = go.cast();
          items = items.concat(
              "<li>" + SafeHtmlUtils.fromString((h.getName() != null) ? h.getName() : "").asString() + "</li>");
        } else if (go.getObjectType().equalsIgnoreCase("Facility") ||
            go.getObjectType().equalsIgnoreCase("RichFacility")) {
          Facility f = go.cast();
          items = items.concat(
              "<li>" + SafeHtmlUtils.fromString((f.getName() != null) ? f.getName() : "").asString() + "</li>");
        } else if (go.getObjectType().equalsIgnoreCase("Attribute") ||
            go.getObjectType().equalsIgnoreCase("AttributeDefinition")) {
          Attribute a = go.cast();
          items = items.concat(
              "<li>" + SafeHtmlUtils.fromString((a.getDisplayName() != null) ? a.getDisplayName() : "").asString() +
                  "</li>");
        } else if (go.getObjectType().equalsIgnoreCase("UserExtSource")) {
          UserExtSource ues = go.cast();
          items = items.concat(
              "<li>" + SafeHtmlUtils.fromString(ues.getLogin() + " / " + ues.getExtSource().getName()).asString() +
                  "</li>");
        } else if (go.getObjectType().equalsIgnoreCase("ApplicationMail")) {
          //items = items.concat("<li>"+go.getName()+"</li>");
          ApplicationMail mail = go.cast();
          items = items.concat(
              "<li>" + SafeHtmlUtils.fromString(ApplicationMail.getTranslatedMailType(mail.getMailType())).asString() +
                  "</li>");
        } else if (go.getObjectType().equalsIgnoreCase("PublicationForGUI") ||
            go.getObjectType().equalsIgnoreCase("Publication")) {
          Publication pub = go.cast();
          items = items.concat(
              "<li>" + SafeHtmlUtils.fromString((pub.getTitle() != null) ? pub.getTitle() : "").asString() + "</li>");
        } else if (go.getObjectType().equalsIgnoreCase("ThanksForGUI")) {
          Thanks th = go.cast();
          items = items.concat(
              "<li>" + SafeHtmlUtils.fromString((th.getOwnerName() != null) ? th.getOwnerName() : "").asString() +
                  "</li>");
        } else if (go.getObjectType().equalsIgnoreCase("Author")) {
          Author aut = go.cast();
          items = items.concat(
              "<li>" + SafeHtmlUtils.fromString((aut.getDisplayName() != null) ? aut.getDisplayName() : "").asString() +
                  "</li>");
        } else if (go.getObjectType().equalsIgnoreCase("ResourceTag")) {
          ResourceTag tag = go.cast();
          items = items.concat(
              "<li>" + SafeHtmlUtils.fromString((tag.getName() != null) ? tag.getName() : "").asString() + "</li>");
        } else if (go.getObjectType().equalsIgnoreCase("TaskResult")) {
          TaskResult result = go.cast();
          items = items.concat("<li>" + SafeHtmlUtils.fromString(
              (result.getDestination() != null) ? (result.getDestination().getDestination() + " / " + result.getId()) :
                  "").asString() + "</li>");
        } else {
          items = items.concat(
              "<li>" + SafeHtmlUtils.fromString((go.getName() != null) ? go.getName() : "").asString() + "</li>");
        }
  ...
  public static cz.metacentrum.perun.webgui.widgets.CustomButton getRefreshButton(final TabItem tabToRefres) {
    cz.metacentrum.perun.webgui.widgets.CustomButton cb =
        new cz.metacentrum.perun.webgui.widgets.CustomButton("", SmallIcons.INSTANCE.updateIcon(), new ClickHandler() {
          @Override
  ...
  public Widget getHeader() {
    ...
    // image
    Image img = new Image(Utils.logoUrl());
    layout.add(img);
  ...
  public Widget getFooter() {
    ...
    if (link != null && !link.isEmpty()) {
      ...
      a3.setTarget("_blank");
      foot = new HTML(
          "<strong>About: </strong>" + a + "<strong>&nbsp;|&nbsp;License: </strong>" + a2 + "&nbsp;|&nbsp;" + a3 +
              "&nbsp;|&nbsp;<strong>Support: </strong>" + mail + ", " + lnk);
    } else {
      foot = new HTML("<strong>About: </strong>" + a + "&nbsp;|&nbsp;<strong>License: </strong>" + a2 +
          "&nbsp;|&nbsp;<strong>Support: </strong>" + mail + ", " + lnk);
    }
    ...
    ft.setWidget(0, 1, new HTML(
        PerunWebConstants.INSTANCE.footerPerunCopyright() + " " + JsonUtils.getCurrentYear() + ", version: " +
            PerunWebConstants.INSTANCE.guiVersion() + SafeHtmlUtils.htmlEscape(rpcVersionString)));
  ...
  public ToggleButton getToggleLanguageButton() {
    ...
    languageButton.addClickHandler(new ClickHandler() {
      public void onClick(ClickEvent event) {
        ...
        Confirm conf =
            new Confirm(languageButton.getTitle(), new HTML(WidgetTranslation.INSTANCE.changeLanguageConfirmText()),
                new ClickHandler() {
                  public void onClick(ClickEvent event) {
                    // on OK
                    // set proper locale
                    ...
                    if (!localeName.equals(Utils.getNativeLanguage().get(0))) {
                      ...
                      languageButton.setTitle(
                          WidgetTranslation.INSTANCE.changeLanguageToCzech(Utils.getNativeLanguage().get(1)));
                    }
   ...
   * @param resizeWidth
   */
  private void generateResizingCommand(final Widget panel, final int minHeight, final int correction,
                                       final TabItem tabItem, final boolean resizeWidth) {
    ...
    Command c = new Command() {
      public void execute() {
        ...
        int clientHeight =
            (Window.getClientHeight() > WebGui.MIN_CLIENT_HEIGHT) ? Window.getClientHeight() : WebGui.MIN_CLIENT_HEIGHT;
