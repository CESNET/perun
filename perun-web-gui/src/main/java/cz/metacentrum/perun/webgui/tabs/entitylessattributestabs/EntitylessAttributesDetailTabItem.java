...
public class EntitylessAttributesDetailTabItem implements TabItem {
  ...
  public Widget draw() {
    ...
    if (Arrays.asList("core", "virt").contains(def.getDefinition()) || def.getEntity().equals("entityless")) {
      ...
      unique.addValueChangeHandler(valueChangeEvent -> {
        ...
        if (valueChangeEvent.getValue()) {
          ...
          UiElements.generateInfo("Change not allowed",
              "Once converted to UNIQUE, attributes can't be converted back to non-unique.");
          unique.setValue(true);
        }
    ...
    updateButton.addClickHandler(event -> {
      if ((!Objects.equals(def.getDescription(), description.getTextBox().getText().trim()) ||
          !Objects.equals(def.getDisplayName(), displayName.getTextBox().getText().trim()))) {
