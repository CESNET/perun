...
public class AllPublicationsTabItem implements TabItem, TabItemWithUrl {
  ...
  public static final String URL = "publications";
  /**
   * Perun web session
   */
  ...
  public Widget draw() {
    ...
    lock.addClickHandler(new ClickHandler() {
      public void onClick(ClickEvent event) {
        ...
        if (list != null && !list.isEmpty()) {
          LockUnlockPublications request = new LockUnlockPublications(
              JsonCallbackEvents.disableButtonEvents(lock, JsonCallbackEvents.refreshTableEvents(callback)));
    ...
    // switch lock state button
    final CustomButton unlock =
        new CustomButton("Unlock", "Unlock selected publications", SmallIcons.INSTANCE.lockOpenIcon());
    unlock.addClickHandler(new ClickHandler() {
      public void onClick(ClickEvent event) {
        ArrayList<Publication> list = callback.getTableSelectedList();
        if (list != null && !list.isEmpty()) {
          LockUnlockPublications request = new LockUnlockPublications(
              JsonCallbackEvents.disableButtonEvents(unlock, JsonCallbackEvents.refreshTableEvents(callback)));
    ...
    CustomButton applyFilter =
        new CustomButton("Apply", "Show publications based on filter", SmallIcons.INSTANCE.filterAddIcon());
    CustomButton clearFilter =
        new CustomButton("Clear", "Show all publications", SmallIcons.INSTANCE.filterDeleteIcon());
