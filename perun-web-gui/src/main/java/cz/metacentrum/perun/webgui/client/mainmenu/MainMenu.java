...
public class MainMenu {
  ...
  public void prepare() {
    ...
    if (session.isPerunAdmin()) {
      MainMenuSection perunAdmin =
          new MainMenuSection("Perun admin", new VosTabItem(), iconsLarge.perunIcon(), PERUN_ADMIN);
    ...
    if (session.isVoAdmin() || session.isVoObserver() || session.isPerunAdmin()) {
      MainMenuSection voAdmin =
          new MainMenuSection("VO manager", new VosSelectTabItem(), iconsLarge.buildingIcon(), VO_ADMIN);
    ...
    if (session.isGroupAdmin() || session.isVoAdmin() || session.isPerunAdmin()) {
      MainMenuSection groupAdmin =
          new MainMenuSection("Group manager", new GroupsTabItem(null), iconsLarge.groupIcon(), GROUP_ADMIN);
    ...
    if (session.isFacilityAdmin() || session.isPerunAdmin()) {
      MainMenuSection facilityAdmin =
          new MainMenuSection("Facility manager", new FacilitiesSelectTabItem(), iconsLarge.databaseServerIcon(),
              FACILITY_ADMIN);
  ...
  private void buildGroupAdminMenu() {
    ...
    menu.addItem(
        new MainMenuItem("Select group", new GroupsTabItem(session.getActiveVo()), SmallIcons.INSTANCE.groupIcon()));
    menu.addSplitter();
  ...
  private void buildFacilityAdminMenu() {
    ...
    menu.addItem(
        new MainMenuItem("Select facility", new FacilitiesSelectTabItem(), SmallIcons.INSTANCE.databaseServerIcon()));
    menu.addSplitter();
  ...
  private void buildUserMenu() {
    ...
    menu.addItem(new MainMenuItem((user != null) ? user.getFullNameWithTitles() : "My profile", detail,
        SmallIcons.INSTANCE.userGrayIcon()));
