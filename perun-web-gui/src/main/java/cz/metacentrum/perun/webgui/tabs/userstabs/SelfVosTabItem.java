...
public class SelfVosTabItem implements TabItem, TabItemWithUrl {
  ...
  public Widget draw() {
    ...
    this.titleWidget.setText(
        Utils.getStrippedStringWithEllipsis(user.getFullNameWithTitles().trim()) + ": VO settings");
  ...
  private Widget displayVoSubtab(final VirtualOrganization vo) {
    ...
    voHeader.setHTML(0, 1, "<p class=\"subsection-heading\">" +
        SafeHtmlUtils.fromString((vo.getName() != null) ? vo.getName() : "").asString() + "</p>");
    ...
    a.addClickHandler(new ClickHandler() {
      ...
      public void onClick(ClickEvent event) {
        if (tabPanel != null) {
          ...
          SelfResourcesSettingsTabItem tab =
              ((SelfResourcesSettingsTabItem) tabPanel.getTabItem(tabPanel.getSelectedIndex() + 1));
          tab.setVo(vo);
          tab.draw();
    ...
    resourcesSettingsTable.setHTML(0, 0,
        "Manage VO resources specific settings like: shell, data/files quotas, mailing list exclusions");
  ...
  public void open() {
    ...
    session.getUiElements().getBreadcrumbs()
        .setLocation(MainMenu.USER, Utils.getStrippedStringWithEllipsis(user.getFullNameWithTitles().trim()),
            UsersTabs.URL + UrlMapper.TAB_NAME_SEPARATOR + getUrl() + "?id=" + userId, "VO settings",
