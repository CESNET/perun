...
public class JsonClient {
  ...
  public void retrieveData(String url, String params, final JsonCallback callback) {
    ...
    // url to call
    final String requestUrl = URL.encode(
        PerunWebSession.getInstance().getRpcUrl() + url + "?callback=" + perunRequest.getStartTime() + "&" + params);
            ...
            if (resp.getStatusCode() == 401 || resp.getStatusCode() == 403) {
              ...
              if (runningRequests.get(requestUrl) != null) {
                ...
                if ((runningRequests.get(requestUrl).getDuration() / (1000 * 60)) >= 5) {
                  ...
                  error.setErrorInfo(
                      "Your request couldn't be handled within 5 minutes. Please refresh your view (table) to try it again.");
              ...
              error.setErrorInfo(
                  "You are either without network connection or your session has expired. Click \"OK\" to reload the page (all unsaved changes will be lost).");
