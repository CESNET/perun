...
public class GetFormItems implements JsonCallback {
  ...
  public void prepareSettings(final ArrayList<ApplicationFormItem> items) {
    ...
    for (final ApplicationFormItem item : items) {
      ...
      if (item.isForDelete()) {
        ...
        // undelete button
        CustomButton undelete = new CustomButton(ButtonTranslation.INSTANCE.undeleteFormItemButton(),
            ButtonTranslation.INSTANCE.undeleteFormItem(), SmallIcons.INSTANCE.arrowLeftIcon(), new ClickHandler() {
      ...
      CustomButton editButton =
          new CustomButton(ButtonTranslation.INSTANCE.editFormItemButton(), ButtonTranslation.INSTANCE.editFormItem(),
              SmallIcons.INSTANCE.applicationFormEditIcon());
      editButton.addClickHandler(new ClickHandler() {
        public void onClick(ClickEvent event) {
          if (group == null) {
            session.getTabManager()
                .addTabToCurrentTab(new EditFormItemTabItem(id, 0, item, applFormItems, refreshEvents));
          } else {
            session.getTabManager().addTabToCurrentTab(
                new EditFormItemTabItem(group.getVoId(), group.getId(), item, applFormItems, refreshEvents));
          }
        }
      });
      ...
      CustomButton removeButton =
          new CustomButton(ButtonTranslation.INSTANCE.deleteButton(), ButtonTranslation.INSTANCE.deleteFormItem(),
      ...
      CustomButton configureGroupsButton = new CustomButton(ButtonTranslation.INSTANCE.addGroupToAutoReg(),
          ButtonTranslation.INSTANCE.addGroupToAutoReg(), SmallIcons.INSTANCE.groupIcon());
  ...
  private VerticalPanel createVPForIcon(Image icon, String whenDisabledOrHidden, String disabledOrHidden,
                                        String dependency) {
  ...
  private void prepareErrorSettings(PerunError error) {
    ...
    if (error != null && error.getName().equalsIgnoreCase("FormNotExistsException")) {
      ...
      final CustomButton create =
          new CustomButton("Create empty form", ButtonTranslation.INSTANCE.createEmptyApplicationForm(),
              SmallIcons.INSTANCE.addIcon());
      create.addClickHandler(new ClickHandler() {
        public void onClick(ClickEvent event) {
          // disable button event with refresh page on finished
          CreateApplicationForm request = new CreateApplicationForm(entity, id,
              JsonCallbackEvents.disableButtonEvents(create, new JsonCallbackEvents() {
  ...
  public void prepareApplicationForm(final ArrayList<ApplicationFormItem> items) {
    ...
    for (final ApplicationFormItem item : items) {
      ...
      String value = "";
      if (item.getShortname().equals("affiliation") || item.getShortname().equals("mail") ||
  ...
  public ArrayList<ApplicationFormItemData> getValues() {
    ...
    for (RegistrarFormItemGenerator gen : applFormGenerators) {
      ...
      ApplicationFormItemData data =
          ApplicationFormItemData.construct(formItem, formItem.getShortname(), value, prefilled, "");
