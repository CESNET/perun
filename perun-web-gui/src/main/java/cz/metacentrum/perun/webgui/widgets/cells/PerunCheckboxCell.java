...
public class PerunCheckboxCell<T extends JavaScriptObject> extends AbstractEditableCell<T, Boolean> {
  ...
  public PerunCheckboxCell(boolean dependsOnSelection, boolean handlesSelection, boolean editable,
                           boolean blockManualAndSyncGroups) {
    super("change", "keydown");
    this.dependsOnSelection = dependsOnSelection;
    this.handlesSelection = handlesSelection;
    this.editable = editable;
  ...
  public void render(Context context, T value, SafeHtmlBuilder sb) {
    ...
    if (((GeneralObject) value).getObjectType().equalsIgnoreCase("Group") ||
        ((GeneralObject) value).getObjectType().equalsIgnoreCase("RichGroup")) {
    ...
    if (((GeneralObject) value).getObjectType().equalsIgnoreCase("RichMember")) {
      if (((RichMember) value).getMembershipType().equalsIgnoreCase("INDIRECT") && !editable) {
        sb.append(getDisabledCheckbox(
            "User is an INDIRECT member and can't be removed from the group directly. Remove the user from the source group or connection between the groups."));
        return;
      }
    }
