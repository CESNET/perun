...
public class GroupRelationsTabItem implements TabItem, TabItemWithUrl {
  ...
  public Widget draw() {
    ...
    CustomButton createButton =
        TabMenu.getPredefinedButton(ButtonType.ADD, true, ButtonTranslation.INSTANCE.addGroupUnion(),
            new ClickHandler() {
    ...
    final CustomButton removeButton =
        TabMenu.getPredefinedButton(ButtonType.REMOVE, ButtonTranslation.INSTANCE.removeGroupUnion());
      ...
      public void onClick(ClickEvent event) {
        ...
        UiElements.showDeleteConfirm(itemsToRemove, text, new ClickHandler() {
          ...
          public void onClick(ClickEvent clickEvent) {
            RemoveGroupUnions request =
                new RemoveGroupUnions(JsonCallbackEvents.disableButtonEvents(removeButton, events));
            if (unions.isReverse()) {
  ...
  public boolean isAuthorized() {
    return session.isVoAdmin(group.getVoId()) || session.isVoObserver(group.getVoId()) ||
