...
public class GroupSettingsTabItem implements TabItem, TabItemWithUrl {
  ...
  public Widget draw() {
    ...
    final GetResourceRequiredAttributesV2 reqAttrs =
        new GetResourceRequiredAttributesV2(null, JsonCallbackEvents.passDataToAnotherCallback(jsonCallback));
    ...
    final ClickHandler saveAttrsClickHandler = new ClickHandler() {
      public void onClick(ClickEvent event) {
        ...
        if (UiElements.cantSaveEmptyListDialogBox(list)) {
          ...
          final JsonCallbackEvents events = JsonCallbackEvents.disableButtonEvents(saveChangesButton,
              JsonCallbackEvents.refreshTableEvents(jsonCallback));

          if (memChb.getValue() == true) {
            // to all members selected
            UiElements.generateInfo(WidgetTranslation.INSTANCE.saveConfirmTitle(),
      ...
      public void onClick(ClickEvent event) {
        ...
        if (UiElements.cantSaveEmptyListDialogBox(list)) {
          ...
          final JsonCallbackEvents events =
              JsonCallbackEvents.disableButtonEvents(saveChangesButton, new JsonCallbackEvents() {
                public void onFinished(JavaScriptObject jso) {
          ...
          if (memChb.getValue() == true) {
            // to all members selected
            UiElements.generateInfo(WidgetTranslation.INSTANCE.saveConfirmTitle(),
                "Same value(s) for selected attribute(s) will be set to ALL members of group.", new ClickHandler() {
                  public void onClick(ClickEvent event) {
                    ArrayList<RichMember> memList = membersDropDown.getAllObjects();
                    for (int i = 0; i < memList.size(); i++) {
                      if (i == 0) {
    ...
    final ClickHandler removeAttrsClickHandler = new ClickHandler() {
      public void onClick(ClickEvent event) {
        ...
        if (UiElements.cantSaveEmptyListDialogBox(list)) {
          ...
          // refresh table and disable button a event for GetAttributes
          final JsonCallbackEvents events =
              JsonCallbackEvents.disableButtonEvents(removeButton, JsonCallbackEvents.refreshTableEvents(jsonCallback));

          final RemoveAttributes request = new RemoveAttributes();

          if (memChb.getValue() == true) {
            // to all members selected
            UiElements.generateInfo(WidgetTranslation.INSTANCE.deleteConfirmTitle(),
                "Selected attribute(s) will be removed from ALL members of group.", new ClickHandler() {
                  public void onClick(ClickEvent event) {
                    ArrayList<RichMember> memList = membersDropDown.getAllObjects();
                    for (int i = 0; i < memList.size(); i++) {
                      if (i == 0) {
    ...
    final ClickHandler removeReqAttrsClickHandler = new ClickHandler() {
      public void onClick(ClickEvent event) {
        ...
        if (UiElements.cantSaveEmptyListDialogBox(list)) {
          ...
          // refresh table and disable button a event for GetAttributes
          final JsonCallbackEvents events =
              JsonCallbackEvents.disableButtonEvents(removeButton, new JsonCallbackEvents() {
                public void onFinished(JavaScriptObject jso) {
                  jsonCallback.clearTable();
                  // set back resourceToGetServicesFrom
                  Map<String, Integer> ids = reqAttrs.getIds();
