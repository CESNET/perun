...
public class GetCompleteCandidates implements JsonCallback, JsonCallbackTable<MemberCandidate> {
  ...
  // loader image
  private AjaxLoaderImage loaderImage =
      new AjaxLoaderImage(true, WidgetTranslation.INSTANCE.emptySearchForCandidates());
  // Custom events
  ...
  public static String getExtSourceNameOrOrganizationColumn(MemberCandidate candidate) {
    ...
    if (candidate.getCandidate().getUserExtSource().getExtSource().getType()
        .equalsIgnoreCase("cz.metacentrum.perun.core.impl.ExtSourceX509")) {
      return Utils.convertCertCN(candidate.getCandidate().getUserExtSource().getExtSource().getName());
    } else if (candidate.getCandidate().getUserExtSource().getExtSource().getType()
      ...
      public String getValue(MemberCandidate candidate) {
        ...
        if (groupId == 0) {
          ...
          if (candidate.getMember() != null &&
              ...
              "DIRECT".equalsIgnoreCase(candidate.getMember().getMembershipType())) {
            return "Member of Group";
          }
          if (candidate.getMember() != null &&
              candidate.getMember().getSourceGroupId() != 0 &&
              "INDIRECT".equalsIgnoreCase(candidate.getMember().getMembershipType())) {
