...
public class UserDetailTabItem implements TabItem, TabItemWithUrl {
  ...
  private Widget loadInformationOverview() {
    ...
    layout.setHTML(0, 1,
        SafeHtmlUtils.fromString((user.getFullNameWithTitles() != null) ? user.getFullNameWithTitles() : "")
            .asString());
    ...
    final CustomButton saveAttrButton =
        TabMenu.getPredefinedButton(ButtonType.SAVE, "Save changes in attributes for user");
    saveAttrButton.addClickHandler(new ClickHandler() {
      public void onClick(ClickEvent event) {
        ...
        SetAttributes request = new SetAttributes(
            JsonCallbackEvents.disableButtonEvents(saveAttrButton, JsonCallbackEvents.refreshTableEvents(attributes)));
    ...
    removeAttrButton.addClickHandler(new ClickHandler() {
      public void onClick(ClickEvent event) {
        ...
        RemoveAttributes request = new RemoveAttributes(JsonCallbackEvents.disableButtonEvents(removeAttrButton,
            JsonCallbackEvents.refreshTableEvents(attributes)));
    ...
    CellTable<VirtualOrganization> simpeVosTable =
        vos.getTable(new FieldUpdater<VirtualOrganization, VirtualOrganization>() {
  ...
  private Widget loadExternalIdentities() {
    ...
    // buttons
    CustomButton addUserExtSourceButton =
        TabMenu.getPredefinedButton(ButtonType.ADD, true, "Add new external identity to user", new ClickHandler() {
          public void onClick(ClickEvent event) {
    ...
    final CustomButton removeUserExtSourceButton =
        TabMenu.getPredefinedButton(ButtonType.REMOVE, "Removes selected external identities from this user");
    removeUserExtSourceButton.addClickHandler(new ClickHandler() {
      public void onClick(ClickEvent event) {
        ...
        UiElements.showDeleteConfirm(list, "Following external identities will be removed from user.",
            new ClickHandler() {
              @Override
              public void onClick(ClickEvent event) {
                for (int i = 0; i < list.size(); i++) {
                  ...
                  if (i == list.size() - 1) {
                    RemoveUserExtSource request = new RemoveUserExtSource(
                        JsonCallbackEvents.disableButtonEvents(removeUserExtSourceButton, reloadTabEvents));
                    request.removeUserExtSource(user.getId(), list.get(i).getId());
                  } else {
                    RemoveUserExtSource request =
                        new RemoveUserExtSource(JsonCallbackEvents.disableButtonEvents(removeUserExtSourceButton));
                    request.removeUserExtSource(user.getId(), list.get(i).getId());
                  }
                }
              }
  ...
  private Widget loadResources() {
    ...
    cz.metacentrum.perun.webgui.json.usersManager.GetAssignedRichResources resources =
        new cz.metacentrum.perun.webgui.json.usersManager.GetAssignedRichResources(user.getId(), PerunEntity.USER);
  ...
  private void loadMemberSubContent(final SimplePanel subContent, final Hyperlink voLabel,
                                    final ListBoxWithObjects<VirtualOrganization> listbox) {
    ...
    JsonCallbackEvents loadEvent = new JsonCallbackEvents() {
      ...
      public void onFinished(JavaScriptObject jso) {
        ...
        voLabel.setHTML(SafeHtmlUtils.fromSafeConstant("<h2>" + SafeHtmlUtils.fromString(
            (listbox.getSelectedObject().getName() != null) ? listbox.getSelectedObject().getName() : "").asString() +
            "</h2>"));
        voLabel.setTargetHistoryToken(
            session.getTabManager().getLinkForTab(new VoDetailTabItem(listbox.getSelectedObject())));
        ...
        GetAssignedResources res =
            new GetAssignedResources(member.getId(), PerunEntity.MEMBER, new JsonCallbackEvents() {
              @Override
              public void onFinished(JavaScriptObject jso) {
                resList.clear();
        ...
        final CustomButton saveAttrButton =
            TabMenu.getPredefinedButton(ButtonType.SAVE, "Save changes in attributes for member");
        saveAttrButton.addClickHandler(new ClickHandler() {
          public void onClick(ClickEvent event) {
            ...
            if (UiElements.cantSaveEmptyListDialogBox(list)) {
              ...
              SetAttributes request = new SetAttributes(JsonCallbackEvents.disableButtonEvents(saveAttrButton,
                  JsonCallbackEvents.refreshTableEvents(attributes)));
              request.setAttributes(ids, list);
        ...
        menu.addWidget(
            TabMenu.getPredefinedButton(ButtonType.ADD, true, "Set new attributes for member", new ClickHandler() {
              public void onClick(ClickEvent event) {
        ...
        final CustomButton removeAttrButton =
            TabMenu.getPredefinedButton(ButtonType.REMOVE, "Remove attributes from member");
        removeAttrButton.addClickHandler(new ClickHandler() {
          public void onClick(ClickEvent event) {
            ArrayList<Attribute> list = attributes.getTableSelectedList();
            if (UiElements.cantSaveEmptyListDialogBox(list)) {
        ...
        for (final Attribute a : userLoginAttrs) {
          if (a.getBaseFriendlyName().equalsIgnoreCase("login-namespace")) {
            if (a.getValueAsObject() != null) {
              // name
              innerTable.setHTML(rowCount, 0,
                  "<strong>" + SafeHtmlUtils.fromString(a.getDisplayName()).asString() + "</strong>");
              // value
              ...
              if (Utils.getSupportedPasswordNamespaces().contains(a.getFriendlyNameParameter())) {
                ...
                CustomButton cb =
                    new CustomButton("Change password…", SmallIcons.INSTANCE.keyIcon(), new ClickHandler() {
                      public void onClick(ClickEvent event) {
                        session.getTabManager().addTabToCurrentTab(
                            new SelfPasswordTabItem(user, a.getFriendlyNameParameter(), a.getValue(),
                                SelfPasswordTabItem.Actions.CHANGE));
                      }
                    });
                ...
                CustomButton cb2 =
                    new CustomButton("Reset to randomly generated password", SmallIcons.INSTANCE.keyIcon(),
                        new ClickHandler() {
                          public void onClick(ClickEvent event) {

                            String response = Utils.getBinaryResource(
                                PerunWebSession.getInstance().getRpcUrl() + "usersManager/changePasswordRandom",
                                "{ \"userId\": " + userId + " , \"loginNamespace\": \"" + a.getFriendlyNameParameter() +
                                    "\" }");

                            counter++;
                            Utils.convertBinaryToDownloadableFile(counter,
                                "PasswordReset" + Random.nextInt(10000) + ".pdf", "application/pdf", response);
  ...
  private void loadFacilitySubContent(final SimplePanel subContent, final Hyperlink facilityLabel,
                                      final ListBoxWithObjects<Facility> listbox) {

    final VerticalPanel entryPanel = new VerticalPanel();
    entryPanel.setSize("100%", "100%");
    subContent.setWidget(entryPanel);

    facilityLabel.setHTML(SafeHtmlUtils.fromSafeConstant("<h2>" + SafeHtmlUtils.fromString(
        (listbox.getSelectedObject().getName() != null) ? listbox.getSelectedObject().getName() : "").asString() +
        "</h2>"));
    facilityLabel.setTargetHistoryToken(
        session.getTabManager().getLinkForTab(new FacilityDetailTabItem(listbox.getSelectedObject())));
  ...
  public VerticalPanel loadServiceIdentities() {
    ...
    if (user.isServiceUser()) {
      ...
      // buttons
      menu.addWidget(
          TabMenu.getPredefinedButton(ButtonType.ADD, true, "Add new user to service identity: " + user.getLastName(),
              new ClickHandler() {
      ...
      final CustomButton removeUserButton =
          TabMenu.getPredefinedButton(ButtonType.REMOVE, "Remove user from service identity " + user.getLastName());
      ...
      removeUserButton.addClickHandler(new ClickHandler() {
        public void onClick(ClickEvent clickEvent) {
          ...
          if (fullList.size() == list.size()) {
            ...
            UiElements.generateAlert("Remove warning",
                "<strong><span class=\"serverResponseLabelError\">If you remove all users from service identity you won't be able to use it in the future.</br></br>Please consider keeping at least one user, e.g. add someone else before you remove yourself.</span></strong><p><strong>Do you wish to continue anyway ?</strong>",
                new ClickHandler() {
                  @Override
                  public void onClick(ClickEvent event) {

                    UiElements.showDeleteConfirm(list,
                        "Following users will be removed from service identity and they will lose all access to it. Only users associated with service identity can add other users again. If you remove all users connected to the service identity, you won't be able to use it in future!",
                        new ClickHandler() {
                          @Override
                          public void onClick(ClickEvent event) {
                            for (int i = 0; i < list.size(); i++) {
                              ...
                              if (i == list.size() - 1) {
                                req = new RemoveSpecificUserOwner(
                                    JsonCallbackEvents.disableButtonEvents(removeUserButton,
                                        JsonCallbackEvents.refreshTableEvents(request)));
                              } else {
                                req = new RemoveSpecificUserOwner(
                                    JsonCallbackEvents.disableButtonEvents(removeUserButton));
                              }
            ...
            UiElements.showDeleteConfirm(list,
                "Following users will be removed from service identity and they will lose any access to it. Only users associated with service identity can add other users again. If you remove all users connected to the service identity, it will be deleted too!",
                new ClickHandler() {
                  @Override
                  public void onClick(ClickEvent event) {
                    for (int i = 0; i < list.size(); i++) {
      ...
      // buttons
      menu.addWidget(
          TabMenu.getPredefinedButton(ButtonType.ADD, true, "Add new service identity to " + user.getFullName(),
              new ClickHandler() {
                public void onClick(ClickEvent clickEvent) {
                  session.getTabManager().addTabToCurrentTab(new ConnectServiceIdentityTabItem(user), true);
                }
      ...
      final CustomButton removeUserButton =
          TabMenu.getPredefinedButton(ButtonType.REMOVE, "Remove service identity from " + user.getFullName());
      menu.addWidget(removeUserButton);
      removeUserButton.addClickHandler(new ClickHandler() {
        @Override
        public void onClick(ClickEvent clickEvent) {
          final ArrayList<User> list = request.getTableSelectedList();
          UiElements.showDeleteConfirm(list, "Following service identities will be removed from user.",
              new ClickHandler() {
                @Override
                public void onClick(ClickEvent event) {
                  // TODO - SHOULD HAVE ONLY ONE CALLBACK TO CORE
  ...
  public VerticalPanel loadSponsoredUsers() {
    ...
    if (user.isSponsoredUser()) {
      ...
      // buttons
      menu.addWidget(TabMenu.getPredefinedButton(ButtonType.ADD, true, "Add new sponsor to " + user.getLastName(),
          new ClickHandler() {
            public void onClick(ClickEvent clickEvent) {
              session.getTabManager().addTabToCurrentTab(new ConnectServiceIdentityTabItem(user), true);
            }
      ...
      final CustomButton removeUserButton =
          TabMenu.getPredefinedButton(ButtonType.REMOVE, "Remove sponsors from " + user.getLastName());
      menu.addWidget(removeUserButton);
      removeUserButton.addClickHandler(new ClickHandler() {
        public void onClick(ClickEvent clickEvent) {
          ...
          if (fullList.size() == list.size()) {
            ...
            UiElements.generateAlert("Remove warning",
                "<strong><span class=\"serverResponseLabelError\">If you remove all users from sponsored user you won't be able to use it in the future.</br></br>Please consider keeping at least one user, e.g. add someone else before you remove yourself.</span></strong><p><strong>Do you wish to continue anyway ?</strong>",
                new ClickHandler() {
                  @Override
                  public void onClick(ClickEvent event) {

                    UiElements.showDeleteConfirm(list,
                        "Following users will be removed from sponsored user and they will lose all access to it. Only users associated with sponsored user can add other users again. If you remove all users connected to the sponsored user, you won't be able to use it in future!",
                        new ClickHandler() {
                          @Override
                          public void onClick(ClickEvent event) {
                            for (int i = 0; i < list.size(); i++) {
            ...
            // if not selected myself, continue same way
            UiElements.showDeleteConfirm(list,
                "Following users will be removed from sponsored user and they will lose any access to it. Only users associated with sponsored user can add other users again. If you remove all users connected to the sponsored user, it will be deleted too!",
                new ClickHandler() {
                  @Override
                  public void onClick(ClickEvent event) {
                    for (int i = 0; i < list.size(); i++) {
      ...
      // buttons
      menu.addWidget(
          TabMenu.getPredefinedButton(ButtonType.ADD, true, "Add new sponsored user to " + user.getFullName(),
              new ClickHandler() {
                public void onClick(ClickEvent clickEvent) {
                  session.getTabManager().addTabToCurrentTab(new ConnectServiceIdentityTabItem(user), true);
                }
      ...
      final CustomButton removeUserButton =
          TabMenu.getPredefinedButton(ButtonType.REMOVE, "Remove sponsored user from " + user.getFullName());
      menu.addWidget(removeUserButton);
      removeUserButton.addClickHandler(new ClickHandler() {
        @Override
        public void onClick(ClickEvent clickEvent) {
          final ArrayList<User> list = request.getTableSelectedList();
          UiElements.showDeleteConfirm(list, "Following sponsored users will be removed from user.",
              new ClickHandler() {
                @Override
                public void onClick(ClickEvent event) {
                  // TODO - SHOULD HAVE ONLY ONE CALLBACK TO CORE
  ...
  public void open() {
    ...
    session.getUiElements().getBreadcrumbs()
        .setLocation(MainMenu.PERUN_ADMIN, "Users", PerunAdminTabs.URL + UrlMapper.TAB_NAME_SEPARATOR + "users",
            user.getFullNameWithTitles(), getUrlWithParameters());
  }
