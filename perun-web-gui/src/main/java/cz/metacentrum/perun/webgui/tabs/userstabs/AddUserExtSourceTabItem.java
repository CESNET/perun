  ...
  public Widget draw() {
    ...
    JsonCallbackEvents fillEvent = new JsonCallbackEvents() {
      ...
      public void onFinished(JavaScriptObject jso) {
        ...
        callDone = true;
        if (!externalLogin.getText().isEmpty() && !extSourcesDropDown.isEmpty() &&
            JsonUtils.checkParseInt(loaTextBox.getText()) && callDone) {
    ...
    layout.setHTML(3, 1,
        "0 - not verified = default</br>1 - verified email</br>2 - verified identity</br>3 - verified identity, strict password strength");
