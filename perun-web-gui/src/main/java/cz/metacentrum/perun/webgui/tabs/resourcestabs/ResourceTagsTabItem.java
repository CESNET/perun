...
public class ResourceTagsTabItem implements TabItem, TabItemWithUrl {
  ...
  public Widget draw() {
    ...
    CustomButton assignTagsButton =
        TabMenu.getPredefinedButton(ButtonType.ADD, true, ButtonTranslation.INSTANCE.assignTagsToResource(),
            new ClickHandler() {
    ...
    final CustomButton removeTagsButton =
        TabMenu.getPredefinedButton(ButtonType.REMOVE, ButtonTranslation.INSTANCE.removeSelectedTagsFromResource());
          ...
          public void onClick(ClickEvent clickEvent) {
            ...
            for (int i = 0; i < tagsToRemove.size(); i++) {
              if (i == tagsToRemove.size() - 1) {
                RemoveResourceTag request = new RemoveResourceTag(resource.getId(),
                    JsonCallbackEvents.disableButtonEvents(removeTagsButton, localEvents));
                request.removeResourceTag(tagsToRemove.get(i));
              } else {
                RemoveResourceTag request =
                    new RemoveResourceTag(resource.getId(), JsonCallbackEvents.disableButtonEvents(removeTagsButton));
                request.removeResourceTag(tagsToRemove.get(i));
              }
            }
          }
