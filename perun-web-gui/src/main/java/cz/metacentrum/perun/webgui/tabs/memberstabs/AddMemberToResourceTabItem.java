  ...
  public Widget draw() {
    ...
    if (state == State.SELECT_MEMBERS) {
      ...
      final FindCompleteRichMembers findMembers =
          new FindCompleteRichMembers(PerunEntity.VIRTUAL_ORGANIZATION, voId, "", null);
      ...
      final CustomButton addButton =
          TabMenu.getPredefinedButton(ButtonType.ADD, "Add selected members to list", new ClickHandler() {
            @Override
      ...
      final CustomButton continueButton =
          TabMenu.getPredefinedButton(ButtonType.CONTINUE, "Continue to resource selection", new ClickHandler() {
            @Override
            public void onClick(ClickEvent clickEvent) {
              // change state
      ...
      final JsonCallbackEvents fillResEvent = new JsonCallbackEvents() {
        ...
        public void onFinished(JavaScriptObject jso) {
          ...
          GetAssignedServices servCall =
              new GetAssignedServices(resourcesListbox.getSelectedObject().getId(), fillServEvent);
      ...
      final CustomButton addToGroupButton =
          new CustomButton("Add to Group", "Add members to selected Group", SmallIcons.INSTANCE.addIcon());
      ...
      addToGroupButton.addClickHandler(new ClickHandler() {
        ...
        public void onClick(ClickEvent clickEvent) {
          ...
          if (UiElements.cantSaveEmptyListDialogBox(assGroupCall.getTableSelectedList())) {
            ...
            GetAssignedResources getRes =
                new GetAssignedResources(grp.getId(), PerunEntity.GROUP, new JsonCallbackEvents() {
                  @Override
                  public void onFinished(JavaScriptObject jso) {
            ...
            content.setHTML(0, 0,
                "<p><strong>By adding member into this group, he/she will gain access to following resources:</strong>");
      ...
      menu.addWidget(new CustomButton("Create group…", "Create new empty group", SmallIcons.INSTANCE.addIcon(),
          new ClickHandler() {
            @Override
            public void onClick(ClickEvent clickEvent) {
              // get back and reload
        ...
        public void onClick(ClickEvent event) {
          ...
          final JsonCallbackEvents assignEvent = new JsonCallbackEvents() {
            ...
            public void onFinished(JavaScriptObject jso) {
              Group g = jso.cast();
              if (g != null) {
                AssignGroupToResources assign =
                    new AssignGroupToResources(JsonCallbackEvents.disableButtonEvents(createButton, redirectEvent));
          ...
          if (asSubGroup.getValue()) {
            if (vosGroups.getSelectedObject() != null) {
              cg.createGroupInGroup(vosGroups.getSelectedObject().getId(),
                  groupNameTextBox.getTextBox().getText().trim(), groupDescriptionTextBox.getText());
            } else {
              UiElements.generateInfo("No parent group selected",
                  "You checked create this group as sub-group, but no parent group is selected. Please select parent group.");
            }
          } else {
