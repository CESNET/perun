...
public class CopyAttributes {
	 ...
	 */
	public CopyAttributes(JsonCallbackEvents events) {
		this.events = events;
	}
	...
	public void copyFacilityAttributes(int sourceFacility, int destinationFacility)
	{
		...
		JsonCallbackEvents newEvents = new JsonCallbackEvents(){
			...
			public void onError(PerunError error) {
				session.getUiElements().setLogErrorText("Copying facility settings failed.");
				events.onError(error);
			};

			public void onFinished(JavaScriptObject jso) {
				session.getUiElements().setLogSuccessText("Facility settings copied.");
