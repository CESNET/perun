...
public class ServicePackageDetailTabItem implements TabItem, TabItemWithUrl {
  ...
  public Widget draw() {
      ...
      public void onClick(ClickEvent event) {
        ...
        if (UiElements.cantSaveEmptyListDialogBox(list)) {
          ...
          for (int i = 0; i < list.size(); i++) {
            ...
            final int ii = i;
            if (i == list.size() - 1) {
              request = new AddServiceToServicesPackage(
                  JsonCallbackEvents.disableButtonEvents(add, JsonCallbackEvents.mergeEvents(new JsonCallbackEvents() {
                    @Override
              ...
              request =
                  new AddServiceToServicesPackage(JsonCallbackEvents.disableButtonEvents(add, new JsonCallbackEvents() {
                    @Override
                    public void onFinished(JavaScriptObject jso) {
                      services.getSelectionModel().setSelected(list.get(ii), false);
                    }
    ...
    final CustomButton remove =
        new CustomButton("Remove", "Remove services from package", SmallIcons.INSTANCE.arrowLeftIcon());
    ...
    ft.getFlexCellFormatter()
        .setAlignment(2, 0, HasHorizontalAlignment.ALIGN_CENTER, HasVerticalAlignment.ALIGN_MIDDLE);
    ft.setWidth("130px");
  ...
  public void open() {
    ...
    session.getUiElements().getBreadcrumbs()
        .setLocation(MainMenu.PERUN_ADMIN, "Service package detail", getUrlWithParameters());
  }
