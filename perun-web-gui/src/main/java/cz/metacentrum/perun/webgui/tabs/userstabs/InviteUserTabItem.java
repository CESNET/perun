...
public class InviteUserTabItem implements TabItem {
  ...
  public Widget draw() {
    ...
    if (groupId == 0) {
      ft.setHTML(0, 0,
          "Person you are inviting will receive an email with link to VOs application form. You can set up an email template in: Vo manager (advanced) -> Application form -> Notifications.");
    } else {
      ft.setHTML(0, 0,
          "Person you are inviting will receive an email with link to groups application form. You can set up an email template in: Group manager (advanced) -> Application form -> Notifications.");
    }
    ...
    sendInvitationButton.addClickHandler(new ClickHandler() {
      ...
      public void onClick(ClickEvent event) {
        ...
        SendInvitation invite = new SendInvitation(vo.getId(), groupId,
            JsonCallbackEvents.closeTabDisableButtonEvents(sendInvitationButton, tab, true));
        invite.inviteUser(email.getTextBox().getText().trim(), name.getTextBox().getText().trim(),
            languages.getValue(languages.getSelectedIndex()));
