...
public class ResourceGroupSettingsTabItem implements TabItem, TabItemWithUrl {
  ...
  public static final String URL = "grpsettings";
  /**
   * Perun web session
   */
  ...
  public Widget draw() {
    ...
    final GetRequiredAttributesV2 reqAttrs =
        new GetRequiredAttributesV2(ids, JsonCallbackEvents.passDataToAnotherCallback(resReqAttrs));
    // puts first table
    ...
    final GetAssignedGroups groups = new GetAssignedGroups(resourceId, new JsonCallbackEvents() {
      ...
      public void onFinished(JavaScriptObject jso) {
        ...
        if (grp == null || grp.isEmpty()) {
          ...
          lastSelectedGroup = 0;
          ((AjaxLoaderImage) table.getEmptyTableWidget()).setEmptyResultMessage(
              "No group or service assigned. Please assign them first to see their resource setting.");
            ...
            new ClickHandler() {
              public void onClick(ClickEvent event) {
                ...
                session.getTabManager()
                    .addTabToCurrentTab(new SetNewAttributeTabItem(ids, resReqAttrs.getList()), true);
              }
            });
    if (!session.isVoAdmin(resource.getVoId()) && !session.isFacilityAdmin(resource.getFacilityId())) {
