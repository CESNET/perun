 ...
 */
public class GetAssignedRichResources
    implements JsonCallback, JsonCallbackTable<RichResource>, JsonCallbackOracle<RichResource> {

  // JSON URL
  static private final String JSON_URL = "resourcesManager/getAssignedRichResources";
  // Selection model
  final MultiSelectionModel<RichResource> selectionModel =
      new MultiSelectionModel<RichResource>(new GeneralKeyProvider<RichResource>());
  // Session
    ...
    if (filter.equalsIgnoreCase("")) {
      ...
      for (RichResource res : fullBackup) {
        ...
        for (ResourceTag r : res.getResourceTags()) {
          ...
          if (r.getName()
              .contains(filter.substring(0, (filter.length() > 6) ? filter.length() - 6 : filter.length()).trim())) {
            list.add(res);
            break;
          }
        }
