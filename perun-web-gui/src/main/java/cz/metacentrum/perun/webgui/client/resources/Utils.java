...
public class Utils {
  ...
  public static String getIdentityConsolidatorLink(boolean target) {
    ...
    if (value != null && !value.isEmpty()) {
      ...
      if (target) {
        ...
        value += "?target_url=" + Window.Location.getProtocol() + "//" + Window.Location.getHost() +
            Window.Location.getPath() + URL.encodeQueryString(URL.encodeQueryString(Window.Location.getQueryString()));
      }
      return value;
      ...
      if (target) {
        // FIXME - ENCODE QUERY STRING 2 TIMES BECAUSE OF CONSOLIDATOR APP
        url += "?target_url=" + Window.Location.getProtocol() + "//" + Window.Location.getHost() +
            Window.Location.getPath() + URL.encodeQueryString(URL.encodeQueryString(Window.Location.getQueryString()));
      }
  ...
  public static String getIdentityConsolidatorLink(String authz, boolean target) {
    ...
    if (authz == null || authz.isEmpty()) {
      return getIdentityConsolidatorLink(target);
    }
  ...
  public static ArrayList<String> getNamespacesForPreferredGroupNames() {
    ...
    ArrayList<String> result = new ArrayList<String>();
    String skip =
        PerunWebSession.getInstance().getConfiguration().getCustomProperty("namespacesForPreferredGroupNames");
    for (int i = 0; i < skip.split(",").length; i++) {
      result.add(skip.split(",")[i]);
    }
    return result;
  ...
  public static ArrayList<String> getSupportedPasswordNamespaces() {
    ...
    if (PerunWebSession.getInstance().getConfiguration() != null) {
      String value =
          PerunWebSession.getInstance().getConfiguration().getCustomProperty("getSupportedPasswordNamespaces");
      if (value != null && !value.isEmpty()) {
  ...
  public static String translateIdp(String name) {
    ...
    orgs.put("https://login.feld.cvut.cz/idp/shibboleth",
    ...
    orgs.put("https://marisa.uochb.cas.cz/simplesaml/saml2/idp/metadata.php",
   ...
   */
  public static native void convertBinaryToDownloadableFile(int count, String fileName, String contentType,
                                                            String dataInput) /*-{
		$wnd.jQuery('<a></a>')
  ...
  public static String reverseString(String string) {
    ...
    return string.charAt(string.length() - 1) + reverseString(string.substring(1, string.length() - 1)) +
        string.charAt(0);
  }
