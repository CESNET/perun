...
public class MemberSettingsTabItem implements TabItem {
  ...
  public Widget draw() {
    ...
    titleWidget.setText(
        Utils.getStrippedStringWithEllipsis(member.getUser().getFullNameWithTitles().trim()) + ": Settings");
    ...
    final GetResourceRequiredAttributesV2 resourceRequired =
        new GetResourceRequiredAttributesV2(ids, JsonCallbackEvents.passDataToAnotherCallback(callback));
    final GetRequiredAttributes required =
        new GetRequiredAttributes(ids, JsonCallbackEvents.passDataToAnotherCallback(callback));
              ...
              request =
                  new SetAttributes(JsonCallbackEvents.disableButtonEvents(saveChangesButton, new JsonCallbackEvents() {
                    public void onFinished(JavaScriptObject jso) {
                      ...
                      callback.getMemberResourceAttributes(member.getId(), listbox.getSelectedObject().getId());
                      callback.retrieveData();
                      callback.getUserFacilityAttributes(listbox.getSelectedObject().getFacilityId(),
