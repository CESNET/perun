...
public class CreateServicePackageTabItem implements TabItem {
  ...
  public Widget draw() {
    ...
    final CustomButton createButton =
        TabMenu.getPredefinedButton(ButtonType.CREATE, ButtonTranslation.INSTANCE.createServicePackage());
    createButton.addClickHandler(new ClickHandler() {
      public void onClick(ClickEvent event) {
        if (validator.validateTextBox() && validator2.validateTextBox()) {
          CreateServicePackage request =
              new CreateServicePackage(JsonCallbackEvents.closeTabDisableButtonEvents(createButton, tab, true));
          request.createServicePackage(packageName.getTextBox().getText().trim(),
              packageDescription.getTextBox().getText().trim());
        }
      }
    });
