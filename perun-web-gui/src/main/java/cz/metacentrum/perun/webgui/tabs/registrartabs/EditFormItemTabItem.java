...
public class EditFormItemTabItem implements TabItem {
   ...
   * @param events
   */
  public EditFormItemTabItem(int voId, int groupId, ApplicationFormItem item, ArrayList<ApplicationFormItem> otherItems,
  ...
  private Widget itemTextTab(String locale) {
    ...
    if ("HTML_COMMENT".equals(item.getType()) || "HEADING".equals(item.getType())) {
      ...
      row++;
      ft.setHTML(row, 1,
          "Label displayed to users to identify item on application form. If empty, \"Short name\" from basic settings is used as fallback.");
      ftf.setStyleName(row, 1, "inputFormInlineComment");
  ...
  private FlexTable buildItemsTable(final FlexTable boxItemTable, final Map<String, String> values,
                                    final String locale) {
    ...
    Label boxContentLabel =
        new Label(item.getType().substring(0, 1) + item.getType().toLowerCase().substring(1) + " options:");
    ...
    HTML comment = new HTML(
        "Define possible options for selection in SELECTIONBOX, COMBOBOX, CHECKBOX, RADIO widget. Empty options are not used.");
    ...
    CustomButton addNewButton =
        new CustomButton(ButtonTranslation.INSTANCE.addButton(), ButtonTranslation.INSTANCE.addNewSelectionBoxItem(),
  ...
  private Widget basicInformationTab() {
    ...
    GetAttributesDefinition attrDef = new GetAttributesDefinition(new JsonCallbackEvents() {
      ...
      public void onError(PerunError error) {
        ...
        if (item.getPerunDestinationAttribute() != null && !item.getPerunDestinationAttribute().isEmpty()) {
          // add and select returned perun dest attr
          perunDestinationAttributeListBox.addItem(item.getPerunDestinationAttribute(),
      ...
      public void onFinished(JavaScriptObject jso) {
        ...
        if (list != null && !list.isEmpty()) {
          ...
          for (AttributeDefinition def : list) {
            ...
            boolean isMapOrListAttribute =
                def.getType().contains("ArrayList") || def.getType().contains("LinkedHashMap");
            ...
            if (def.getEntity().equalsIgnoreCase("user") || def.getEntity().equalsIgnoreCase("member")) {
              perunSourceAttributeListBox.addItem(
                  def.getFriendlyName() + " (" + def.getEntity() + " / " + def.getDefinition() + ")", def.getName());
              perunDestinationAttributeListBox.addItem(
                  def.getFriendlyName() + " (" + def.getEntity() + " / " + def.getDefinition() + ")", def.getName());
    ...
    row++;
    ft.setHTML(row, 1,
        "Internal item identification (used as fallback when you forgot to set \"Label\" for some language).");
    ftf.setStyleName(row, 1, "inputFormInlineComment");
    ...
    if (!item.getType().equals("SUBMIT_BUTTON") && !item.getType().equals("AUTO_SUBMIT_BUTTON") &&
        !item.getType().equals("HTML_COMMENT") && !item.getType().equals("HEADING")) {
      ...
      row++;
      ft.setHTML(row, 1, "Select attribute, which will be used to pre-fill form value. You can select also VO " +
          (groupId != 0 ? "and group " : "") + "attributes.");
      ftf.setStyleName(row, 1, "inputFormInlineComment");
      ftf.setColSpan(row, 1, 2);
  ...
  public Widget draw() {
    ...
    tabMenu.addWidget(
        TabMenu.getPredefinedButton(ButtonType.SAVE, ButtonTranslation.INSTANCE.saveFormItem(), new ClickHandler() {
          public void onClick(ClickEvent event) {
  ...
  protected void saveItem() {
    ...
    if (perunDestinationAttributeListBox.getSelectedIndex() > 0) {
      // some value set
      item.setPerunDestinationAttribute(
          perunDestinationAttributeListBox.getValue(perunDestinationAttributeListBox.getSelectedIndex()));
    } else {
      // empty value set
    ...
    if (perunSourceAttributeListBox.getSelectedIndex() > 0) {
      // some value set
      item.setPerunSourceAttribute(
          perunSourceAttributeListBox.getValue(perunSourceAttributeListBox.getSelectedIndex()));
    } else {
      // empty value set
