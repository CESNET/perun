...
public class MemberResourcesTabItem implements TabItem {
  ...
  public Widget draw() {
    ...
    this.titleWidget.setText(
        Utils.getStrippedStringWithEllipsis(member.getUser().getFullNameWithTitles().trim()) + ": resources");
    ...
    CustomButton addButton =
        TabMenu.getPredefinedButton(ButtonType.ADD, true, "Add member to new resource", new ClickHandler() {
          @Override
    ...
    CellTable<RichResource> table = resourcesCall.getTable(new FieldUpdater<RichResource, String>() {
      ...
      public void update(int i, RichResource resource, String s) {
        if (session.isVoAdmin(resource.getVoId()) || session.isFacilityAdmin(resource.getFacilityId())) {
          ...
          UiElements.generateInfo("Not privileged",
              "You are not allowed to manage this resource. You must be VO manager or Facility manager.");
        }
      }
    });
