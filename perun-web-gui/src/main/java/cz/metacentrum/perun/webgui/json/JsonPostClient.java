...
public class JsonPostClient {
  ...
  public JsonPostClient() {
    ...
    if (c == null) {
      ...
      layout.setHTML(0, 1, "<p>" +
          "Processing of your request(s) is taking longer than usual, but it's actively processed by the server.<p>Please do not close opened window/tab nor repeat your action. You will be notified once operation completes.<p>Remaining requests: " +
          counter);
  ...
  private void send() {
    ...
    final String requestUrl =
        URL.encode(PerunWebSession.getInstance().getRpcUrl() + url + "?callback=" + perunRequest.getStartTime());
      ...
      builder.sendRequest(payload, new RequestCallback() {
        ...
        public void onResponseReceived(Request req, Response resp) {
          ...
          if (resp.getStatusCode() == 200) {
            ...
            error.setErrorInfo(
                "Server responded with HTTP error: " + resp.getStatusCode() + " - " + resp.getStatusText());
            ...
            if (resp.getStatusCode() == 401 || resp.getStatusCode() == 403) {
              ...
              error.setErrorInfo(
                  "You are not authorized to server. Your session might have expired. Please refresh the browser window to re-login.");
              ...
              if (runningRequests.get(requestUrl) != null) {
                ...
                if ((runningRequests.get(requestUrl).getDuration() / (1000 * 60)) >= 5) {
                  ...
                  Scheduler.get().scheduleFixedDelay(new Scheduler.RepeatingCommand() {
                    ...
                    public boolean execute() {
                      GetPendingRequests req =
                          new GetPendingRequests(perunRequest.getStartTime(), new JsonCallbackEvents() {
                            @Override
                            public void onFinished(JavaScriptObject jso) {
                              final PerunRequest req = jso.cast();
                              if ((req.getCallbackId().equals(perunRequest.getStartTime() + "")) &&
                                  req.getEndTime() > 0) {
                                ...
                                if (again) {
                                  ...
                                  if ("".equalsIgnoreCase(error.getErrorId()) &&
                                      "".equalsIgnoreCase(error.getErrorInfo())) {
                                    // not error, OK
                                    if (session.getUiElements() != null) {
                                      session.getUiElements().setLogText("Response not NULL, not ERROR.");
                                    }
                  ...
                  error.setErrorInfo(
                      "Server encounter internal error while processing your request. Please report this error and retry.");
  ...
  protected void onRequestError(JavaScriptObject jso) {
    ...
    if (jso != null) {
      ...
      PerunError e = (PerunError) JsonUtils.parseJson("{\"errorId\":\"0\",\"name\":\"Cross-site request\",\"type\":\"" +
          WidgetTranslation.INSTANCE.jsonClientAlertBoxErrorCrossSiteType() + "\",\"message\":\"" +
          WidgetTranslation.INSTANCE.jsonClientAlertBoxErrorCrossSiteText() + "\"}").cast();
      if (session.getUiElements() != null) {
        session.getUiElements()
            .setLogErrorText("Error while sending request: The response was null or cross-site request.");
      }
      if (!hidden) {
        // creates a alert box
        JsonErrorHandler.alertBox(e);
