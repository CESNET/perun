...
public class UsersPublicationsTabItem implements TabItem, TabItemWithUrl {
  ...
  public static final String URL = "userpubs";
  /**
   * Perun web session
   */
  ...
  public Widget draw() {
    ...
    this.titleWidget.setText(
        Utils.getStrippedStringWithEllipsis(user.getFullNameWithTitles().trim()) + ": Publications");
          ...
          public void onClick(ClickEvent event) {
            ...
            for (int i = 0; i < list.size(); i++) {
              if (list.get(i).isLocked() && !session.isPerunAdmin()) {
                // skip locked pubs
                UiElements.generateAlert("Publication locked",
                    "Publication <strong>" + SafeHtmlUtils.fromString(list.get(i).getTitle()).asString() +
                        "</strong> is locked by " +
                ...
                if (list.get(i).getCreatedBy().equalsIgnoreCase(session.getPerunPrincipal().getActor())
                  ...
                  if (i == list.size() - 1) {
                    DeletePublication request =
                        new DeletePublication(JsonCallbackEvents.disableButtonEvents(removeButton, refreshTable));
                    request.deletePublication(list.get(i).getId());
                  } else {
                    DeletePublication request =
                        new DeletePublication(JsonCallbackEvents.disableButtonEvents(removeButton));
                    request.deletePublication(list.get(i).getId());
                  }
                } else {
                  ...
                  if (i == list.size() - 1) {
                    DeleteAuthorship request =
                        new DeleteAuthorship(JsonCallbackEvents.disableButtonEvents(removeButton, refreshTable));
                    request.deleteAuthorship(list.get(i).getId(), userId);
                  } else {
                    DeleteAuthorship request =
                        new DeleteAuthorship(JsonCallbackEvents.disableButtonEvents(removeButton));
                    request.deleteAuthorship(list.get(i).getId(), userId);
                  }
                }
              }
    ...
    final CustomButton showButton =
        new CustomButton("Show / Hide filter", "Show / Hide filtering options", SmallIcons.INSTANCE.filterIcon());
    ...
    // buttons
    CustomButton applyFilter =
        new CustomButton("Apply", "Shows publications based on filter", SmallIcons.INSTANCE.filterAddIcon());
    CustomButton clearFilter =
        new CustomButton("Clear", "Shows all publications of User", SmallIcons.INSTANCE.filterDeleteIcon());
  ...
  public void open() {
    ...
    session.getUiElements().getBreadcrumbs()
        .setLocation(MainMenu.USER, Utils.getStrippedStringWithEllipsis(user.getFullNameWithTitles().trim()),
            UsersTabs.URL + UrlMapper.TAB_NAME_SEPARATOR + getUrl() + "?id=" + userId, "Publications",
