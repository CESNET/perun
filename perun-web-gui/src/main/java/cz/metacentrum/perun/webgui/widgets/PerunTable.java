...
public class PerunTable<T extends JavaScriptObject> extends CellTable<T> {
  ...
  public void addDescriptionColumn(FieldUpdater<T, String> tableFieldUpdater, int width) {
    ...
    if (tableFieldUpdater == null) {
      ...
      this.columnSortHandler.setComparator(descriptionColumn,
          new GeneralComparator<T>(GeneralComparator.Column.DESCRIPTION));
    ...
    if (hyperlinksAllowed) {
      Column<T, T> descriptionColumn =
          JsonUtils.addCustomCellColumn(new HyperlinkCell<T>("description"), tableFieldUpdater);
