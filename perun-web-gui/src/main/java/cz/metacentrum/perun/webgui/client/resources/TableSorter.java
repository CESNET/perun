    ...
    Collections.sort(list, new Comparator<T>() {
      public int compare(T o1, T o2) {
        ...
        return customCollator.compare(o3.getService().getName(), o4.getService().getName()) +
            new Date((long) o4.getTimestampNative()).compareTo(new Date((long) o3.getTimestampNative()));
      }
    });
    return list;
  ...
  public ArrayList<T> sortByDestination(ArrayList<T> list) {
    ...
    Collections.sort(list, new Comparator<T>() {
      public int compare(T o1, T o2) {
        ...
        if ("TaskResult".equals(((GeneralObject) o1).getObjectType()) &&
            "TaskResult".equals(((GeneralObject) o2).getObjectType())) {
          return smartCompare(((TaskResult) o1).getDestination().getDestination(),
              ((TaskResult) o2).getDestination().getDestination());
        }

        if ("Destination".equals(((GeneralObject) o1).getObjectType()) &&
            "Destination".equals(((GeneralObject) o2).getObjectType())) {
          return smartCompare(((Destination) o1).getDestination(), ((Destination) o2).getDestination());
        }

        if ("RichDestination".equals(((GeneralObject) o1).getObjectType()) &&
            "RichDestination".equals(((GeneralObject) o2).getObjectType())) {
          return smartCompare(((Destination) o1).getDestination(), ((Destination) o2).getDestination());
        }
