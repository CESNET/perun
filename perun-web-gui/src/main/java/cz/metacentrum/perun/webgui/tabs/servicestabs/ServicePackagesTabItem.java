...
public class ServicePackagesTabItem implements TabItem, TabItemWithUrl {
  ...
  public static final String URL = "pack";
  /**
   * Perun web session
   */
  ...
  public Widget draw() {
    ...
    // create button
    tabMenu.addWidget(
        TabMenu.getPredefinedButton(ButtonType.CREATE, true, ButtonTranslation.INSTANCE.createServicePackage(),
            new ClickHandler() {
              @Override
    ...
    final CustomButton deleteButton =
        TabMenu.getPredefinedButton(ButtonType.DELETE, ButtonTranslation.INSTANCE.deleteSelectedServicePackages());
    ...
    final CustomButton save =
        TabMenu.getPredefinedButton(ButtonType.SAVE, ButtonTranslation.INSTANCE.saveChangesInServicesPackages());
  ...
  public void open() {
    ...
    session.getUiElements().getBreadcrumbs()
        .setLocation(MainMenu.PERUN_ADMIN, "Services packages", getUrlWithParameters());
  }
