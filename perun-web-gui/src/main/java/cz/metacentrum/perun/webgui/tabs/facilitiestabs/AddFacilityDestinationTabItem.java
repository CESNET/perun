...
public class AddFacilityDestinationTabItem implements TabItem {
  ...
  public Widget draw() {
    ...
    final CustomButton addButton =
        TabMenu.getPredefinedButton(ButtonType.ADD, ButtonTranslation.INSTANCE.addDestination());
    ...
    final GetFacilityAssignedServices getAssignedServices =
        new GetFacilityAssignedServices(facility.getId(), fillAssignedServices);
      ...
      public void onClick(ClickEvent event) {
        ...
        if (validator.validateSuggestBox()) {
          ...
          if (services.getSelectedIndex() == 0) {
            ...
            if (useHosts.getValue() == true) {
              // auto by hosts
              AddDestinationsByHostsOnFacility request = new AddDestinationsByHostsOnFacility(facility,
                  JsonCallbackEvents.closeTabDisableButtonEvents(addButton, tab, true));
              request.addDestinationByHosts(services.getAllObjects());
            } else {
              // default
              AddDestination request =
                  new AddDestination(facility, JsonCallbackEvents.closeTabDisableButtonEvents(addButton, tab, true));
              request.addDestination(destination.getSuggestBox().getText().trim(),
                  type.getValue(type.getSelectedIndex()), services.getAllObjects(), propTypeSelect.getSelectedValue());
            }
          } else {
            // selected one
            if (useHosts.getValue() == true) {
