...
public class urn_perun_group_resource_attribute_def_def_o365EmailAddresses_muTest {
  ...
  public void fillAttribute() throws Exception {
    ...
    when(am.getPerunBeanIdsForUniqueAttributeValue(eq(session),
        argThat(new BeanAttributeMatcher("group_resource")))).thenReturn(
        Sets.newHashSet(new Pair<>(group.getId(), resource.getId())));
    when(
  ...
  public void testUniqClash() throws Exception {
    ...
    when(am.getPerunBeanIdsForUniqueAttributeValue(eq(session),
        argThat(new BeanAttributeMatcher("group_resource")))).thenReturn(Sets.newHashSet(new Pair<>(1000, 2000)));
    when(
        am.getPerunBeanIdsForUniqueAttributeValue(eq(session), argThat(new BeanAttributeMatcher("member")))).thenReturn(
        Sets.newHashSet());
    classInstance.checkAttributeSemantics(session, group, resource, attributeToCheck);
