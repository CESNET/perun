...
public class urn_perun_group_attribute_def_def_unixGroupName_namespaceTest {
  ...
  public void testGroupWithSameGID() throws Exception {
    ...
    when(sess.getPerunBl().getAttributesManagerBl().getAllAttributesStartWithNameWithoutNullValue(sess, group,
        AttributesManager.NS_GROUP_ATTR_DEF + ":unixGID-namespace:")).thenReturn(
    ...
    when(sess.getPerunBl().getModulesUtilsBl()
        .haveRightToWriteAttributeInAnyGroupOrResource(sess, listOfGroups, new ArrayList<>(), attributeToCheck,
            attributeToCheck)).thenReturn(true);
    when(sess.getPerunBl().getModulesUtilsBl()
  ...
  public void testResourceWithSameGID() throws Exception {
    ...
    when(sess.getPerunBl().getModulesUtilsBl()
        .getListOfResourceGIDsFromListOfGroupGIDs(sess, new ArrayList<>())).thenReturn(
        Collections.singletonList(attributeToCheck));
    when(sess.getPerunBl().getModulesUtilsBl()
