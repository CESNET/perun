...
public class urn_perun_user_attribute_def_virt_groupNamesTest {
  ...
  public void getUserGroupNamesAttributeValue() throws Exception {
    ...
    when(session.getPerunBl().getDatabaseManagerBl().getJdbcPerunTemplate().query(
        ...
        urn_perun_user_attribute_def_virt_groupNames.ROW_MAPPER, user.getId(), Status.VALID.getCode(),
        MemberGroupStatus.VALID.getCode())).thenReturn(dbResult);
    ...
    List<String> expected =
        Arrays.asList(vo1.getShortName(), vo2.getShortName(), vo1.getShortName() + ":" + group1.getName(),
