...
public class urn_perun_member_resource_attribute_def_virt_isBannedTest {
  ...
  public void resolveVirtualAttributeValueChangeTest() throws Exception {
    ...
    when(session.getPerunBl().getMembersManagerBl().getMemberById(any(PerunSessionImpl.class), anyInt())).thenReturn(
        member);
    when(
        session.getPerunBl().getResourcesManagerBl().getResourceById(any(PerunSessionImpl.class), anyInt())).thenReturn(
        resource);
    when(session.getPerunBl().getAttributesManagerBl()
        .getAttributeDefinition(any(PerunSessionImpl.class), anyString())).thenReturn(
        classInstance.getAttributeDefinition());
    resolvedEvents = classInstance.resolveVirtualAttributeValueChange(session, event1);
    assertEquals(resolvedEvents.size(), 1);
    assertEquals(resolvedEvents.get(0).getMessage(),
        isBanned.serializeToString() + " changed for " + resource.serializeToString() + " and " +
    ...
    when(session.getPerunBl().getFacilitiesManagerBl()
        .getAssignedResources(any(PerunSessionImpl.class), any(Facility.class))).thenReturn(
        Collections.singletonList(resource));
    when(session.getPerunBl().getResourcesManagerBl()
        .getAssignedMembers(any(PerunSessionImpl.class), any(Resource.class))).thenReturn(
  ...
  public void resolveVirtualAttributeValueChangeTestWithWrongMatch() throws Exception {
    ...
    when(
        ...
        resource);
    when(session.getPerunBl().getAttributesManagerBl()
        .getAttribute(any(PerunSessionImpl.class), any(Member.class), any(Resource.class), anyString())).thenReturn(
        isBanned);
