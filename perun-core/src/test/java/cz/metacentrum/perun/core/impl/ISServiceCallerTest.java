...
public class ISServiceCallerTest {
  ...
  private static final String TEST_ERR_RESPONSE_ERROR =
      "Navrhované heslo nelze použít, protože začíná nebo končí mezerou. Zvolte si prosím jiné.";
  ...
  public void testParseErrResponse() throws UnsupportedEncodingException {
    int reqId = 1;
    ISResponseData data =
        isServiceCaller.parseResponse(new ByteArrayInputStream(TEST_ERR_RESPONSE.getBytes(StandardCharsets.UTF_8)),
            reqId);
  ...
  public void testParseOkResponse() throws UnsupportedEncodingException {
    int reqId = 1;
    ISResponseData data =
        isServiceCaller.parseResponse(new ByteArrayInputStream(TEST_OK_RESPONSE.getBytes(StandardCharsets.UTF_8)),
            reqId);
