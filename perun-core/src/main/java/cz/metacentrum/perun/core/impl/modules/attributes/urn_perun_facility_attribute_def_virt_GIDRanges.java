 ...
 */
public class urn_perun_facility_attribute_def_virt_GIDRanges extends FacilityVirtualAttributesModuleAbstract
  ...
  private static final String A_E_namespaceGIDRanges =
    ...
    try {
      ...
      if (gidNamespaceAttribute.getValue() == null) {
        throw new WrongReferenceAttributeValueException(attribute, gidNamespaceAttribute,
            "There is missing GID namespace on the facility.");
      }
      Attribute namespaceGIDRangesAttribute =
          getNamespaceGIDRangesAttribute(sess, gidNamespaceAttribute.valueAsString());
      sess.getPerunBl().getAttributesManagerBl()
          .checkAttributeSemantics(sess, gidNamespaceAttribute.valueAsString(), namespaceGIDRangesAttribute);
  ...
  @Override
  public boolean setAttributeValue(PerunSessionImpl sess, Facility facility, Attribute attribute)
    ...
    Attribute namespaceGIDRangesAttribute =
        getNamespaceGIDRangesAttribute(sess, (String) gidNamespaceAttribute.getValue());
    if (!(attribute.getValue() == null ? namespaceGIDRangesAttribute.getValue() == null :
        attribute.getValue().equals(namespaceGIDRangesAttribute.getValue()))) {
      //attribute from param have other value then physical attribute
      throw new WrongReferenceAttributeValueException(attribute, namespaceGIDRangesAttribute,
