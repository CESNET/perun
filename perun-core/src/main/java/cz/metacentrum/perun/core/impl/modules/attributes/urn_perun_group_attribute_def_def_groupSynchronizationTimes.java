 ...
 */
public class urn_perun_group_attribute_def_def_groupSynchronizationTimes extends GroupAttributesModuleAbstract
  ...
  public void checkAttributeSemantics(PerunSessionImpl sess, Group group, Attribute attribute)
    ...
    try {
      Attribute foundAttribute = sess.getPerunBl().getAttributesManagerBl()
          .getAttribute(sess, group, GroupsManager.GROUPSYNCHROINTERVAL_ATTRNAME);
      if (foundAttribute.getValue() != null) {
        throw new WrongReferenceAttributeValueException(attribute, foundAttribute, group, null, group, null,
            "Attribute " + attribute.getName() + " cannot be set because attribute " +
                GroupsManager.GROUPSYNCHROINTERVAL_ATTRNAME + " is already set.");
      }
    } catch (AttributeNotExistsException exc) {
      throw new ConsistencyErrorException(
          "Attribute " + GroupsManager.GROUPSYNCHROINTERVAL_ATTRNAME + " is supposed to exist", exc);
    }
  }
  ...
  public AttributeDefinition getAttributeDefinition() {
    ...
    attr.setDescription(
        "List of time values for group synchronization in format HH:MM rounded to 5 minute. For example 08:50 or 20:55");
    return attr;
  }
}