...
public interface ServicesManager {
  ...
  List<Destination> addDestinationsForAllServicesOnFacility(PerunSession perunSession, Facility facility,
                                                            Destination destination)
      throws PrivilegeException, FacilityNotExistsException, DestinationAlreadyAssignedException,
   ...
   */
  void addRequiredAttributes(PerunSession perunSession, Service service,
  ...
  /**
   * Bans the Service on the destination - each pair defined by the rich destination. It wouldn't be possible to execute
   * the given Service on this destination, however, it still can be executed on all the other destinations in the
   * facility.
   *
   * @param perunSession
   * @param richDestinations the list of rich destinations
   */
  void blockServicesOnDestinations(PerunSession perunSession, List<RichDestination> richDestinations)
      throws PrivilegeException, DestinationNotExistsException, FacilityNotExistsException;

  /**
   * Bans services on facility. It wouldn't be possible to execute the given services on the whole facility nor on any
   ...
   */
  void blockServicesOnFacility(PerunSession sess, List<Service> services, Facility facility)
   ...
   */
  void forceServicePropagationBulk(PerunSession sess, Facility facility, List<Service> services)
   ...
   * @throws ForceServicePropagationDisabledException when forcing propagation is not possible
   */
  void forceServicePropagationBulk(PerunSession sess, List<Service> services)
      throws PrivilegeException, ForceServicePropagationDisabledException;

  /**
   * Forces service propagation on all facilities for all services where hostname is used. Service must be correctly
   * assigned, have destination matching hostname and not be blocked on any level (globally, facility, destination).
   *
   * @param perunSession
   * @param hostname     Hostname to get facilities for
   * @return message Space separated list of forced services or error message (no facilities found, services are
   * blocked)
   */
   ...
   */
  Destination getDestinationById(PerunSession perunSession, int id)
  ...
  /**
   * Generates hashed data with group structure for given service and facility. If enforcing consents is turned on on
  ...
  /**
   * Generates hashed hierarchical data structure for given service and facility. If enforcing consents is turned on on
   * the instance and on the resource's consent hub, generates only the users that granted a consent to all the service
   * required attributes. New UNSIGNED consents are created to users that don't have a consent containing all the
   * service required attributes.
   * <p>
   ...
   * @throws ServiceNotExistsException
   */
  Service getServiceByName(PerunSession perunSession, String name)
      throws PrivilegeException, ServiceNotExistsException;
   ...
   * destination
   */
  boolean isServiceBlockedOnDestination(PerunSession sess, Service service, int destinationId)
      throws PrivilegeException, DestinationNotExistsException, FacilityNotExistsException;
  ...
  /**
   * Removes destinations defined by list of rich destinations. Each destination is removed from the rich destination's
   ...
   * @throws DestinationAlreadyRemovedException
   */
  void removeDestinationsByRichDestinations(PerunSession perunSession, List<RichDestination> richDestinations)
      throws PrivilegeException, ServiceNotExistsException, FacilityNotExistsException,
                 DestinationAlreadyRemovedException;
  ...
  /**
   * Free the denial of the Service on the destination - each pair defined by the rich destination. If the Service was
   * banned on the destination, it will be freed. In case the Service was not banned on the destination, nothing will
   * happen.
   *
   * @param perunSession
   * @param richDestinations the list of rich destinations
   */
  void unblockServicesOnDestinations(PerunSession perunSession, List<RichDestination> richDestinations)
      throws PrivilegeException, FacilityNotExistsException, DestinationNotExistsException;
   ...
   */
  void unblockServicesOnFacility(PerunSession sess, List<Service> services, Facility facility)
