...
public interface VosManager {
   ...
   * attribute. If user is member only in member vo, creates member in parent vo and sets memberOrganizations list
   * attribute.
   *
   * @param sess     session
   * @param vo       vo
   ...
   * sponsors of the users will be set to the sponsored members.
   *
   * @param sess session
  ...
  /**
   * For the given vo, creates sponsored members for each sponsored user who is a member of the given vo. The sponsored
   * members will be sponsored by the given user, not by its original sponsors.
   *
   * @param sess       session
   * @param vo         vo where members will be converted
   ...
   * @param perunSession
   * @param vo
   * @param forceDelete  force the deletion of the VO, regardless there are any existing entities associated with the VO
   ...
   * @param vo
   * @param role
   * @return List of groups, who are administrators of the Vo with supported role. Returns empty list if there is no VO
   * group admin.
   * @throws VoNotExistsException
   * @throws InternalErrorException
   * @throws PrivilegeException
   ...
   * @throws PrivilegeException      privilege exception
   */
  List<MemberCandidate> getCompleteCandidates(PerunSession sess, Group group, List<String> attrNames,
                                              String searchString) throws GroupNotExistsException, PrivilegeException;
  ...
  /**
   * Get list of Vo administrators, which are directly assigned (not by group membership) with specific attributes. From
   * list of specificAttributes get all Users Attributes and find those for every RichAdmin (only, other attributes are
   * not searched)
   *
   * @param perunSession
   * @param vo
  ...
  /**
   * Gets all Enriched Bans for given vo. Attr names specify which attributes should be included in RichMember.
   *
   * @param sess      session
   * @param voId      id of vo
   * @param attrNames List of attribute names, returns all attributes if empty or null
   ...
   * @throws VoNotExistsException
   */
  List<EnrichedBanOnVo> getEnrichedBansForVo(PerunSession sess, int voId, List<String> attrNames)
  ...
  /**
   * Get list of richUser administrators of specific vo for specific role. If some group is administrator of the VO, all
   * VALID members are included in the list.
   * <p>
   * Supported roles: VOOBSERVER, TOPGROUPCREATOR, VOADMIN, SPONSOR
   * <p>
   * If "onlyDirectAdmins" is "true", return only direct users of the vo for supported role with specific attributes. If
   * "allUserAttributes" is "true", do not specify attributes through list and return them all in objects richUser .
  ...
  List<RichUser> getRichAdmins(PerunSession perunSession, Vo vo, String role, List<String> specificAttributes,
      ...
      throws PrivilegeException, VoNotExistsException, RoleNotSupportedException, UserNotExistsException;
   ...
   */
  @Deprecated
  List<RichUser> getRichAdminsWithAttributes(PerunSession perunSession, Vo vo)
      throws PrivilegeException, VoNotExistsException, UserNotExistsException;

  /**
   * Get list of Vo administrators with specific attributes. From list of specificAttributes get all Users Attributes
  ...
  /**
   * Get list of Vos by Access Righs: If User is: - PERUNADMIN : get all Vos - VoAdmin : Vo where user is Admin -
   ...
   */
  void removeAdmin(PerunSession perunSession, Vo vo, User user)
   ...
   * @throws GroupNotExistsException
   */
  void removeAdmin(PerunSession perunSession, Vo vo, Group group)
   ...
   * @throws MemberNotExistsException if there is no such member
   */
  void removeBanForMember(PerunSession sess, Member member)
      throws PrivilegeException, BanNotExistsException, MemberNotExistsException;

  /**
   * Removes member vo from given vo. Updates memberOrganizations list attribute for those members of parent vo who came
   * from the member vo.
   *
   * @param sess     session
   * @param vo       vo
   ...
   * @throws VoNotExistsException       if any of the vos don't exist
   */
  void removeMemberVo(PerunSession sess, Vo vo, Vo memberVo)
   ...
   * @throws PrivilegeException
   */
  void removeSponsorRole(PerunSession sess, Vo vo, User user)
   ...
   * @throws PrivilegeException
   */
  void removeSponsorRole(PerunSession sess, Vo vo, Group group)
   ...
   */
  BanOnVo setBan(PerunSession sess, BanOnVo ban)
