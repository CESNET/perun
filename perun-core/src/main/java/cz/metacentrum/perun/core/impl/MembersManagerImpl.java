...
public class MembersManagerImpl implements MembersManagerImplApi {
  ...
  final static String memberMappingSelectQuery =
      "members.id as members_id, members.user_id as members_user_id, members.vo_id as members_vo_id, members.status as members_status, " +
  ...
  final static String groupsMembersMappingSelectQuery =
      memberMappingSelectQuery + ", groups_members.membership_type as membership_type, " +
  ...
  final static String groupsAssignedMembersMappingSelectQuery =
      groupsMembersMappingSelectQuery + ", groups_resources_state.status as group_resource_status";
  ...
  static final RowMapper<Member> MEMBER_MAPPER = (rs, i) -> {
    Member member = new Member(rs.getInt("members_id"), rs.getInt("members_user_id"), rs.getInt("members_vo_id"),
        Status.getStatus(rs.getInt("members_status")),
        rs.getString("members_created_at"), rs.getString("members_created_by"), rs.getString("members_modified_at"),
        rs.getString("members_modified_by"),
  ...
  public Member createMember(PerunSession sess, Vo vo, User user) throws AlreadyMemberException {
    ...
    try {
      ...
      jdbc.update(
          "insert into members (id, vo_id, user_id, status, created_by,created_at,modified_by,modified_at,created_by_uid,modified_by_uid) "
              + "values (?,?,?,?,?," + Compatibility.getSysdate() + ",?," + Compatibility.getSysdate() + ",?,?)",
          newId, vo.getId(),
          user.getId(), Status.INVALID.getCode(), sess.getPerunPrincipal().getActor(),
  ...
  public List<Member> getMembersByIds(PerunSession perunSession, List<Integer> ids) {
    try {
      return jdbc.execute(
          "select " + memberMappingSelectQuery + " from members where id " + Compatibility.getStructureForInClause(),
  ...
  public List<Member> getMembersByUsers(PerunSession sess, List<User> users, Vo vo) {
    ...
    try {
      return this.namedParameterJdbcTemplate.query("SELECT " + memberMappingSelectQuery +
              " FROM members WHERE members.user_id IN ( :ids ) AND members.vo_id=:vo",
          parameters, MEMBER_MAPPER);
    } catch (EmptyResultDataAccessException ex) {
      return new ArrayList<>();
    } catch (RuntimeException ex) {
  ...
  public UUID storePasswordResetRequest(PerunSession sess, User user, String namespace, String mail,
    ...
    try {
      return jdbc.queryForObject(
          ...
          UUID.class,
          namespace, user.getId(), mail, validityTo, sess.getPerunPrincipal().getActor(),
          sess.getPerunPrincipal().getUserId());
    } catch (RuntimeException e) {
      throw new InternalErrorException(e);
    }
  ...
  public List<Member> getSponsoredMembers(PerunSession sess, Vo vo, User sponsor) {
    try {
      return jdbc.query("SELECT " + memberMappingSelectQuery +
  ...
  public List<Member> getSponsoredMembers(PerunSession sess, Vo vo) {
    try {
      return jdbc.query("SELECT DISTINCT " + memberMappingSelectQuery +
          " FROM members JOIN members_sponsored ms ON (members.id=ms.sponsored_id) " +
  ...
  public Paginated<Member> getMembersPage(PerunSession sess, Vo vo, MembersPageQuery query, String policy)
    ...
    if (query.getGroupId() != null) {
      groupByQuery +=
          ", users.last_name, users.first_name, groups_members.group_id, groups_members.source_group_id, groups_members.membership_type, groups_members.source_group_status";
    }
  ...
  public void moveMembersApplications(PerunSession sess, Member sourceMember, Member targetMember) {
    try {
      jdbc.update("update application set user_id=?, modified_at=" + Compatibility.getSysdate() +
              ", modified_by_uid=? where user_id=? and vo_id=?",
          targetMember.getUserId(), sess.getPerunPrincipal().getUserId(), sourceMember.getVoId(),
          sourceMember.getVoId());
    } catch (RuntimeException e) {
      throw new InternalErrorException(e);
    }
  }
