...
public class urn_perun_user_facility_attribute_def_virt_defaultUnixGID
  ...
  public Attribute getAttributeValue(PerunSessionImpl sess, User user, Facility facility,
    ...
    try {
      ...
      Attribute defaultUnixGID = sess.getPerunBl().getAttributesManagerBl()
      ...
      String gidNamespace = (String) sess.getPerunBl().getAttributesManagerBl()
          .getAttribute(sess, facility, AttributesManager.NS_FACILITY_ATTR_DEF + ":unixGID-namespace").getValue();
      if (gidNamespace == null) {
        return attr;
      }

      String groupNameNamespace = (String) sess.getPerunBl().getAttributesManagerBl()
          .getAttribute(sess, facility, AttributesManager.NS_FACILITY_ATTR_DEF + ":unixGroupName-namespace").getValue();
      if (groupNameNamespace == null) {
        return attr;
      }

      Attribute userPreferredGroupNames = sess.getPerunBl().getAttributesManagerBl().getAttribute(sess, user,
          AttributesManager.NS_USER_ATTR_DEF + ":preferredUnixGroupName-namespace:" + groupNameNamespace);
      ...
      if (userPreferredGroupNames.getValue() != null) {
        ...
        for (Resource resource : resources) {
          String groupNameForTest = (String) sess.getPerunBl().getAttributesManagerBl().getAttribute(sess, resource,
              AttributesManager.NS_RESOURCE_ATTR_DEF + ":unixGroupName-namespace:" + groupNameNamespace).getValue();
        ...
        for (Resource resource : resources) {
          List<Group> groupsFromResource =
              sess.getPerunBl().getGroupsManagerBl().getAssignedGroupsToResource(sess, resource);
          for (Group group : groupsFromResource) {
            ...
            if (!groupMembers.isEmpty()) {
              String groupNamesForTest = (String) sess.getPerunBl().getAttributesManagerBl().getAttribute(sess, group,
                  AttributesManager.NS_GROUP_ATTR_DEF + ":unixGroupName-namespace:" + groupNameNamespace).getValue();
      ...
      Attribute basicGid = sess.getPerunBl().getAttributesManagerBl()
          .getAttribute(sess, facility, user, AttributesManager.NS_USER_FACILITY_ATTR_DEF + ":basicDefaultGID");
