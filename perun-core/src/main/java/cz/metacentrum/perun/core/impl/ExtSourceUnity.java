...
public class ExtSourceUnity extends ExtSourceImpl implements ExtSourceApi {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(ExtSourceUnity.class);
  ...
  private UnityEntity connectAndGetValidEntityByGroup(int entityId, String groupName) throws IOException {
    ...
    if (groups != null && compareGroupName(groups, groupName)) {
      ...
      if (entity == null) {
        throw new InternalErrorException("Entity returned for entityId " + entityId + " was null.");
      }
  ...
  protected HttpURLConnection createConnection(String uri) throws IOException {
    ...
    if (responseCode != 200) {
      LOG.warn(
          "Cannot establish connection to " + uri + ". Response code is " + responseCode + ". Error message is \'" +
  ...
  /**
   * Method obtains JSONArray, that represents attributes of the entity with specific entity id.
   *
   * @param id entity id
  ...
  private List<Map<String, String>> jsonParsing(String query, int maxResults) {
    try {
          ...
          switch (attrName) {
            ...
            default:
              throw new IllegalArgumentException(
                  "Query can use only 'login' or 'email' values. " + "Value \'" + attrName + "\' is not allowed.");
          }
      ...
      LOG.error("IOException while trying to connect to Unity REST API to obtain user(s) containing value " + query,
          ex);
    }
    return null;
  }
  ...
  private List<Map<String, String>> jsonParsingGroups(String queryForGroup) {
    try {
      ...
      LOG.error("IOException while trying to connect to Unity REST API to obtain users of the group " + queryForGroup,
          ex);
    }
  ...
  /**
   * Method obtains JSON object, that represents entity in Unity API and checks if it's state is VALID. If yes, then it
   * transforms object to UnityEntity class. If not, then it does nothing.
   *
  ...
  /**
   * Method transforms unityEntity to Map<String,String> where first String represents mapName and second String
   * represents mapValue according to mapAttribute in the extSources.xml file.
   *
  ...
  private Map<String, String> processUnityMapping(UnityEntity unityEntity) {
    ...
    for (String mappingElement : mappingArray) {
      ...
      if (index <= 0) {
        throw new InternalErrorException(
            "There is no text in unityMapping attribute or " + "there is no '=' character.");
      }
    ...
    if (unityEntity.getExtSourceLogin() != null) {
      map.put(ExtSourcesManagerImpl.USEREXTSOURCEMAPPING + "1",
          "https://extidp.cesnet.cz/idp/shibboleth|cz.metacentrum.perun.core.impl.ExtSourceIdp|" +
          unityEntity.getExtSourceLogin() + "@google.extidp.cesnet.cz");
    }
  ...
  private static class UnityEntity {
    ...
    public String toString() {
      return "UnityEntity{" + "entityId='" + entityId + '\'' + ", value='" + login + '\'' + ", fullName='" + fullName +
             '\'' + ", email='" + email + '\'' + '}';
    }
  }
}
