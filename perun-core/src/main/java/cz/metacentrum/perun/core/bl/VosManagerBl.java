...
public interface VosManagerBl {
  ...
  /**
   * Finds MemberCandidates who can join the Group. If the given vo is not null, it searches only users who belong to
   * this Vo or who have ues in any of given extSources.
   *
   * @param sess         session
   * @param vo           vo if vo is null, users are searched in whole perun, otherwise users are searched in members of
   *                     given vo and in users with ues in any of given extSources
   * @param group        group to be used
   * @param attrNames    name of attributes to be searched
   * @param searchString depends on the extSource of the Vo, could by part of the name, email or something like that.
   ...
   * @return true if user is directly in role for the vo, or if "checkGroups" flag is set and at least one of the groups
   * is in the role
