...
public class AdminmetaPasswordManagerModule extends GenericPasswordManagerModule {
	...
	protected final Pattern adminMetaPasswordContainsSpec = Pattern.compile(".*[\\x20-\\x2F\\x3A-\\x40\\x5B-\\x60\\x7B-\\x7E].*");
	...
	public void checkLoginFormat(PerunSession sess, String login) throws InvalidLoginException {
		...
		((PerunBl)sess.getPerun()).getModulesUtilsBl().checkLoginNamespaceRegex(actualLoginNamespace, login, adminMetaLoginPattern);
	...
	public void checkPasswordStrength(PerunSession sess, String login, String password) throws PasswordStrengthException {
		...
		if (password.length() < adminMetaPasswordMinLength) {
			log.warn("Password for {}:{} is too short. At least {} characters are required.", actualLoginNamespace, login, adminMetaPasswordMinLength);
			throw new PasswordStrengthException("Password for " + actualLoginNamespace + ":" + login + " is too short. At least "+ adminMetaPasswordMinLength +" characters are required.");
		}
		...
		int groupsCounter = 0;
		if (adminMetaPasswordContainsDigit.matcher(password).matches()) groupsCounter++;
		if (adminMetaPasswordContainsUpper.matcher(password).matches()) groupsCounter++;
		if (adminMetaPasswordContainsLower.matcher(password).matches()) groupsCounter++;
		if (adminMetaPasswordContainsSpec.matcher(password).matches()) groupsCounter++;
