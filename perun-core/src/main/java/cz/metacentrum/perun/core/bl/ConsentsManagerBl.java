...
public interface ConsentsManagerBl {
  ...
  /**
   * Creates a new consent with status 'UNSIGNED'. Consent's attributes are computed based on the attributes of the user
   * and the consent hub. Attributes in the consent are ignored!
   * <p>
   * If the user has already 'UNSIGNED' consent for the same consent hub, that consent is deleted and replaced with the
   * new one.
   *
  ...
  /**
   * This method runs in a new transaction!! Because it is used by getData methods which run in read-only serializable
   * transactions.
   * <p>
   * Returns members from the given members list that have a valid consent for the propagation of the service required
   * attributes to the facility.
   * <p>
   * Users must have a consent for the facility's consent hub with status GRANTED and it contains all the required
   * attributes.
   * <p>
   * If the user has no consent in any status that contains all the required attributes, a new UNSIGNED consent is
   * created for the user.
   * <p>
   * If the consent logic is turned off on the instance (property forceConsents) or the facility's consent hub doesn't
   * enforce consents, all members are returned and no unsigned consents for users are created.
   *
   * @param sess     perun session
   ...
   * enforce consents, all members are returned and no unsigned consents for users are created.
   * <p>
   * If the consent evaluation is turned off (in CLI without the -c option), the members with UNSIGNED consent creation
   * is skipped.
   *
   * @param sess        perun session
   * @param service     the service
   ...
   * <p>
   * For given ConsentHub, collects all attributes from services assigned to it and compares it against attributes from
   * users' Consent objects that are assigned to resources of given ConsentHub object.
   * <p>
   ...
   * <p>
   * Method finds all consent hubs that contain given service in any of its facilities and start consent consolidation
   * for each consent hub (only selected service in consent hubs will be checked). Service defines whether only active
   * users will be evaluated or expired ones as well. If new consent is created, attributes from ALL services under
   * given consent hub are gathered for it.
   *
   * @param sess    session
