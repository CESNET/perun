...
public class TasksManagerImpl implements TasksManagerImplApi {
  ...
  public static final String TASK_RESULT_MAPPING_SELECT_QUERY =
      ...
      "tasks_results_return_code, tasks_results.timestamp as tasks_results_timestamp ";
  public static final RowMapper<TaskResult> TASKRESULT_ROWMAPPER = (resultSet, i) -> {
    ...
    } else if (resultSet.getString("tasks_results_status")
        .equalsIgnoreCase(TaskResult.TaskResultStatus.ERROR.toString())) {
      taskResult.setStatus(TaskResult.TaskResultStatus.ERROR);
    } else if (resultSet.getString("tasks_results_status")
        .equalsIgnoreCase(TaskResult.TaskResultStatus.DENIED.toString())) {
      taskResult.setStatus(TaskResult.TaskResultStatus.DENIED);
    } else if (resultSet.getString("tasks_results_status")
        .equalsIgnoreCase(TaskResult.TaskResultStatus.WARNING.toString())) {
  ...
  public static final String TASK_MAPPING_SELECT_QUERY =
      " tasks.id as tasks_id, tasks.schedule as tasks_schedule, tasks.recurrence as tasks_recurrence, " +
      "tasks.delay as tasks_delay, tasks.status as tasks_status, tasks.start_time as tasks_start_time, tasks" +
      ".end_time as tasks_end_time ";
  ...
  /**
   * Create new instance of this class. Used for the tests only
   */
  ...
  /**
   * Method create formatter with default settings for perun timestamps and set ResolverStyle to STRICT Timestamp
   * format:  "dd-MM-yyyy HH:mm:ss" - "ex. 01-01-2014 10:10:10"
   * <p>
      ...
      if (!isASingleByteUTF8Char(data[limit - 1])) {
        ...
        while (!isAStartingByteUTF8Char(data[limit + i])) {
          if (i == MAX_NUMBER_OF_UTF8_BYTES) {
            LOG.error(
                "The message data contains invalid UTF-8 character. The byte limit for one character was exceeded.");
            break;
          }
   ...
   * "dd-MM-yyyy HH:mm:ss" - "ex. 01-01-2014 10:10:10"
   * <p>
   ...
   * <p>
   * That means, check if the value is in range 00000000 ... 01111111. If the given byte starts with '1' it means its
   * lower than 0 because bytes in Java are represented with inversion code.
   *
   ...
   * <p>
   * An initial character can be in those formats: 110XXXXXX 1110XXXXX 11110XXXX 0XXXXXXXX
   * <p>
   * If the given value is in range 10000000(inclusive) ... 11000000(exclusive) it means that the byte is part of a
   * UTF-8 character composed of multiple bytes.
   * <p>
   * The value 10000000 for byte in Java is equal to -128 and this value is minimal. The value 11000000 for byte in Java
   * is equal to -64.
   *
   * @param b byte to check
  ...
  public int deleteOldTaskResults(int numDays) {
    try {
      ...
      return jdbc.update("delete from tasks_results where " + "id in (" + "select otr.id from tasks_results otr " +
                         "         left join ( " +
                         " select tr.destination_id, tr.task_id, max(tr.timestamp) as maxtimestamp " +
                         " from tasks_results tr " + " inner join tasks t on tr.task_id = t.id " +
                         ...
                         Compatibility.toDate("?", "'DD-MM-YYYY HH24:MI:SS'") + " )", compareDate);
    } catch (RuntimeException ex) {
      throw new InternalErrorException(ex);
    }
  }
  ...
  public Task getTask(int serviceId, int facilityId) {
    try {
      ...
      return getMyJdbcTemplate().queryForObject(
          "select " + TASK_MAPPING_SELECT_QUERY + ", " + FacilitiesManagerImpl.FACILITY_MAPPING_SELECT_QUERY + ", " +
          ServicesManagerImpl.SERVICE_MAPPING_SELECT_QUERY + " from tasks " +
          " left join services on tasks.service_id = services.id" +
          " left join facilities on facilities.id = tasks.facility_id" +
          " where tasks.service_id = ? and tasks.facility_id = ?", TASK_ROWMAPPER, serviceId, facilityId);
  ...
  public Task getTaskById(int id) {
    try {
      ...
      return getMyJdbcTemplate().queryForObject(
          "select " + TASK_MAPPING_SELECT_QUERY + ", " + FacilitiesManagerImpl.FACILITY_MAPPING_SELECT_QUERY + ", " +
          ServicesManagerImpl.SERVICE_MAPPING_SELECT_QUERY +
          " from tasks left join services on tasks.service_id = services.id " +
          "left join facilities on facilities.id = tasks.facility_id where tasks.id = ?", TASK_ROWMAPPER, id);
    } catch (EmptyResultDataAccessException ex) {
      return null;
    }
  }
  ...
  public TaskResult getTaskResultById(int taskResultId) {
    ...
    return jdbc.queryForObject(
        "select " + TASK_RESULT_MAPPING_SELECT_QUERY + ", " +
            ServicesManagerImpl.DESTINATION_MAPPING_SELECT_QUERY + ", " +
        ServicesManagerImpl.SERVICE_MAPPING_SELECT_QUERY +
        ...
        " left join services on services.id = tasks.service_id" + " where tasks_results.id = ?", TASKRESULT_ROWMAPPER,
        taskResultId);
  }
  ...
  public List<TaskResult> getTaskResults() {
    ...
    return jdbc.query(
        ...
        " from tasks_results left join destinations on tasks_results.destination_id = destinations.id " +
        " left join tasks on tasks.id = tasks_results.task_id " +
        " left join services on services.id = tasks.service_id", TASKRESULT_ROWMAPPER);
  }
  ...
  public List<TaskResult> getTaskResultsByDestinations(List<String> destinationsNames) {
    ...
    try {
      return getNamedParameterJdbcTemplate().query(
          "select " + TASK_RESULT_MAPPING_SELECT_QUERY + ", " + ServicesManagerImpl.DESTINATION_MAPPING_SELECT_QUERY +
          ", " + ServicesManagerImpl.SERVICE_MAPPING_SELECT_QUERY +
          " from tasks_results left join destinations on tasks_results.destination_id = destinations.id" +
          " left join tasks on tasks.id = tasks_results.task_id " +
  ...
  public List<TaskResult> getTaskResultsByTask(int taskId) {
    ...
    return jdbc.query(
        ...
        " from tasks_results left join destinations on tasks_results.destination_id = destinations.id" +
        " left join tasks on tasks.id = tasks_results.task_id " +
        " left join services on services.id = tasks.service_id" + " where tasks_results.task_id = ? ",
  ...
  public List<TaskResult> getTaskResultsByTaskAndDestination(int taskId, int destinationId) {
    ...
    return jdbc.query(
        ...
        " from tasks_results left join destinations on tasks_results.destination_id = destinations.id" +
        " left join tasks on tasks.id = tasks_results.task_id " +
        " left join services on services.id = tasks.service_id" + " where tasks_results.task_id = ? AND" +
        " tasks_results.destination_id=?", TASKRESULT_ROWMAPPER, taskId, destinationId);
  }
  ...
  public List<TaskResult> getTaskResultsByTaskOnlyNewest(int taskId) {
    ...
    return jdbc.query(
        "select " + TASK_RESULT_MAPPING_SELECT_QUERY + ", " +
            ServicesManagerImpl.DESTINATION_MAPPING_SELECT_QUERY + ", " +
        ServicesManagerImpl.SERVICE_MAPPING_SELECT_QUERY + " from tasks_results" +
        " left join destinations on tasks_results.destination_id = destinations.id" +
        " left join tasks on tasks.id = tasks_results.task_id" +
        " left join services on services.id = tasks.service_id" + " inner join (" +
        " SELECT destination_id, MAX(modified_at) AS modified_at_max" + " FROM tasks_results where task_id=?" +
        " GROUP BY destination_id) tr2" + " on tasks_results.destination_id = tr2.destination_id" +
        " and tasks_results.modified_at = tr2.modified_at_max" + " inner join (" +
        " SELECT destination_id, modified_at, MAX(id) AS id_max" + " FROM tasks_results where task_id=?" +
        " GROUP BY destination_id, modified_at) tr3" + " on tasks_results.destination_id = tr3.destination_id" +
        " and tasks_results.modified_at = tr3.modified_at" + " and tasks_results.id   = tr3.id_max" +
        " where tasks_results.task_id=?", TASKRESULT_ROWMAPPER, taskId, taskId, taskId);
  }
  ...
  public int insertNewTaskResult(TaskResult taskResult) {
    ...
    // By java, such byte is displayed as 'empty string' and is not visible in aLOG.
  ...
  public int insertTask(Task task) {
    ...
    try {
      ...
      getMyJdbcTemplate().update(
          "insert into tasks(id, service_id, facility_id, schedule, recurrence, delay, status) values (?,?,?, " +
          Compatibility.toDate("?", "'DD-MM-YYYY HH24:MI:SS'") + ",?,?,?)", newTaskId, task.getServiceId(),
          task.getFacilityId(), task.getSchedule().format(getDateTimeFormatter()), task.getRecurrence(),
          task.getDelay(), task.getStatus().toString());
      LOG.debug("Added task with ID {}", newTaskId);
      return newTaskId;
    } catch (DataIntegrityViolationException ex) {
      LOG.error("Data: id, service_id, facility_id, schedule, recurrence, delay, status is: " + newTaskId + ", " +
                task.getServiceId() + ", " + task.getFacilityId() + ", " +
                task.getSchedule().format(getDateTimeFormatter()) + ", " + task.getRecurrence() + ", " +
                task.getDelay() + ", " + task.getStatus().toString() + ". Exception:" + ex.toString(), ex);
    } catch (Exception ex) {
  ...
  public boolean isThereSuchTask(Service service, Facility facility) {
    ...
    // jdbc template cannot be null
    List<Integer> tasks =
        getMyJdbcTemplate().queryForList("select id from tasks where service_id = ? and facility_id = ?",
            new Integer[] {service.getId(), facility.getId()}, Integer.class);
    if (tasks.size() == 0) {
      return false;
    } else if (tasks.size() > 1) {
      throw new IllegalArgumentException(
          "There is a duplicate Task for constraints Service[" + service.getId() + "], Facility[" + facility.getId() +
          "]");
    }
    return true;
  }
  ...
  public List<Task> listAllTasksForFacility(int facilityId) {
    try {
      ...
      return getMyJdbcTemplate().query(
          ...
          ServicesManagerImpl.SERVICE_MAPPING_SELECT_QUERY +
          " from tasks left join services on tasks.service_id = services.id " +
          "left join facilities on facilities.id = tasks.facility_id where facilities.id = ?", TASK_ROWMAPPER,
          facilityId);
    } catch (EmptyResultDataAccessException ex) {
      return null;
    }
  ...
  public List<Task> listAllTasksForService(int serviceId) {
    ...
    return getMyJdbcTemplate().query(
        ...
        ServicesManagerImpl.SERVICE_MAPPING_SELECT_QUERY +
        " from tasks left join services on tasks.service_id = services.id " +
        "left join facilities on facilities.id = tasks.facility_id where services.id = ?", TASK_ROWMAPPER, serviceId);
  }
  ...
  public List<Task> listAllTasksInState(Task.TaskStatus state) {
    ...
    return getMyJdbcTemplate().query(
        ...
        ServicesManagerImpl.SERVICE_MAPPING_SELECT_QUERY +
        " from tasks left join services on tasks.service_id = services.id " +
        "left join facilities on facilities.id = tasks.facility_id where tasks.status = ?", TASK_ROWMAPPER, textState);
  }
  ...
  public List<Task> listAllTasksNotInState(Task.TaskStatus state) {
    ...
    return getMyJdbcTemplate().query(
        ...
        ServicesManagerImpl.SERVICE_MAPPING_SELECT_QUERY +
        " from tasks left join services on tasks.service_id = services.id " +
        "left join facilities on facilities.id = tasks.facility_id where tasks.status != ? ", new Object[] {textState},
        TASK_ROWMAPPER);
  }
  ...
  public void updateTask(Task task) {
    ...
    // jdbc template cannot be null
    getMyJdbcTemplate().update("update tasks set service_id = ?, facility_id = ?, schedule = " +
                               Compatibility.toDate("?", "'DD-MM-YYYY HH24:MI:SS'") + ", recurrence = ?, delay = ?, " +
                               "status = ?, start_time = " + Compatibility.toDate("?", "'DD-MM-YYYY HH24:MI:SS'") +
                               ", end_time = " + Compatibility.toDate("?", "'DD-MM-YYYY HH24:MI:SS'") + " where id = ?",
        task.getServiceId(), task.getFacilityId(), scheduled, task.getRecurrence(), task.getDelay(),
