...
public class TasksManagerImpl implements TasksManagerImplApi {
  ...
  public final static String taskMappingSelectQuery =
  ...
  public int deleteOldTaskResults(int numDays) {
    try {
      ...
      return jdbc.update("delete from tasks_results where " +
              ...
              Compatibility.toDate("?", "'DD-MM-YYYY HH24:MI:SS'") + " )",
          compareDate);
    } catch (RuntimeException ex) {
      throw new InternalErrorException(ex);
    }
    ...
    try {
      ...
      getMyJdbcTemplate().update(
          "insert into tasks(id, service_id, facility_id, schedule, recurrence, delay, status) values (?,?,?, " +
              Compatibility.toDate("?", "'DD-MM-YYYY HH24:MI:SS'") + ",?,?,?)",
          newTaskId, task.getServiceId(), task.getFacilityId(), task.getSchedule().format(getDateTimeFormatter()),
          task.getRecurrence(), task.getDelay(), task.getStatus().toString());
      ...
      log.error("Data: id, service_id, facility_id, schedule, recurrence, delay, status is: " + newTaskId + ", " +
          ...
          + task.getSchedule().format(getDateTimeFormatter()) + ", " + task.getRecurrence() + ", " + task.getDelay() +
          ", " + task.getStatus().toString() + ". Exception:" + ex.toString(), ex);
    } catch (Exception ex) {
  ...
  public void updateTask(Task task) {
    ...
    // jdbc template cannot be null
    getMyJdbcTemplate().update(
        "update tasks set service_id = ?, facility_id = ?, schedule = " +
            Compatibility.toDate("?", "'DD-MM-YYYY HH24:MI:SS'") + ", recurrence = ?, delay = ?, "
            + "status = ?, start_time = " + Compatibility.toDate("?", "'DD-MM-YYYY HH24:MI:SS'") + ", end_time = " +
            Compatibility.toDate("?", "'DD-MM-YYYY HH24:MI:SS'") + " where id = ?", task.getServiceId(),
        task.getFacilityId(), scheduled, task.getRecurrence(), task.getDelay(), task.getStatus().toString(), startTime,
        endTime, task.getId());
  }
