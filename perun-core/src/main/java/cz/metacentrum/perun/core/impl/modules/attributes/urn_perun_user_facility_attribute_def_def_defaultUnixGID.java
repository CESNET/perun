 ...
 */
public class urn_perun_user_facility_attribute_def_def_defaultUnixGID extends UserFacilityAttributesModuleAbstract
  ...
  public void checkAttributeSemantics(PerunSessionImpl sess, User user, Facility facility, Attribute attribute)
    ...
    if (namespaceAttribute.getValue() == null) {
      throw new WrongReferenceAttributeValueException(attribute, namespaceAttribute, user, facility, facility, null,
    ...
    try {
      unixGroupNameNamespace = sess.getPerunBl().getAttributesManagerBl()
          .getAttribute(sess, facility, AttributesManager.NS_FACILITY_ATTR_DEF + ":unixGroupName-namespace");
    } catch (AttributeNotExistsException ex) {
      throw new ConsistencyErrorException(ex);
    }
    if (unixGroupNameNamespace.getValue() == null) {
      throw new WrongReferenceAttributeValueException(attribute, unixGroupNameNamespace, user, facility, facility, null,
          "Reference attribute is null");
    }
    ...
    List<Resource> allowedResourcesWithSameGid =
    ...
    try {
      groupGidAttribute = new Attribute(sess.getPerunBl().getAttributesManagerBl()
          .getAttributeDefinition(sess, AttributesManager.NS_GROUP_ATTR_DEF + ":unixGID-namespace:" + namespaceName));
    ...
    for (Group group : groupWithSameGid) {
      ...
      try {
        Attribute unixGroupName = sess.getPerunBl().getAttributesManagerBl().getAttribute(sess, group,
            AttributesManager.NS_GROUP_ATTR_DEF + ":unixGroupName-namespace:" + unixGroupNameNamespaceName);
