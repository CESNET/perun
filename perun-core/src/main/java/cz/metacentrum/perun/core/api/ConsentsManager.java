...
public interface ConsentsManager {
   ...
   * <p>
   * Corresponding consent hubs (containing the service) will have consents evaluated ONLY for selected service. Service
   * defines whether only active users will be evaluated or expired ones as well. If new consent is created, attributes
   * from ALL services under given consent hub are gathered for it.
   *
   * @param sess    session
   ...
   */
  ConsentHub getConsentHubByFacility(PerunSession sess, int facilityId)
   ...
   * @throws InternalErrorException
   */
  ConsentHub getConsentHubByName(PerunSession sess, String name)
   ...
   */
  ConsentHub getConsentHubByResource(PerunSession sess, int resourceId)
   ...
   * @throws ResourceNotExistsException
   */
  List<Consent> getConsentsForConsentHubByResource(PerunSession sess, int resourceId)
   ...
   */
  ConsentHub updateConsentHub(PerunSession perunSession, ConsentHub consentHub)
