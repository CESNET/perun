...
public class AuthzResolver {
  ...
  public static boolean isAuthorizedForAttribute(PerunSession sess, ActionType actionType, AttributeDefinition attrDef,
    ...
    if (bean instanceof Vo) {
      return isAuthorizedForAttribute(sess, actionType, attrDef, (Vo) bean);
    }
    if (bean instanceof User) {
      return isAuthorizedForAttribute(sess, actionType, attrDef, (User) bean);
    }
    if (bean instanceof Member) {
      return isAuthorizedForAttribute(sess, actionType, attrDef, (Member) bean);
    }
    if (bean instanceof Group) {
      return isAuthorizedForAttribute(sess, actionType, attrDef, (Group) bean);
    }
    if (bean instanceof Resource) {
      return isAuthorizedForAttribute(sess, actionType, attrDef, (Resource) bean);
    }
    if (bean instanceof Facility) {
      return isAuthorizedForAttribute(sess, actionType, attrDef, (Facility) bean);
    }
    if (bean instanceof Host) {
      return isAuthorizedForAttribute(sess, actionType, attrDef, (Host) bean);
    }
    if (bean instanceof UserExtSource) {
      return isAuthorizedForAttribute(sess, actionType, attrDef, (UserExtSource) bean);
    }
