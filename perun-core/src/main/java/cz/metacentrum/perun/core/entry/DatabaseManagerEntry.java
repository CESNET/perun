...
public class DatabaseManagerEntry implements DatabaseManager {
	...
	final public static String ACTUAL_DB_VERSION = "3.1.11";
	...
	public boolean isDBVersionUpToDate(PerunSession sess) throws InternalErrorException, PrivilegeException {
		Utils.checkPerunSession(sess);

		// Authorization
		if (!AuthzResolver.isAuthorized(sess, Role.PERUNADMIN)) throw new PrivilegeException(sess, "isDBVersionUpToDate");

		return getDatabaseManagerBl().isDBVersionUpToDate();
	}
