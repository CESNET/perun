...
public class ExtSourceREMS extends ExtSourceSqlComplex implements ExtSourceApi {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(ExtSourceREMS.class);

  /**
   * Finds out for given ues and login exits user in Perun. Format of ues is {extSourceName}|{extSourceClass}|{eppn}|0.
  ...
  private boolean existsSubjectWithUes(String ues) {
    ...
    if (extSourceSplit.length != 4) {
      LOG.error(
          "Ivalid format of additionalues_1. It should be '{extSourceName}|{extSourceClass}|{eppn}|0'. Actual: {}",
          ues);
      return false;
    }
  ...
  /**
   * Filters subjects that does not have a corresponding user in Perun by ues REMS or by additionalueses in format:
   * {extSourceName}|{extSourceClass}|{eppn}|0. The eppn is used as a 'login'.
   *
  ...
  private PerunSession getSession() {
    final PerunPrincipal pp = new PerunPrincipal("ExtSourceREMS", ExtSourcesManager.EXTSOURCE_NAME_INTERNAL,
  ...
  /**
   * Checks if the given subject has a corresponding user in Perun by ues REMS or by additionalueses in format:
   * {extSourceName}|{extSourceClass}|{eppn}|0. The eppn is used as a 'login'.
   *
   * @param subject subject
  ...
  private boolean isExistingUser(Map<String, String> subject) {
    ...
    // test if subject does not exist already with UserExtSourceREMS. If so, it means that subject already exists in
    // perun.
    List<User> usersFromREMS =
        perunBl.getUsersManagerBl().getUsersByExtSourceTypeAndLogin(sess, ExtSourcesManager.EXTSOURCE_REMS, login);
