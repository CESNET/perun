...
public interface GroupsManagerImplApi {
  ...
  /**
   * Adds group from list of groups which can be registered into during vo or group registration and are representing
   * options of the specified application form item.
   *
   * @param sess  session
   * @param group group to add
   */
  ...
  /**
   * Deletes group from list of groups which can be registered into during vo or group registration and are representing
   * options of the specified application form item.
   *
   * @param sess  session
   * @param group group to delete
   ...
   * @param excludeStatusInsteadOfIncludeStatus does the list of statuses means exclude members or include members with
   *                                            these statuses
   * @return list of members
   * @throws InternalErrorException
   */
  List<Member> getGroupMembers(PerunSession sess, Group group, List<Status> statuses,
  ...
  /**
   * Get group members by member ID -> meaning we will get all (DIRECT/INDIRECT) group memberships for specified member
   * (or user, since it will be the same).
   *
   * @param sess
   * @param group
   * @param memberId
  ...
  /**
   * Checks if relation exists between result group and operand group. It matters which one is result group and which
   * one is operand group.
   *
  ...
  /**
   * Checks if relation between groups exists. It checks both ways. Does not matter which one is result group and which
   * one is operand group.
   *
   * @param group1 group
   * @param group2 group
  ...
  /**
   * Check if the relation between given groups can be deleted. Determined by parent flag (it flags relations created by
   * hierarchical structure). It matters which group is resultGroup and which is operandGroup!!!
   *
