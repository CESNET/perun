...
public class urn_perun_user_facility_attribute_def_def_accountExpirationTime
    extends UserFacilityAttributesModuleAbstract implements UserFacilityAttributesModuleImplApi {

  private static final String A_F_D_accountExpirationTime =
      AttributesManager.NS_FACILITY_ATTR_DEF + ":accountExpirationTime";
  ...
  public void checkAttributeSemantics(PerunSessionImpl perunSession, User user, Facility facility, Attribute attribute)
    ...
    try {
      facilityAccExpAttribute = perunSession.getPerunBl().getAttributesManagerBl()
          .getAttribute(perunSession, facility, A_F_D_accountExpirationTime);
  ...
  @Override
  public Attribute fillAttribute(PerunSessionImpl perunSession, User user, Facility facility,
    ...
    for (Resource r : perunSession.getPerunBl().getUsersManagerBl().getAllowedResources(perunSession, facility, user)) {
      try { //getting all resources at which user has access
        resourceExpTime = (Integer) perunSession.getPerunBl().getAttributesManagerBl()
            .getAttribute(perunSession, r, attribute.getName()).getValue();
    ...
    Integer facilityAccExpTime;
    try {
      facilityAccExpTime = (Integer) perunSession.getPerunBl().getAttributesManagerBl()
          .getAttribute(perunSession, facility, attribute.getName()).getValue();
    } catch (AttributeNotExistsException ex) {
      throw new InternalErrorException(ex);
    }
