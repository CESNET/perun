 ...
 */
public class urn_perun_facility_attribute_def_def_pbsServer extends FacilityAttributesModuleAbstract
  ...
  public void checkAttributeSemantics(PerunSessionImpl perunSession, Facility facility, Attribute attribute)
    ...
    if (pbsServer == null) {
      throw new WrongReferenceAttributeValueException(attribute, "PbsServer cannot be null.");
    }
    ...
    try {
      Attribute name = new Attribute(perunSession.getPerunBl().getAttributesManagerBl()
          .getAttributeDefinition(perunSession, AttributesManager.NS_FACILITY_ATTR_CORE + ":name"));
      name.setValue(pbsServer);
      if (!success) {
        throw new WrongReferenceAttributeValueException(attribute, name, facility, null,
            "There is no facility with attribute name same as the pbsServer attribute.");
      }
