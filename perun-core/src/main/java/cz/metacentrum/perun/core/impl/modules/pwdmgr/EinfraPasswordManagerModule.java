...
public class EinfraPasswordManagerModule extends GenericPasswordManagerModule {
  ...
  public void checkLoginFormat(PerunSession sess, String login) throws InvalidLoginException {
    ...
    ((PerunBl) sess.getPerun()).getModulesUtilsBl()
        .checkLoginNamespaceRegex(actualLoginNamespace, login, einfraLoginPattern);
    ...
    // TODO - we will probably want to split regex checks on multiple conditions
    //  and let user know about all problems at once
  ...
  public void checkPasswordStrength(PerunSession sess, String login, String password) throws PasswordStrengthException {
    ...
    if (password.length() < einfraPasswordMinLength) {
      ...
      throw new PasswordStrengthException(
          "Password for " + actualLoginNamespace + ":" + login + " is too short. At least " + einfraPasswordMinLength +
    ...
    int groupsCounter = 0;
    if (einfraPasswordContainsDigit.matcher(password).matches()) {
      groupsCounter++;
    }
    if (einfraPasswordContainsUpper.matcher(password).matches()) {
      groupsCounter++;
    }
    if (einfraPasswordContainsLower.matcher(password).matches()) {
      groupsCounter++;
    }
    if (einfraPasswordContainsSpec.matcher(password).matches()) {
      groupsCounter++;
    }
    ...
    if (groupsCounter < 3) {
      ...
      throw new PasswordStrengthException("Password for " + actualLoginNamespace + ":" + login +
                                              ...
                                              " lower-case letter, upper-case letter, digit, spec. character.");
    }
  ...
  private String getEinfraLogin(PerunSession sess, User user) {
    ...
    String login = null;
    try {
      Attribute attribute = ((PerunBl) sess.getPerun()).getAttributesManagerBl().getAttribute(sess, user,
      ...
      if (attribute.getValue() == null) {
        ...
        throw new LoginNotExistsRuntimeException(
            user + " doesn't have login in namespace 'einfra', so the alternative password can't be set.");
      }
      ...
      throw new InternalErrorException(
          "We couldn't retrieve 'einfra' login for the " + user + " to create/delete alternative password.");
    }
    return login;
  ...
  private String getMail(PerunSession sess, User user) {
    ...
    try {
      Attribute attribute = ((PerunBl) sess.getPerun()).getAttributesManagerBl()
      ...
      throw new InternalErrorException(
          "We couldn't retrieve mail for the " + user + " to create/delete alternative password.");
    }
  ...
  @Override
  public String handleSponsorship(PerunSession sess, SponsoredUserData userData)
   ...
   * <p>
   * Login is verified in the given priority: 1) If the login is exceptionally allowed, returns true. 2) If the login
   * matches some of the hardcoded prefixes, returns false. 3) Checks the specified login format in the instance
   * configuration.
   *
   * @param sess  session
  ...
  public void validatePassword(PerunSession sess, String userLogin, User user) throws InvalidLoginException {
    ...
    if (user == null) {
      ...
      try {
        ...
        Attribute kerberosLoginsAttr = perunBl.getAttributesManagerBl().getAttribute(sess, user,
