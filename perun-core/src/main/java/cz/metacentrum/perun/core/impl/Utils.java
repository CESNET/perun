...
public class Utils {
  ...
  private static final String A_E_IDENTITY_ALERTS =
      AttributesManager.NS_ENTITYLESS_ATTR_DEF + ":identityAlertsTemplates";
  ...
  private static UserExtSource parseAdditionalUserExtSource(PerunSession sess, String[] userExtSourceRaw) {
    ...
    try {
      ...
      additionalExtSource =
          ((PerunBl) sess.getPerun()).getExtSourcesManagerBl().getExtSourceByName(sess, userExtSourceRaw[0]);
    } catch (ExtSourceNotExistsException e) {
      try {
        // Create new one if not exists
        additionalExtSource = new ExtSource(userExtSourceRaw[0], userExtSourceRaw[1]);
        additionalExtSource =
            ((PerunBl) sess.getPerun()).getExtSourcesManagerBl().createExtSource(sess, additionalExtSource, null);
   ...
   */
  private static List<Attribute> parseUESAttributes(PerunSession sess, String login,
                                                    String additionalExtLoginAndAttribute) {
    ...
    if (loginAndAttributes.length > 1) {
      ...
      for (String uesAttributeWithValue : uesAttributesWithValues) {
        ...
        try {
          ...
          Attribute attribute =
              new Attribute(sess.getPerun().getAttributesManager().getAttributeDefinition(sess, uesAttribute[0]),
  ...
  public static void checkWithoutDiacritic(String name) {
    ...
    if (!Normalizer.isNormalized(name, Form.NFKD)) {
      throw new DiacriticNotAllowedException(
          "Name of the entity is not in the normalized form NFKD (diacritic not allowed)!");
    }
  ...
  public static void checkWithoutSpecialChars(String name) {
    ...
    if (!name.matches("^[0-9 \\p{L}]*$")) {
      throw new SpecialCharsNotAllowedException("The special chars in the name of entity are not allowed!");
    }
  }
  ...
  public static void checkWithoutSpecialChars(String name, String allowedSpecialChars) {
    ...
    if (!name.matches("^([0-9 \\p{L}" + allowedSpecialChars + "])*$")) {
      throw new SpecialCharsNotAllowedException(
          "The special chars (except " + allowedSpecialChars + ") in the name of entity are not allowed!");
    }
  }
  ...
  public static void checkWithoutNumbers(String name) {
    ...
    if (!name.matches("^([^0-9])*$")) {
      throw new NumbersNotAllowedException("The numbers in the name of entity are not allowed!");
    }
  }
  ...
  public static void checkWithoutSpaces(String name) {
    ...
    if (name.contains(" ")) {
      throw new SpaceNotAllowedException("The spaces in the name of entity are not allowed!");
    }
  }
  ...
  public static void checkRangeOfNumbers(int number, int lowestValue, int highestValue) {
    ...
    if (number < lowestValue || number > highestValue) {
      throw new NumberNotInRangeException(
          "Number is not in range, Lowest=" + lowestValue + " < Number=" + number + " < Highest=" + highestValue);
    }
  }
  ...
  public static List<String> generateStringsByPattern(String pattern) throws WrongPatternException {
    ...
    for (String value : values) {
      if (value.contains("]") || (value.contains("["))) {
        throw new WrongPatternException(
            "The pattern \"" + pattern + "\" has a wrong syntax. Too much closing brackets.");
      }
    }
    ...
    for (String generator : generators) {
      if (generator.contains("]") || (generator.contains("["))) {
        throw new WrongPatternException(
            "The pattern \"" + pattern + "\" has a wrong syntax. Too much opening brackets.");
      }
    }
    ...
    for (String range : generators) {
      ...
      if (m.find()) {
        ...
        try {
          ...
          throw new WrongPatternException(
              "The pattern \"" + pattern + "\" has a wrong syntax. Wrong format of the range.");
        }

        // if end is before start -> abort
        if (startNumber > endNumber) {
          throw new WrongPatternException(
              "The pattern \"" + pattern + "\" has a wrong syntax. Start number has to be lower than end number.");
        ...
        throw new WrongPatternException(
            "The pattern \"" + pattern + "\" has a wrong syntax. The format numer-number not found.");
      }
    }
   ...
   */
  public static void sendValidationEmail(User user, String url, String email, UUID changeUuid, String subject,
                                         String content, String customUrlPath, String idp) {
   ...
   */
  private static String prepareSubjectOfEmail(String optionalSubject, String defaultSubject,
   ...
   */
  private static String prepareBodyOfEmail(String optionalBody, String defaultBody,
                                           Map<String, String> parametersToReplace) {
  ...
  public static String cipherInput(String plainText, boolean decrypt) {
    ...
    try {
      ...
      c.init((decrypt) ? Cipher.DECRYPT_MODE : Cipher.ENCRYPT_MODE, k,
  ...
  public static String bigDecimalBytesToReadableStringWithMetric(BigDecimal quota) {
    if (quota == null) {
      throw new InternalErrorException(
          "Quota in BigDecimal can't be null if we want to convert it to number with metric.");
    }
    ...
    if (!quota.divide(BigDecimal.valueOf(ModulesUtilsBlImpl.E)).stripTrailingZeros().toPlainString().contains(".")) {
      //divide by 1024^5
      stringWithMetric =
          quota.divide(BigDecimal.valueOf(ModulesUtilsBlImpl.E)).stripTrailingZeros().toPlainString() + "E";
    } else if (!quota.divide(BigDecimal.valueOf(ModulesUtilsBlImpl.P)).stripTrailingZeros().toPlainString()
        .contains(".")) {
      //divide by 1024^4
      stringWithMetric =
          quota.divide(BigDecimal.valueOf(ModulesUtilsBlImpl.P)).stripTrailingZeros().toPlainString() + "P";
    } else if (!quota.divide(BigDecimal.valueOf(ModulesUtilsBlImpl.T)).stripTrailingZeros().toPlainString()
        .contains(".")) {
      //divide by 1024^3
      stringWithMetric =
          quota.divide(BigDecimal.valueOf(ModulesUtilsBlImpl.T)).stripTrailingZeros().toPlainString() + "T";
    } else if (!quota.divide(BigDecimal.valueOf(ModulesUtilsBlImpl.G)).stripTrailingZeros().toPlainString()
        .contains(".")) {
      //divide by 1024^2
      stringWithMetric =
          quota.divide(BigDecimal.valueOf(ModulesUtilsBlImpl.G)).stripTrailingZeros().toPlainString() + "G";
    } else if (!quota.divide(BigDecimal.valueOf(ModulesUtilsBlImpl.M)).stripTrailingZeros().toPlainString()
        .contains(".")) {
      //divide by 1024^1
      stringWithMetric =
  ...
  public static synchronized String toASCII(String input, Character replacement) {
    ...
    for (int i = 0; i < input.length(); i++) {
      ...
      String normalizedChar =
          Normalizer.normalize(String.valueOf(c), Normalizer.Form.NFD).replaceAll("[^\\p{ASCII}]", "");
  ...
  public static LocalDate getClosestExpirationFromStaticDate(Matcher matcher) {
    ...
    boolean extensionInThisYear =
        LocalDate.of(2000, LocalDate.now().getMonth(), LocalDate.now().getDayOfMonth()).isBefore(extensionDate);
  ...
  public static String escapeStringForLDAP(String searchString) {
    ...
    return searchString.replace("\\", "\\5C").replace("*", "\\2A").replace("(", "\\28").replace(")", "\\29")
        .replace("\000", "\\00");
  }
   ...
   * @return search query
   */
  public static String prepareSqlWhereForUserMemberSearch(String searchString, MapSqlParameterSource namedParams,
                                                          boolean exactMatch) {
   ...
   * @return search query
   */
  public static String prepareSqlWhereForGroupSearch(String searchString, MapSqlParameterSource namedParams,
                                                     boolean subgroups) {
    if (isEmpty(searchString)) {
      return "";
    }
    ...
    String groupNameDscQueryString =
   ...
   */
  public static Map<String, Pair<String, String>> getAttributesQuery(List<String> memberAttributes,
                                                                     List<String> userAttributes,
                                                                     List<String> uesAttributes) {
  ...
  private static String insertOrganization(PerunSession sess, String message, UserExtSource newUes) {
    ...
    try {
      Attribute orgAttr =
          ((PerunBl) sess.getPerun()).getAttributesManagerBl().getAttribute(sess, newUes, A_UES_ORGANIZATION);
  ...
  private static String getAlertTemplate(PerunSession sess, String templateName, String defaultValue) {
    ...
    try {
      Attribute attr =
          ((PerunBl) sess.getPerun()).getAttributesManagerBl().getAttribute(sess, KEY_EN, A_E_IDENTITY_ALERTS);
