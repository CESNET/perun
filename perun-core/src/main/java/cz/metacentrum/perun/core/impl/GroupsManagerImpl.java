...
public class GroupsManagerImpl implements GroupsManagerImplApi {
  ...
  protected final static String groupMappingSelectQuery =
      "groups.id as groups_id, groups.uu_id as groups_uu_id, groups.parent_group_id as groups_parent_group_id, groups.name as groups_name, groups.dsc as groups_dsc, "
          +
  ...
  protected final static String memberGroupsMappingSelectQuery =
      groupMappingSelectQuery + ", groups_members.member_id as group_member_id, "
          +
  ...
  protected final static String assignedGroupMappingSelectQuery =
      groupMappingSelectQuery + ", groups_resources_state.status as groups_resources_state_status, " +
  ...
  private static final String applicationFormMappingSelectQuery =
      "id,vo_id,group_id,automatic_approval,automatic_approval_extension,automatic_approval_embedded,module_names from application_form";
  ...
  public Group createGroup(PerunSession sess, Vo vo, Group group) throws GroupExistsException {
    ...
    if (group.getParentGroupId() == null) {
      ...
      if (1 ==
          jdbc.queryForInt("select count('x') from groups where lower(name)=lower(?) and vo_id=? and parent_group_id=?",
    ...
    try {
      ...
      jdbc.update(
          ...
          group.getName(), group.getDescription(), vo.getId(), sess.getPerunPrincipal().getActor(),
  ...
  public Group getGroupById(PerunSession sess, int id) throws GroupNotExistsException {
    try {
      return jdbc.queryForObject("select " + groupMappingSelectQuery + " from groups where groups.id=? ", GROUP_MAPPER,
          id);
  ...
  public List<Group> getUserGroups(PerunSession sess, User user) {
    try {
      return jdbc.query(
          "select " + groupMappingSelectQuery + "from groups join groups_members on groups.id=groups_members.group_id" +
  ...
  public List<Group> getUserGroups(PerunSession sess, User user, List<Status> memberStatuses,
    ...
    try {
      ...
      return namedParameterJdbcTemplate.query("select " + groupMappingSelectQuery + "from groups" +
              ...
              " join members on members.id=groups_members.member_id where members.user_id=:uid and members.status in (:voStatuses)",
    ...
    try {
      ...
      if (excludeStatus) {
        ...
        return this.namedParameterJdbcTemplate.query("select " + MembersManagerImpl.groupsMembersMappingSelectQuery +
                " from groups_members join members on members.id=groups_members.member_id " +
                "where groups_members.group_id=:group_id and members.status not in (:statuses)", parameters,
            MembersManagerImpl.MEMBER_MAPPER_WITH_GROUP);
      } else {
        // Include members with one of the status
        return this.namedParameterJdbcTemplate.query("select " + MembersManagerImpl.groupsMembersMappingSelectQuery +
                " from groups_members join members on members.id=groups_members.member_id " +
                "where groups_members.group_id=:group_id and members.status in (:statuses)", parameters,
            MembersManagerImpl.MEMBER_MAPPER_WITH_GROUP);
      }
    } catch (EmptyResultDataAccessException e) {
      return new ArrayList<>();
  ...
  public List<Member> getServiceGroupMembers(PerunSession sess, Group group) {
    try {
      return jdbc.query(
          "SELECT DISTINCT " + MembersManagerImpl.memberMappingSelectQuery + ", " +
              MembersManagerImpl.groupsMembersMappingSelectQuery +
  ...
  public Paginated<Group> getGroupsPage(PerunSession sess, Vo vo, GroupsPageQuery query) {
    ...
    authorizedGroupsIds.removeIf(groupId ->
        !AuthzResolverBlImpl.isAuthorizedForGroup(sess, "filter-getGroupsPage_Vo_GroupsPageQuery_List<String>_policy",
            groupId, vo.getId()));
  ...
  public Group getGroupByName(PerunSession sess, Vo vo, String name) throws GroupNotExistsException {
    try {
      return jdbc.queryForObject(
          "select " + groupMappingSelectQuery + " from groups where groups.name=? and groups.vo_id=?",
  ...
  public List<Group> getGroupsByIds(PerunSession perunSession, List<Integer> ids) {
    try {
      return jdbc.execute(
          "select " + groupMappingSelectQuery + " from groups where id " + Compatibility.getStructureForInClause(),
  ...
  public Member addMember(PerunSession sess, Group group, Member member, MembershipType type, int sourceGroupId)
    ...
    try {
      jdbc.update(
          ...
          group.getId(),
          member.getId(), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getActor(),
  ...
  public List<Group> getAllMemberGroups(PerunSession sess, Member member) {
    try {
      return jdbc.query("select distinct " + groupMappingSelectQuery +
    ...
    try {
      ...
      for (Group authorizedGroup : listOfGroupAdmins) {
        setOfAdmins.addAll(jdbc.query("select " + UsersManagerImpl.userMappingSelectQuery +
                ...
                "join groups_members on groups_members.member_id=members.id where groups_members.group_id=? and groups_members.source_group_status=? and members.status=?",
  ...
  public List<Group> getGroupAdmins(PerunSession sess, Group group) {
    try {
      return jdbc.query(
          "select " + groupMappingSelectQuery + " from authz join groups on authz.authorized_group_id=groups.id " +
              "where authz.group_id=? and authz.role_id=(select id from roles where name='groupadmin')",
  ...
  public List<Group> getGroupsStructuresToSynchronize(PerunSession sess) {
    try {
      ...
      return jdbc.query("select " + groupMappingSelectQuery + " from groups, attr_names, group_attr_values " +
              "where attr_names.attr_name=? and attr_names.id=group_attr_values.attr_id and group_attr_values.attr_value='true' and " +
              "group_attr_values.group_id=groups.id", GROUP_MAPPER,
  ...
  public void saveGroupRelation(PerunSession sess, Group resultGroup, Group operandGroup, boolean parentFlag) {
    try {
      jdbc.update("INSERT INTO groups_groups(result_gid, operand_gid, created_at, created_by, " +
          ...
          resultGroup.getId(), operandGroup.getId(), sess.getPerunPrincipal().getActor(),
          sess.getPerunPrincipal().getActor(), parentFlag);
    } catch (RuntimeException e) {
      throw new InternalErrorException(e);
    }
  }
  ...
  public boolean isRelationRemovable(PerunSession sess, Group resultGroup, Group operandGroup) {
    try {
      return 1 > jdbc.queryForInt("SELECT parent_flag" + Compatibility.castToInteger() +
              " FROM groups_groups WHERE result_gid=? AND operand_gid=?",
          resultGroup.getId(), operandGroup.getId());
    } catch (RuntimeException e) {
      throw new InternalErrorException(e);
    }
  ...
  public boolean isRelationBetweenGroups(Group group1, Group group2) {
    try {
      return 1 <= jdbc.queryForInt(
          "SELECT count(1) FROM groups_groups WHERE (result_gid = ? AND operand_gid = ?) OR (result_gid = ? AND operand_gid = ?)",
    ...
    try {
      ...
      for (List<Integer> partMemberIds : Lists.partition(memberIds, 10000)) {
        ...
        Map<Integer, List<Integer>> partMap =
            namedParameterJdbcTemplate.query("select member_id, source_group_status FROM groups_members" +
                    " join members on groups_members.member_id=members.id where group_id=(:groupId) and member_id in (:partMemberIds)",
                parameters, MEMBERID_MEMBERGROUPSTATUS_EXTRACTOR);
  ...
  public List<Facility> getFacilitiesWhereGroupIsAdmin(PerunSession session, Group group) {
    try {
      return jdbc.query("select " + FacilitiesManagerImpl.facilityMappingSelectQuery +
              ...
              "where authorized_group_id=? and authz.role_id=(select id from roles where name='facilityadmin')",
  ...
  public List<Group> getGroupsWhereGroupIsAdmin(PerunSession session, Group group) {
    try {
      return jdbc.query("select " + GroupsManagerImpl.groupMappingSelectQuery +
              ...
              "where authorized_group_id=? and authz.role_id=(select id from roles where name='groupadmin')",
  ...
  public List<Group> getGroupsForAutoRegistration(PerunSession sess, Vo vo) {
    try {
      return jdbc.query("select " + groupMappingSelectQuery +
          " from groups where vo_id=? and id IN (select group_id from groups_to_register)", GROUP_MAPPER, vo.getId());
    } catch (RuntimeException ex) {
      throw new InternalErrorException(ex);
    }
  }
  ...
  public List<Group> getAllGroupsForAutoRegistration(PerunSession sess) {
    try {
      return jdbc.query(
          "select " + groupMappingSelectQuery + " from groups where id IN (select group_id from groups_to_register)",
          GROUP_MAPPER);
    } catch (RuntimeException ex) {
      throw new InternalErrorException(ex);
    }
  ...
  public List<Group> getGroupsForAutoRegistration(PerunSession sess, Vo vo, ApplicationFormItem formItem) {
    try {
      return jdbc.query("select " + groupMappingSelectQuery +
              " from groups where vo_id=? and id IN (select group_id from auto_registration_groups where application_form_item_id = ?)",
          GROUP_MAPPER, vo.getId(), formItem.getId());
    } catch (RuntimeException ex) {
      throw new InternalErrorException(ex);
    }
  }
  ...
  public List<Group> getGroupsForAutoRegistration(PerunSession sess, Group group, ApplicationFormItem formItem) {
    try {
      return jdbc.query("select " + groupMappingSelectQuery +
              " from groups where vo_id=? and id IN (select group_id from auto_registration_groups where application_form_item_id = ?)",
          GROUP_MAPPER, group.getVoId(), formItem.getId());
    } catch (RuntimeException ex) {
      throw new InternalErrorException(ex);
    }
  }
  ...
  public ApplicationForm getParentApplicationFormForAutoRegistrationGroup(Group group) {
    try {
      int appFormItemId =
          jdbc.queryForInt("SELECT application_form_item_id FROM auto_registration_groups WHERE group_id = ? LIMIT 1",
              group.getId());
  ...
  public List<Group> getAllAllowedGroupsToHierarchicalVo(PerunSession sess, Vo vo, Vo memberVo) {
    try {
      return jdbc.query("SELECT " + groupMappingSelectQuery + " FROM groups WHERE vo_id=? AND id IN " +
              "(SELECT group_id FROM allowed_groups_to_hierarchical_vo WHERE vo_id=?)", GROUP_MAPPER, memberVo.getId(),
          vo.getId());
    } catch (RuntimeException e) {
      throw new InternalErrorException(e);
    }
