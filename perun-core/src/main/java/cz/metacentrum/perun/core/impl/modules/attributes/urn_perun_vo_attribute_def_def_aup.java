 ...
 */
public class urn_perun_vo_attribute_def_def_aup extends VoAttributesModuleAbstract
    ...
    try {
      ...
      for (int i = 0; i < array.length(); i++) {
        ...
        if (!object.has("version")) {
          throw new WrongAttributeValueException(attribute, "AUP is missing key version in JSON.");
        }
        if (!object.has("date")) {
          throw new WrongAttributeValueException(attribute, "AUP is missing key date in JSON.");
        }
        if (!object.has("link")) {
          throw new WrongAttributeValueException(attribute, "AUP is missing key link in JSON.");
        }
        if (!object.has("text")) {
          throw new WrongAttributeValueException(attribute, "AUP is missing key text in JSON.");
        }
      }
