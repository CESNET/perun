...
public class SecurityTeamsManagerEntry implements cz.metacentrum.perun.core.api.SecurityTeamsManager {
	...
	public void addUserToBlacklist(PerunSession sess, SecurityTeam securityTeam, User user, String description) throws PrivilegeException, SecurityTeamNotExistsException, UserNotExistsException, UserAlreadyBlacklistedException {
		...
		//Authorization
		if (!AuthzResolver.authorizedInternal(sess, "addUserToBlacklist_SecurityTeam_User_String_policy", Arrays.asList(securityTeam, user))) {
	...
	@Override
	public void removeUserFromBlacklist(PerunSession sess, SecurityTeam securityTeam, User user) throws PrivilegeException, SecurityTeamNotExistsException, UserNotExistsException {
		Utils.checkPerunSession(sess);
		getSecurityTeamsManagerBl().checkSecurityTeamExists(sess, securityTeam);
		getPerunBl().getUsersManagerBl().checkUserExists(sess, user);
		if (!getSecurityTeamsManagerBl().checkUserIsInBlacklist(sess, securityTeam, user)) {
			return;
		}

		//Authorization
		if (!AuthzResolver.authorizedInternal(sess, "removeUserFromBlacklist_SecurityTeam_User_policy", Arrays.asList(securityTeam, user))) {
