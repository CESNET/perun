...
public class FacilitiesManagerImpl implements FacilitiesManagerImplApi {
  ...
  public final static String facilityMappingSelectQuery =
  ...
  protected final static String banOnFacilityMappingSelectQuery =
  ...
  public Facility createFacility(PerunSession sess, Facility facility) {
    ...
    try {
      ...
      jdbc.update(
          "insert into facilities(id, name, dsc, created_by, created_at, modified_by, modified_at, created_by_uid, modified_by_uid) " +
  ...
  public Facility getFacilityById(PerunSession sess, int id) throws FacilityNotExistsException {
    try {
      return jdbc.queryForObject("select " + facilityMappingSelectQuery + " from facilities where id=?",
          FACILITY_MAPPER, id);
  ...
  public Facility getFacilityByName(PerunSession sess, String name) throws FacilityNotExistsException {
    try {
      return jdbc.queryForObject("select " + facilityMappingSelectQuery + " from facilities where name=?",
  ...
  public List<Facility> getFacilitiesByIds(PerunSession perunSession, List<Integer> ids) {
    try {
      return jdbc.execute("select " + facilityMappingSelectQuery + " from facilities where id " +
  ...
  public List<Facility> getFacilitiesByDestination(PerunSession sess, String destination)
    ...
    try {
      return jdbc.query("select distinct " + facilityMappingSelectQuery +
  ...
  public List<Owner> getOwners(PerunSession sess, Facility facility) {
    try {
      return jdbc.query("select " + OwnersManagerImpl.ownerMappingSelectQuery +
              " from owners left join facility_owners on owners.id = facility_owners.owner_id where facility_id=?",
  ...
  public void setOwners(PerunSession sess, Facility facility, List<Owner> owners) {
    try {
      ...
      for (Owner owner : owners) {
        jdbc.update(
            "insert into facility_owners(facility_id, owner_id,created_by,created_at,modified_by,modified_at,created_by_uid,modified_by_uid) " +
                "values (?,?,?," + Compatibility.getSysdate() + ",?," + Compatibility.getSysdate() + ",?,?)",
            facility.getId(), owner.getId(),
  ...
  public List<Resource> getAssignedResources(PerunSession sess, Facility facility) {
    try {
      return jdbc.query(
          "select " + ResourcesManagerImpl.resourceMappingSelectQuery + " from resources where facility_id=?",
  ...
  public List<Resource> getAssignedResources(PerunSession sess, Facility facility, Vo specificVo,
    ...
    try {
      ...
      if (specificVo != null && specificService != null) {
        ...
        return jdbc.query("select " + ResourcesManagerImpl.resourceMappingSelectQuery +
                " from resources where facility_id=? and vo_id=?",
  ...
  public List<RichResource> getAssignedRichResources(PerunSession sess, Facility facility) {
    try {
      return jdbc.query("select distinct " + ResourcesManagerImpl.resourceMappingSelectQuery + ", " +
              VosManagerImpl.voMappingSelectQuery + ", " +
  ...
  public List<Facility> getFacilitiesByAttributePartialMatch(PerunSession sess, AttributeDefinition attrDef,
    ...
    try {
      return jdbc.query("select " + facilityMappingSelectQuery + " from facilities " +
              ...
              "and lower(" + Compatibility.convertToAscii("facility_attr_values.attr_value") +
  ...
  public Host addHost(PerunSession sess, Host host, Facility facility) {
    ...
    try {
      ...
      jdbc.update(
          "insert into hosts (id, hostname, facility_id, created_by, created_at, modified_by,modified_at,created_by_uid,modified_by_uid) " +
              "values (?,?,?,?," + Compatibility.getSysdate() + ",?," + Compatibility.getSysdate() + ",?,?)",
          newId, host.getHostname(), facility.getId(), sess.getPerunPrincipal().getActor(),
          sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getUserId(),
          sess.getPerunPrincipal().getUserId());
  ...
  public List<Host> getHostsByHostname(PerunSession sess, String hostname) {
    try {
      return jdbc.query("select " + hostMappingSelectQuery + " from hosts where hosts.hostname=? order by id",
  ...
  public Facility getFacilityForHost(PerunSession sess, Host host) {
    try {
      return jdbc.queryForObject(
          "select " + facilityMappingSelectQuery + " from facilities join hosts on hosts.facility_id=facilities.id " +
  ...
  public List<Host> getHosts(PerunSession sess, Facility facility) {
    try {
      return jdbc.query("select " + hostMappingSelectQuery + " from hosts where hosts.facility_id=? order by id",
  ...
  public List<Facility> getFacilitiesWhereUserIsAdmin(PerunSession sess, User user) {
    try {
      return jdbc.query("select " + facilityMappingSelectQuery + " from facilities " +
          ...
          FACILITY_MAPPER, MemberGroupStatus.VALID.getCode(), user.getId(), user.getId(),
          Role.FACILITYADMIN.toLowerCase(), Status.VALID.getCode());
    } catch (RuntimeException e) {
      throw new InternalErrorException(e);
    }
  }
  ...
  public void assignSecurityTeam(PerunSession sess, Facility facility, SecurityTeam securityTeam) {
    try {
      jdbc.update(
          "insert into security_teams_facilities(security_team_id, facility_id, created_by, created_at, modified_by, modified_at, created_by_uid, modified_by_uid) " +
              "values (?,?,?," + Compatibility.getSysdate() + ",?," + Compatibility.getSysdate() + ",?,?)",
          securityTeam.getId(), facility.getId(), sess.getPerunPrincipal().getActor(),
          sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getUserId(),
          sess.getPerunPrincipal().getUserId());
    } catch (RuntimeException e) {
      throw new InternalErrorException(e);
  ...
  public BanOnFacility setBan(PerunSession sess, BanOnFacility banOnFacility) {
    ...
    try {
      ...
      jdbc.update(
          "insert into facilities_bans(id, description, banned_to, user_id, facility_id, created_by, created_at,modified_by,modified_at,created_by_uid,modified_by_uid) " +
              "values (?,?," + (banOnFacility.getValidityTo() != null ?
              "'" + Compatibility.getDate(banOnFacility.getValidityTo().getTime()) + "'" : "DEFAULT") + ",?,?,?," +
              Compatibility.getSysdate() + ",?," + Compatibility.getSysdate() + ",?,?)",
          newId, banOnFacility.getDescription(), banOnFacility.getUserId(), banOnFacility.getFacilityId(),
  ...
  public BanOnFacility getBanById(PerunSession sess, int banId) throws BanNotExistsException {
    try {
      return jdbc.queryForObject("select " + banOnFacilityMappingSelectQuery + " from facilities_bans where id=? ",
  ...
  public BanOnFacility getBan(PerunSession sess, int userId, int faclityId) throws BanNotExistsException {
    try {
      return jdbc.queryForObject(
          "select " + banOnFacilityMappingSelectQuery + " from facilities_bans where user_id=? and facility_id=?",
  ...
  public List<BanOnFacility> getBansForUser(PerunSession sess, int userId) {
    try {
      return jdbc.query("select " + banOnFacilityMappingSelectQuery + " from facilities_bans where user_id=?",
  ...
  public List<BanOnFacility> getBansForFacility(PerunSession sess, int facilityId) {
    try {
      return jdbc.query("select " + banOnFacilityMappingSelectQuery + " from facilities_bans where facility_id=?",
  ...
  public List<BanOnFacility> getAllExpiredBansOnFacilities(PerunSession sess) {
    try {
      return jdbc.query("select " + banOnFacilityMappingSelectQuery + " from facilities_bans where banned_to < " +
