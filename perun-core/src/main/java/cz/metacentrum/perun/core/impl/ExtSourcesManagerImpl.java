...
public class ExtSourcesManagerImpl implements ExtSourcesManagerImplApi {
  ...
  final static String extSourceMappingSelectQuery =
  ...
  public ExtSource createExtSource(PerunSession sess, ExtSource extSource, Map<String, String> attributes)
    ...
    try {
      ...
      jdbc.update(
          "insert into ext_sources (id, name, type, created_by,created_at,modified_by,modified_at,created_by_uid,modified_by_uid) " +
              "values (?,?,?,?," + Compatibility.getSysdate() + ",?," + Compatibility.getSysdate() + ",?,?)", newId,
          extSource.getName(),
          extSource.getType(), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getActor(),
      ...
      if (attributes != null) {
        for (String attr_name : attributes.keySet()) {
          jdbc.update(
              "insert into ext_sources_attributes (attr_name, attr_value, ext_sources_id,created_by, created_at, modified_by, modified_at, created_by_uid, modified_by_uid) " +
                  "values (?,?,?,?," + Compatibility.getSysdate() + ",?," + Compatibility.getSysdate() + ",?,?)",
              attr_name, attributes.get(attr_name), newId,
    ...
    if (!extSourceDb.getName().equals(extSource.getName())) {
      try {
        jdbc.update("update ext_sources set name=? ,modified_by=?, modified_by_uid=?, modified_at=" +
                Compatibility.getSysdate() + " where id=?", extSource.getName(), sess.getPerunPrincipal().getActor(),
            sess.getPerunPrincipal().getUserId(), extSource.getId());
      } catch (RuntimeException e) {
        throw new InternalErrorException(e);
      }
    }
    ...
    if (!extSourceDb.getType().equals(extSource.getType())) {
      try {
        jdbc.update("update ext_sources set type=?, modified_by=?, modified_by_uid=?, modified_at=" +
                Compatibility.getSysdate() + " where id=?", extSource.getType(), sess.getPerunPrincipal().getActor(),
            sess.getPerunPrincipal().getUserId(), extSource.getId());
      } catch (RuntimeException e) {
        throw new InternalErrorException(e);
      }
    ...
    if (!getAttributes(extSourceDb).equals(attributes)) {
      ...
      try {
        ...
        for (String attrName : attributes.keySet()) {
          jdbc.update(
              "insert into ext_sources_attributes (ext_sources_id, attr_name, attr_value, created_by, created_at, modified_by, modified_at, created_by_uid, modified_by_uid) " +
                  "values (?,?,?,?," + Compatibility.getSysdate() + ",?," + Compatibility.getSysdate() + ",?,?)",
              extSource.getId(), attrName,
  ...
  public ExtSource getExtSourceById(PerunSession sess, int id) throws ExtSourceNotExistsException {
    try {
      return jdbc.queryForObject("select " + extSourceMappingSelectQuery + " from ext_sources where id=?",
          EXTSOURCE_MAPPER, id);
  ...
  public ExtSource getExtSourceByName(PerunSession sess, String name) throws ExtSourceNotExistsException {
    try {
      return jdbc.queryForObject("select " + extSourceMappingSelectQuery + " from ext_sources where name=?",
          EXTSOURCE_MAPPER, name);
    } catch (EmptyResultDataAccessException ex) {
  ...
  public void addExtSource(PerunSession sess, Vo vo, ExtSource source) throws ExtSourceAlreadyAssignedException {
    try {
      ...
      jdbc.update(
          "insert into vo_ext_sources (ext_sources_id, vo_id, created_by, created_at, modified_by, modified_at, created_by_uid, modified_by_uid) " +
              "values (?,?,?," + Compatibility.getSysdate() + ",?," + Compatibility.getSysdate() + ",?,?)",
          source.getId(), vo.getId(),
          sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getActor(),
          sess.getPerunPrincipal().getUserId(), sess.getPerunPrincipal().getUserId());
  ...
  public void addExtSource(PerunSession sess, Group group, ExtSource source) throws ExtSourceAlreadyAssignedException {
    try {
      ...
      jdbc.update(
          "insert into group_ext_sources (ext_source_id, group_id, created_by, created_at, modified_by, modified_at, created_by_uid, modified_by_uid) " +
              "values (?,?,?," + Compatibility.getSysdate() + ",?," + Compatibility.getSysdate() + ",?,?)",
          source.getId(), group.getId(),
          sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getActor(),
          sess.getPerunPrincipal().getUserId(), sess.getPerunPrincipal().getUserId());
  ...
  public List<ExtSource> getExtSourcesToSynchronize(PerunSession sess) {
    try {
      return jdbc.query("select " + extSourceMappingSelectQuery +
              " from ext_sources, ext_sources_attributes where ext_sources.id=ext_sources_attributes.ext_sources_id and ext_sources_attributes.attr_name=? and ext_sources_attributes.attr_value=true",
          EXTSOURCE_MAPPER, ExtSourcesManager.EXTSOURCE_SYNCHRONIZATION_ENABLED_ATTRNAME);
