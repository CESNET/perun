...
public class ExtSourcesManagerImpl implements ExtSourcesManagerImplApi {
  ...
  public static final String USEREXTSOURCEMAPPING = "additionalues_";
  static final String EXT_SOURCE_MAPPING_SELECT_QUERY =
      "ext_sources.id as ext_sources_id, ext_sources.name as ext_sources_name, ext_sources.type as ext_sources_type, " +
      "ext_sources.created_at as ext_sources_created_at, ext_sources.created_by as ext_sources_created_by, " +
      "ext_sources.modified_by as ext_sources_modified_by, " +
      "ext_sources.modified_at as ext_sources_modified_at, ext_sources.modified_by_uid as " +
      "ext_sources_modified_by_uid, ext_sources.created_by_uid as ext_sources_created_by_uid";
  private static final Logger LOG = LoggerFactory.getLogger(ExtSourcesManagerImpl.class);
  ...
  private final RowMapper<ExtSource> extsourceMapper = (rs, i) -> {
    try {
      ...
      if (rs.getInt("ext_sources_modified_by_uid") == 0) {
        es.setModifiedByUid(null);
      } else {
        es.setModifiedByUid(rs.getInt("ext_sources_modified_by_uid"));
      }
      if (rs.getInt("ext_sources_created_by_uid") == 0) {
        es.setCreatedByUid(null);
      } else {
        es.setCreatedByUid(rs.getInt("ext_sources_created_by_uid"));
      }
      return es;
  ...
  private static HikariDataSource createHikariPool(String name, String url, String driver, String user,
                                                   String password) {
  ...
  private static Integer parsePoolIntProperty(Element dbpool, String propName) {
    ...
    try {
      ...
      LOG.error("<" + propName + "> of <dbpool name=\"" + dbpool.getName() + "\"> does not contain a number: " +
                element.getText());
      return null;
    }
  }
  ...
  public void addExtSource(PerunSession sess, Vo vo, ExtSource source) throws ExtSourceAlreadyAssignedException {
    try {
      if (0 <
          jdbc.queryForInt("select count('x') from vo_ext_sources where ext_sources_id=? and vo_id=?", source.getId(),
              vo.getId())) {
        throw new ExtSourceAlreadyAssignedException(source);
      }

      jdbc.update(
          "insert into vo_ext_sources (ext_sources_id, vo_id, created_by, created_at, modified_by, modified_at, " +
          "created_by_uid, modified_by_uid) " + "values (?,?,?," + Compatibility.getSysdate() + ",?," +
          Compatibility.getSysdate() + ",?,?)", source.getId(), vo.getId(), sess.getPerunPrincipal().getActor(),
  ...
  public void addExtSource(PerunSession sess, Group group, ExtSource source) throws ExtSourceAlreadyAssignedException {
    try {
      if (0 < jdbc.queryForInt("select count('x') from group_ext_sources where ext_source_id=? and group_id=?",
          source.getId(), group.getId())) {
        throw new ExtSourceAlreadyAssignedException(source);
      }

      jdbc.update(
          "insert into group_ext_sources (ext_source_id, group_id, created_by, created_at, modified_by, modified_at, " +
          "created_by_uid, modified_by_uid) " + "values (?,?,?," + Compatibility.getSysdate() + ",?," +
          Compatibility.getSysdate() + ",?,?)", source.getId(), group.getId(), sess.getPerunPrincipal().getActor(),
          sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getUserId(),
          sess.getPerunPrincipal().getUserId());
  ...
  public void checkExtSourceExists(PerunSession perunSession, ExtSource es) throws ExtSourceNotExistsException {
    if (!extSourceExists(perunSession, es)) {
      throw new ExtSourceNotExistsException("ExtSource: " + es);
    }
  }
  ...
  public ExtSource createExtSource(PerunSession sess, ExtSource extSource, Map<String, String> attributes)
    ...
    try {
      ...
      if (0 < jdbc.queryForInt("select count(id) from ext_sources where name=? and type=?", extSource.getName(),
          extSource.getType())) {
      ...
      jdbc.update(
          "insert into ext_sources (id, name, type, created_by,created_at,modified_by,modified_at,created_by_uid," +
          "modified_by_uid) " + "values (?,?,?,?," + Compatibility.getSysdate() + ",?," + Compatibility.getSysdate() +
          ",?,?)", newId, extSource.getName(), extSource.getType(), sess.getPerunPrincipal().getActor(),
          sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getUserId(),
          sess.getPerunPrincipal().getUserId());
      ...
      if (attributes != null) {
        for (String attrName : attributes.keySet()) {
          jdbc.update(
              "insert into ext_sources_attributes (attr_name, attr_value, ext_sources_id,created_by, created_at, " +
              "modified_by, modified_at, created_by_uid, modified_by_uid) " + "values (?,?,?,?," +
              Compatibility.getSysdate() + ",?," + Compatibility.getSysdate() + ",?,?)", attrName,
              attributes.get(attrName), newId, sess.getPerunPrincipal().getActor(),
              sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getUserId(),
              sess.getPerunPrincipal().getUserId());
        }
      }
  ...
  public Map<String, String> getAttributes(ExtSource extSource) {
    try {
      return jdbc.query(
          "select attr_name, attr_value from ext_sources_attributes where ext_sources_id = " + extSource.getId(),
  ...
  public ExtSource getExtSourceById(PerunSession sess, int id) throws ExtSourceNotExistsException {
    try {
      return jdbc.queryForObject("select " + EXT_SOURCE_MAPPING_SELECT_QUERY + " from ext_sources where id=?",
          extsourceMapper, id);
    } catch (EmptyResultDataAccessException ex) {
  ...
  public ExtSource getExtSourceByName(PerunSession sess, String name) throws ExtSourceNotExistsException {
    try {
      return jdbc.queryForObject("select " + EXT_SOURCE_MAPPING_SELECT_QUERY + " from ext_sources where name=?",
          extsourceMapper, name);
    } catch (EmptyResultDataAccessException ex) {
  ...
  public List<ExtSource> getExtSources(PerunSession sess) {
    try {
      return jdbc.query("SELECT " + EXT_SOURCE_MAPPING_SELECT_QUERY + " FROM ext_sources", extsourceMapper);
    } catch (RuntimeException e) {
      throw new InternalErrorException(e);
    }
  }
  ...
  public List<ExtSource> getExtSourcesToSynchronize(PerunSession sess) {
    try {
      return jdbc.query("select " + EXT_SOURCE_MAPPING_SELECT_QUERY +
                        " from ext_sources, ext_sources_attributes where ext_sources.id=ext_sources_attributes" +
                        ".ext_sources_id " +
                        "and ext_sources_attributes.attr_name=? and ext_sources_attributes.attr_value=true",
          extsourceMapper, ExtSourcesManager.EXTSOURCE_SYNCHRONIZATION_ENABLED_ATTRNAME);
  ...
  public List<Integer> getGroupExtSourcesIds(PerunSession perunSession, Group group) throws InternalErrorException {
    try {
      return jdbc.queryForList("SELECT ext_sources.id " +
                               " FROM group_ext_sources g_exts JOIN ext_sources ON g_exts.ext_source_id=ext_sources" +
                               ".id " +
  ...
  public List<Integer> getVoExtSourcesIds(PerunSession sess, Vo vo) throws InternalErrorException {
    try {
      return jdbc.queryForList(
          "SELECT ext_sources.id " + " FROM vo_ext_sources v JOIN ext_sources ON v.ext_sources_id=ext_sources.id " +
  ...
  public void initialize(PerunSession sess, PerunBl perunBl) {
    ...
    if (sess.getPerun().isPerunReadOnly()) {
      LOG.debug("Loading extSource manager init in readOnly version.");
    }
  ...
  /**
   * Loads the extSources definitions from the XML configuration file. All data from the extSouces XML file are
   ...
   * <driver>oracle.jdbc.OracleDriver</driver><!-- optional for postgresql, oracle, sqlite, mysql; required for
   * others -->
  ...
  public void loadExtSourcesDefinitions(PerunSession sess) {
        ...
        for (Element dbpool : dbpools.elements("dbpool")) {
          ...
          if (main != null) {
            ...
            if (leakDetectionThreshold != null) {
              hds.setLeakDetectionThreshold(leakDetectionThreshold);
            }
    ...
    // check connection attributes for all ExtSourcesSql present in the database (may be more than in
  ...
  public void removeExtSource(PerunSession sess, Vo vo, ExtSource source)
    ...
    try {
      if (jdbc.queryForInt("select count('x') from vo_ext_sources where ext_sources_id=? and vo_id=?", source.getId(),
          vo.getId()) == 0) {
      ...
      if (numAffected != 1) {
        throw new ExtSourceAlreadyRemovedException("ExtSource: " + source + " , Vo: " + vo);
      }
    } catch (RuntimeException e) {
      throw new InternalErrorException(e);
    }
  ...
  @Override
  public void removeExtSource(PerunSession perunSession, Group group, ExtSource source)
      throws ExtSourceNotAssignedException, ExtSourceAlreadyRemovedException {
    try {
      if (jdbc.queryForInt("select count('x') from group_ext_sources where ext_source_id=? and group_id=?",
          source.getId(), group.getId()) == 0) {
        // Source isn't assigned
        throw new ExtSourceNotAssignedException("ExtSource id='" + source.getId() + "'");
      }

      int numAffected =
          jdbc.update("DELETE FROM group_ext_sources WHERE ext_source_id=? AND group_id=?", source.getId(),
              group.getId());
      if (numAffected != 1) {
  ...
  @Override
  public void updateExtSource(PerunSession sess, ExtSource extSource, Map<String, String> attributes)
    ...
    if (!extSourceDb.getName().equals(extSource.getName())) {
      try {
        jdbc.update("update ext_sources set name=? ,modified_by=?, modified_by_uid=?, modified_at=" +
                    Compatibility.getSysdate() + " where id=?", extSource.getName(),
            sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getUserId(), extSource.getId());
      } catch (RuntimeException e) {
        throw new InternalErrorException(e);
      }
    }
    ...
    if (!extSourceDb.getType().equals(extSource.getType())) {
      try {
        jdbc.update("update ext_sources set type=?, modified_by=?, modified_by_uid=?, modified_at=" +
                    Compatibility.getSysdate() + " where id=?", extSource.getType(),
            sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getUserId(), extSource.getId());
      } catch (RuntimeException e) {
        throw new InternalErrorException(e);
      }
    ...
    if (!getAttributes(extSourceDb).equals(attributes)) {
      ...
      try {
        ...
        for (String attrName : attributes.keySet()) {
          jdbc.update(
              "insert into ext_sources_attributes (ext_sources_id, attr_name, attr_value, created_by, created_at, " +
              "modified_by, modified_at, created_by_uid, modified_by_uid) " + "values (?,?,?,?," +
              Compatibility.getSysdate() + ",?," + Compatibility.getSysdate() + ",?,?)", extSource.getId(), attrName,
              attributes.get(attrName), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getActor(),
