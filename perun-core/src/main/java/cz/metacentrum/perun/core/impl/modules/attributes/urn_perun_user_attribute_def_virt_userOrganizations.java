...
public class urn_perun_user_attribute_def_virt_userOrganizations extends UserVirtualAttributesModuleAbstract
  ...
  public Attribute getAttributeValue(PerunSessionImpl perunSession, User user,
    ...
    for (Vo vo : vos) {
      try {
        ...
        String value =
            perunSession.getPerunBl().getAttributesManagerBl().getAttribute(perunSession, member, A_M_organization)
                .valueAsString();
        if (value != null) {
          userOrganizations.put(vo.getShortName(), value);
        }
