...
public class OwnersManagerImpl implements OwnersManagerImplApi {
  ...
  protected final static String ownerMappingSelectQuery =
      "owners.id as owners_id, owners.name as owners_name, owners.contact as owners_contact, owners.type as owners_type, " +
  ...
  public Owner createOwner(PerunSession sess, Owner owner) {
    ...
    try {
      jdbc.update(
          "insert into owners(id, name, contact, type, created_by,created_at,modified_by,modified_at,created_by_uid,modified_by_uid) " +
              "values (?,?,?,?,?," + Compatibility.getSysdate() + ",?," + Compatibility.getSysdate() + ",?,?)", newId,
          owner.getName(),
          owner.getContact(), owner.getType().toString(), sess.getPerunPrincipal().getActor(),
