...
public class OwnersManagerImpl implements OwnersManagerImplApi {
  ...
  protected static final String OWNER_MAPPING_SELECT_QUERY =
      "owners.id as owners_id, owners.name as owners_name, owners.contact as owners_contact, owners.type as " +
      "owners_type, " +
      "owners.created_at as owners_created_at, owners.created_by as owners_created_by, owners.modified_by as " +
      "owners_modified_by, owners.modified_at as owners_modified_at, " +
  ...
  protected static final RowMapper<Owner> OWNER_MAPPER = (resultSet, i) -> {
    ...
    if (resultSet.getInt("owners_modified_by_uid") == 0) {
      owner.setModifiedByUid(null);
    } else {
      owner.setModifiedByUid(resultSet.getInt("owners_modified_by_uid"));
    }
    if (resultSet.getInt("owners_created_by_uid") == 0) {
      owner.setCreatedByUid(null);
    } else {
      owner.setCreatedByUid(resultSet.getInt("owners_created_by_uid"));
    }
    return owner;
  };
  static final Logger LOG = LoggerFactory.getLogger(OwnersManagerImpl.class);
  ...
  public void checkOwnerExists(PerunSession sess, Owner owner) throws OwnerNotExistsException {
    if (!ownerExists(sess, owner)) {
      throw new OwnerNotExistsException("Owner: " + owner);
    }
  }
  ...
  public Owner createOwner(PerunSession sess, Owner owner) {
    ...
    try {
      jdbc.update(
          "insert into owners(id, name, contact, type, created_by,created_at,modified_by,modified_at,created_by_uid," +
          "modified_by_uid) " + "values (?,?,?,?,?," + Compatibility.getSysdate() + ",?," + Compatibility.getSysdate() +
          ",?,?)", newId, owner.getName(), owner.getContact(), owner.getType().toString(),
  ...
  public void deleteOwner(PerunSession sess, Owner owner) throws OwnerAlreadyRemovedException {
    try {
      ...
      if (numAffected == 0) {
        throw new OwnerAlreadyRemovedException("Owner: " + owner);
      }
    } catch (RuntimeException ex) {
      throw new InternalErrorException(ex);
    }
  ...
  public Owner getOwnerById(PerunSession sess, int id) throws OwnerNotExistsException {
    try {
      return jdbc.queryForObject("select " + OWNER_MAPPING_SELECT_QUERY + " from owners where id=?", OWNER_MAPPER, id);
  ...
  public Owner getOwnerByName(PerunSession sess, String name) throws OwnerNotExistsException {
    try {
      return jdbc.queryForObject("select " + OWNER_MAPPING_SELECT_QUERY +
                                     " from owners where name=?", OWNER_MAPPER, name);
    } catch (EmptyResultDataAccessException ex) {
  ...
  public List<Owner> getOwners(PerunSession sess) {
    try {
      return jdbc.query("select " + OWNER_MAPPING_SELECT_QUERY + " from owners", OWNER_MAPPER);
    } catch (EmptyResultDataAccessException ex) {
