...
public class urn_perun_user_attribute_def_virt_institutionsCountries
    extends UserVirtualAttributeCollectedFromUserExtSource<DnsMapCtx> {

  private static final Logger LOG =
      LoggerFactory.getLogger(urn_perun_user_attribute_def_virt_institutionsCountries.class);
  private static final AttributeDefinition DNS_STATE_MAPPING_ATTR =
      new urn_perun_entityless_attribute_def_def_dnsStateMapping().getAttributeDefinition();
  ...
  /**
   * Loads DNS-to-country translation map from attribute urn:perun:entityless:attribute-def:def:dnsStateMaping. DNS
   * domains are in keys, country names are in values of the attribute.
   */
  @Override
  protected DnsMapCtx initModifyValueContext(PerunSessionImpl sess, User user, AttributeDefinition attr) {
    try {
      return new DnsMapCtx(sess, user, attr, sess.getPerunBl().getAttributesManagerBl()
          .getEntitylessStringAttributeMapping(sess, DNS_STATE_MAPPING_ATTR.getName()));
  ...
  private List<AuditEvent> resolveEvent(PerunSessionImpl sess, String key)
    ...
    //find users that are affected by the change - have schacHomeOrganization value ending in key but not ending with
    // longerDomains
    List<User> affectedUsers = sess.getPerunBl().getUsersManagerBl()
        .findUsersWithExtSourceAttributeValueEnding(sess, getSourceAttributeName(), key, longerDomains);
  ...
  /**
   * For a change in dnsStateMapping attribute, finds all affected users and generates audit message about changing this
  ...
  public List<AuditEvent> resolveVirtualAttributeValueChange(PerunSessionImpl sess, AuditEvent message)
    ...
    if (message instanceof AttributeSetForKey && ((AttributeSetForKey) message).getAttribute().getFriendlyName()
        .equals(DNS_STATE_MAPPING_ATTR.getFriendlyName())) {

      messages.addAll(resolveEvent(sess, ((AttributeSetForKey) message).getKey()));
    } else if (message instanceof AttributeRemovedForKey &&
               ((AttributeRemovedForKey) message).getAttribute().getFriendlyName()
                   .equals(DNS_STATE_MAPPING_ATTR.getFriendlyName())) {
