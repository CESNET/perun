...
public interface GroupsManager {
  ...
  String GROUPS_STRUCTURE_SYNCHRO_ENABLED_ATTRNAME =
      AttributesManager.NS_GROUP_ATTR_DEF + ":groupStructureSynchronizationEnabled";
  ...
  String GROUP_STRUCTURE_SYNCHRO_INTERVAL_ATTRNAME =
      AttributesManager.NS_GROUP_ATTR_DEF + ":groupStructureSynchronizationInterval";
  ...
  // Defines the times, when the group has to be synchronized.
  String GROUP_STRUCTURE_SYNCHRO_TIMES_ATTRNAME =
      AttributesManager.NS_GROUP_ATTR_DEF + ":groupStructureSynchronizationTimes";
  // Defines timestamp with start of last successful synchronization
  String GROUP_START_OF_LAST_SUCCESSFUL_SYNC_ATTRNAME =
      AttributesManager.NS_GROUP_ATTR_DEF + ":startOfLastSuccessfulSynchronization";
  ...
  String GROUP_MEMBERSHIP_EXPIRATION_RULES_ATTRNAME =
      AttributesManager.NS_GROUP_ATTR_DEF + ":groupMembershipExpirationRules";
  String GROUP_SYNCHRONIZATION_FILE_ATTRNAME = AttributesManager.NS_GROUP_ATTR_DEF + ":groupSynchronizationFilename";
  // Defines filename attribute used in combination with directory config option in synchronizations
   ...
   * @throws GroupNotExistsException
   * @throws WrongAttributeValueException          if any member attribute value, required by resource (on which the
   *                                               group is assigned), is wrong
   ...
   * @throws RelationExistsException    if group is already allowed to be included to parent vo
   */
  void allowGroupsToHierarchicalVo(PerunSession sess, List<Group> groups, Vo vo)
      throws VoNotExistsException, GroupNotExistsException, PrivilegeException, RelationNotExistsException,
                 RelationExistsException;
   ...
   */
  Group createGroup(PerunSession perunSession, Vo vo, Group group)
   ...
   * @throws VoNotExistsException
   */
  Group createGroupUnion(PerunSession sess, Group resultGroup, Group operandGroup)
   ...
   * @throws GroupAlreadyRemovedFromResourceException if there is at least 1 group on resource affected by deleting from
   *                                                  DB
  ...
  /**
   * Deletes group only if has no subgroups and no members. Other way throw exception. This method is same like
   * deleteGroup(sess, group, false) with false for forceDelete
   *
   * @param perunSession
   * @param group        group to delete
   * @throws GroupNotExistsException
   ...
   * @throws GroupRelationCannotBeRemoved
   */
  void deleteGroup(PerunSession perunSession, Group group)
      throws GroupNotExistsException, PrivilegeException, RelationExistsException, GroupAlreadyRemovedException,
                 GroupAlreadyRemovedFromResourceException, GroupRelationDoesNotExist, GroupRelationCannotBeRemoved,
                 ExternallyManagedException;
   ...
   * @throws InternalErrorException
   * @throws PrivilegeException                       if user has no right to call delete operation on any of these
   *                                                  groups
   ...
   * @throws RelationNotExistsException if group is not allowed to be included in parent vo
   */
  void disallowGroupsToHierarchicalVo(PerunSession sess, List<Group> groups, Vo vo)
      throws VoNotExistsException, GroupNotExistsException, PrivilegeException, RelationNotExistsException;
   ...
   */
  void extendMembershipInGroup(PerunSession sess, Member member, Group group)
  ...
  /**
   * Get groups of Vo by ACCESS RIGHTS: If user is: - PERUNADMIN or VOADMIN : all groups in vo - GROUPADMIN : only
  ...
  /**
   * Get groups of the VO stored in the map reflecting the hierarchy by ACCESS RIGHTS: If user is: - PERUNADMIN or
   * VOADMIN : all Groups - GROUPADMIN : only groups where user is groupAdmin
   *
   * @param sess
   * @param vo
  ...
  /**
   * Get all groups with their specified attributes. If the {@code attrNames} are null or empty, all group attributes
   * are returned.
   *
   * @param sess      session
  ...
  /**
   * Return all RichSubGroups in parentGroup (all levels sub groups) containing selected attributes filtered by role and
   * its type,
   *
   * @param sess        perun session
   ...
   * @throws PrivilegeException
   */
  Group getGroupByName(PerunSession perunSession, Vo vo, String name)
   ...
   */
  Member getGroupMemberById(PerunSession sess, Group group, int memberId)
  ...
  /**
   * Returns list of RichMembers with requested attributes by their member IDs from given group. Skips invalid member
   * IDs (unknown or not members of group). Supports member, member-group (stored in memberAttributes) and user
   * attributes (stored in userAttributes).
   *
   * @param sess      perun session
   ...
   * @return list of RichMembers
   */
  List<RichMember> getGroupRichMembersByIds(PerunSession sess, int groupId, List<Integer> memberIds,
  ...
  /**
   * Get groups of users under the VO by ACCESS RIGHTS: If user is: - PERUNADMIN or VOADMIN : all groups - GROUPADMIN :
   * only groups where user is GroupAdmin
   *
   * @param sess
   * @param vo   vo
   * @return list of groups
   ...
   * @return List of groups
   */
  List<Group> getGroupsWhereUserIsActiveMember(PerunSession session, User user, Vo vo)
  ...
  List<Group> getMemberGroupsByAttribute(PerunSession sess, Member member, Attribute attribute)
      throws PrivilegeException, WrongAttributeAssignmentException, MemberNotExistsException,
   ...
   */
  Group getParentGroup(PerunSession sess, Group group)
      throws GroupNotExistsException, PrivilegeException, ParentGroupNotExistsException;
  ...
  List<RichUser> getRichAdmins(PerunSession perunSession, Group group, List<String> specificAttributes,
      ...
      throws UserNotExistsException, PrivilegeException, GroupNotExistsException;
   ...
   */
  @Deprecated
  List<RichUser> getRichAdminsWithAttributes(PerunSession perunSession, Group group)
  ...
  /**
   * Get list of all richGroups with selected attributes assigned to resource. Allowed namespaces of attributes are
   ...
   * @throws PrivilegeException
   */
  List<RichGroup> getRichGroupsAssignedToResourceWithAttributesByNames(PerunSession sess, Resource resource,
                                                                       List<String> attrNames)
   ...
   * namespaces of attributes are group, group-resource, member-group
   * <p>
   * Last step is filtration of attributes: Attributes are filtered by rights of user in session. User get only those
   * selected attributes he has rights to read.
   *
   ...
   * @throws PrivilegeException
   */
  List<RichGroup> getRichGroupsAssignedToResourceWithAttributesByNames(PerunSession sess, Member member,
   ...
   * @throws UserNotExistsException
   */
  void removeAdmin(PerunSession perunSession, Group group, User user)
   ...
   */
  void removeAdmin(PerunSession perunSession, Group group, Group authorizedGroup)
   ...
   * @throws PrivilegeException
   */
  void removeGroupUnion(PerunSession sess, Group resultGroup, Group operandGroup)
   ...
   * @throws ExternallyManagedException        when the group is externally managed
   */
  void removeMember(PerunSession perunSession, Member member, List<Group> groups)
      throws MemberNotExistsException, NotGroupMemberException, PrivilegeException, GroupNotExistsException,
                 WrongAttributeAssignmentException, AttributeNotExistsException, ExternallyManagedException;
  ...
  /**
   * Set Members Group status for specified DIRECT member and group. Member with newly calculated group membership
   * status is returned.
   * <p>
   * Please note, that if member is also sourced from sub-groups or groups in relation and has VALID status in any of
   * them, then resulting status is still VALID. In order to really expire such member is to set EXPIRED status also to
   * all sourcing sub-groups or groups in relation.
   *
   * @param sess   perun session
   ...
   */
  Member setMemberGroupStatus(PerunSession sess, Member member, Group group, MemberGroupStatus status)
  ...
  /**
   * Synchronize all groups structures (with members) which have enabled group structure synchronization. This method is
   ...
   * @throws PrivilegeException
   */
  Group updateGroup(PerunSession perunSession, Group group)
