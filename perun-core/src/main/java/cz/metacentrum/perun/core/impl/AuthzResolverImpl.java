...
public class AuthzResolverImpl implements AuthzResolverImplApi {
  ...
  private final static String authzComplementaryObjectsWithAuthorizedGroups =
  ...
  private final static String authzRoleMappingSelectQuery =
  ...
  public static Map<String, Set<ActionType>> getRolesWhichCanWorkWithAttribute(ActionType actionType,
    ...
    try {
      List<Pair<String, ActionType>> pairs =
          jdbc.query("select distinct roles.name, action_types.action_type from attributes_authz " +
              ...
              (rs, arg1) -> new Pair<>(rs.getString("name").toUpperCase(),
                  ActionType.valueOf(rs.getString("action_type").toUpperCase())),
    ...
    if (user != null) {
      try {
        ...
        List<Integer> authzServiceUsers = jdbc.query("select specific_user_users.specific_user_id as id from users, " +
                ...
                "and specific_user_users.type=?", Utils.ID_MAPPER, user.getId(),
            SpecificUserType.SERVICE.getSpecificUserType());
  ...
  public Map<String, Map<String, Map<Integer, List<Group>>>> getRoleComplementaryObjectsWithAuthorizedGroups(
    ...
    try {
      // Get user roles based on membership in authorized group(s)
      List<Pair<String, Map<String, Map<Integer, List<Group>>>>> rolePairs =
          jdbc.query("select " + authzComplementaryObjectsWithAuthorizedGroups + ", roles.name as role_name " +
                  "from authz " +
  ...
  public AuthzRoles getRoles(Group group) {
    ...
    if (group != null) {
      try {
        ...
        List<Pair<String, Map<String, Set<Integer>>>> authzRolesPairs =
            jdbc.query("select " + authzRoleMappingSelectQuery
                    +
  ...
  public void addAdmin(PerunSession sess, Facility facility, User user) throws AlreadyAdminException {
    try {
      jdbc.update("insert into authz (user_id, role_id, facility_id, created_by, created_by_uid)" +
              " values (?, (select id from roles where name=?), ?, ?, ?)", user.getId(), Role.FACILITYADMIN.toLowerCase(),
          facility.getId(), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getUserId());
  ...
  public void addAdmin(PerunSession sess, Facility facility, Group group) throws AlreadyAdminException {
    try {
      jdbc.update("insert into authz (authorized_group_id, role_id, facility_id, created_by, created_by_uid)" +
              " values (?, (select id from roles where name=?), ?, ?, ?)", group.getId(), Role.FACILITYADMIN.toLowerCase(),
          facility.getId(), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getUserId());
    } catch (DataIntegrityViolationException e) {
  ...
  public void removeAdmin(PerunSession sess, Facility facility, Group group) throws GroupNotAdminException {
    try {
      if (0 == jdbc.update(
          "delete from authz where authorized_group_id=? and facility_id=? and role_id=(select id from roles where name=?)",
          group.getId(), facility.getId(), Role.FACILITYADMIN.toLowerCase())) {
  ...
  public void addAdmin(PerunSession sess, Resource resource, Group group) throws AlreadyAdminException {
    try {
      jdbc.update(
          "insert into authz (authorized_group_id, role_id, resource_id, vo_id, facility_id, created_by, created_by_uid)" +
  ...
  public void removeAdmin(PerunSession sess, Resource resource, Group group) throws GroupNotAdminException {
    try {
      if (0 == jdbc.update(
          "delete from authz where authorized_group_id=? and resource_id=? and role_id=(select id from roles where name=?)",
          group.getId(), resource.getId(), Role.RESOURCEADMIN.toLowerCase())) {
  ...
  public void addAdmin(PerunSession sess, User sponsoredUser, User user) throws AlreadyAdminException {
    try {
      jdbc.update("insert into authz (user_id, role_id, sponsored_user_id, created_by, created_by_uid)" +
              " values (?, (select id from roles where name=?), ?, ?, ?)", user.getId(), Role.SPONSOR.toLowerCase(),
          sponsoredUser.getId(), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getUserId());
    } catch (DataIntegrityViolationException e) {
  ...
  public void addAdmin(PerunSession sess, User sponsoredUser, Group group) throws AlreadyAdminException {
    try {
      jdbc.update("insert into authz (authorized_group_id, role_id, sponsored_user_id, created_by, created_by_uid)" +
              " values (?, (select id from roles where name=?), ?, ?, ?)", group.getId(), Role.SPONSOR.toLowerCase(),
          sponsoredUser.getId(), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getUserId());
    } catch (DataIntegrityViolationException e) {
      throw new AlreadyAdminException(
  ...
  public void removeAdmin(PerunSession sess, User sponsoredUser, Group group) throws GroupNotAdminException {
    try {
      if (0 == jdbc.update(
          "delete from authz where authorized_group_id=? and sponsored_user_id=? and role_id=(select id from roles where name=?)",
          group.getId(), sponsoredUser.getId(), Role.SPONSOR.toLowerCase())) {
  ...
  public void removeAdmin(PerunSession sess, Group group, Group authorizedGroup) throws GroupNotAdminException {
    try {
      if (0 == jdbc.update(
          "delete from authz where authorized_group_id=? and group_id=? and role_id=(select id from roles where name=?)",
  ...
  public void removeAdmin(PerunSession sess, SecurityTeam securityTeam, Group group) throws GroupNotAdminException {
    try {
      if (0 == jdbc.update(
          "delete from authz where authorized_group_id=? and security_team_id=? and role_id=(select id from roles where name=?)",
          group.getId(), securityTeam.getId(), Role.SECURITYADMIN.toLowerCase())) {
        throw new GroupNotAdminException(
  ...
  public void makeUserPerunAdmin(PerunSession sess, User user) throws AlreadyAdminException {
    try {
      jdbc.update("insert into authz (user_id, role_id, created_by, created_by_uid)" +
              " values (?, (select id from roles where name=?), ?, ?)", user.getId(), Role.PERUNADMIN.toLowerCase(),
          sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getUserId());
    } catch (DataIntegrityViolationException e) {
  ...
  public void makeUserPerunObserver(PerunSession sess, User user) throws AlreadyAdminException {
    try {
      jdbc.update("insert into authz (user_id, role_id, created_by, created_by_uid)" +
              " values (?, (select id from roles where name=?), ?, ?)", user.getId(), Role.PERUNOBSERVER.toLowerCase(),
          sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getUserId());
    } catch (DataIntegrityViolationException e) {
  ...
  public void removePerunObserverFromAuthorizedGroup(PerunSession sess, Group authorizedGroup)
    ...
    try {
      if (0 ==
          jdbc.update("delete from authz where authorized_group_id=? and role_id=(select id from roles where name=?)",
  ...
  public void removePerunAdmin(PerunSession sess, User user) throws UserNotAdminException {
    try {
      if (0 ==
          jdbc.update("delete from authz where user_id=? and role_id=(select id from roles where name=?)", user.getId(),
              Role.PERUNADMIN.toLowerCase())) {
  ...
  public void removePerunObserver(PerunSession sess, User user) throws UserNotAdminException {
    try {
      if (0 ==
          jdbc.update("delete from authz where user_id=? and role_id=(select id from roles where name=?)", user.getId(),
              Role.PERUNOBSERVER.toLowerCase())) {
  ...
  public void makeUserCabinetAdmin(PerunSession sess, User user) {
    try {
      jdbc.update("insert into authz (user_id, role_id, created_by, created_by_uid)" +
              " values (?, (select id from roles where name=?), ?, ?)", user.getId(), Role.CABINETADMIN.toLowerCase(),
          sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getUserId());
    } catch (RuntimeException e) {
      throw new InternalErrorException(e);
    }
  ...
  public void addVoRole(PerunSession sess, String role, Vo vo, User user) throws AlreadyAdminException {
    ...
    try {
      jdbc.update(
          "insert into authz (user_id, role_id, vo_id, created_by, created_by_uid)  values (?, (select id from roles where name=?), ?, ?, ?)",
          user.getId(), role.toLowerCase(), vo.getId(), sess.getPerunPrincipal().getActor(),
          sess.getPerunPrincipal().getUserId());
    } catch (DataIntegrityViolationException e) {
  ...
  public void addVoRole(PerunSession sess, String role, Vo vo, Group group) throws AlreadyAdminException {
    ...
    try {
      jdbc.update("insert into authz (role_id, vo_id, authorized_group_id, created_by, created_by_uid)" +
              " values ((select id from roles where name=?), ?, ?, ?, ?)",
          role.toLowerCase(), vo.getId(), group.getId(), sess.getPerunPrincipal().getActor(),
          sess.getPerunPrincipal().getUserId());
    } catch (DataIntegrityViolationException e) {
  ...
  public void removeVoRole(PerunSession sess, String role, Vo vo, User user) throws UserNotAdminException {
    try {
      if (0 ==
          jdbc.update("delete from authz where user_id=? and vo_id=? and role_id=(select id from roles where name=?)",
              user.getId(), vo.getId(), role.toLowerCase())) {
  ...
  public List<Integer> getVoIdsForGroupInRole(PerunSession sess, Group group, String role) {
    try {
      return jdbc.query(
          "SELECT vo_id FROM authz WHERE role_id=(select id from roles where name=?) and authorized_group_id=? and vo_id is not NULL",
  ...
  public List<Integer> getVoIdsForUserInRole(PerunSession sess, User user, String role) {
    try {
      return jdbc.query(
          "SELECT vo_id FROM authz WHERE role_id=(select id from roles where name=?) and user_id=? and vo_id is not NULL",
  ...
  public void addResourceRole(PerunSession sess, User user, String role, Resource resource)
    ...
    try {
      jdbc.update(
          "insert into authz (user_id, role_id, resource_id, vo_id, facility_id, created_by, created_by_uid) values (?, (select id from roles where name=?), ?, ?, ?, ?, ?)",
          user.getId(),
  ...
  public void addResourceRole(PerunSession sess, Group group, String role, Resource resource)
    ...
    try {
      jdbc.update(
          "insert into authz (role_id, resource_id, authorized_group_id, vo_id, facility_id, created_by, created_by_uid)" +
              " values ((select id from roles where name=?), ?, ?, ?, ?, ?, ?)",
  ...
  public void removeResourceRole(PerunSession sess, String role, Resource resource, Group group)
    ...
    try {
      if (0 == jdbc.update(
          "delete from authz where authorized_group_id=? and resource_id=? and role_id=(select id from roles where name=?)",
          group.getId(), resource.getId(), role.toLowerCase())) {
        throw new GroupNotAdminException(
    ...
    try {
      ...
      if (!onlyDirectAdmins) {
        ...
        for (Group authorizedGroup : listOfAdminGroups) {
          admins.addAll(jdbc.query("select " + UsersManagerImpl.userMappingSelectQuery +
                  " from users join members on users.id=members.user_id " +
                  "join groups_members on groups_members.member_id=members.id where groups_members.group_id=? and members.status=? and groups_members.source_group_status=?",
  ...
  public Set<Vo> getVosWhereUserIsInRoles(User user, List<String> roles) {
    ...
    try {
      return new HashSet<>(namedParameterJdbcTemplate.query(
          "select " + voMappingSelectQuery + " from authz join vos on authz.vo_id=vos.id " +
  ...
  public Set<Facility> getFacilitiesWhereUserIsInRoles(User user, List<String> roles) {
    ...
    try {
      return new HashSet<>(namedParameterJdbcTemplate.query(
          "select " + facilityMappingSelectQuery + " from authz join facilities on authz.facility_id=facilities.id " +
              " left outer join groups_members on groups_members.group_id=authz.authorized_group_id " +
              " left outer join members on members.id=groups_members.member_id " +
              " where (authz.user_id=:uid or members.user_id=:uid) and authz.role_id in " +
              "(select id from roles where name in (:roles))",
  ...
  public Set<Resource> getResourcesWhereUserIsInRoles(User user, List<String> roles) {
    ...
    try {
      return new HashSet<>(namedParameterJdbcTemplate.query(
          "select " + resourceMappingSelectQuery + " from authz join resources on authz.resource_id=resources.id " +
              " left outer join groups_members on groups_members.group_id=authz.authorized_group_id " +
              " left outer join members on members.id=groups_members.member_id " +
              " where (authz.user_id=:uid or members.user_id=:uid) and authz.role_id in " +
              "(select id from roles where name in (:roles))",
  ...
  public Set<Group> getGroupsWhereUserIsInRoles(User user, List<String> roles) {
    ...
    try {
      return new HashSet<>(namedParameterJdbcTemplate.query(
          "select " + groupMappingSelectQuery + " from authz join groups on authz.group_id=groups.id " +
              " left outer join groups_members on groups_members.group_id=authz.authorized_group_id " +
              " left outer join members on members.id=groups_members.member_id " +
              " where (authz.user_id=:uid or members.user_id=:uid) and authz.role_id in " +
              "(select id from roles where name in (:roles))",
    ...
    try {
      ...
      List<Group> indirectRelations = namedParameterJdbcTemplate.query(
          "select " + groupMappingSelectQuery + " from authz join groups on authz.group_id=groups.id " +
              ...
              " where groups.id=:groupId and members.user_id=:uid" +
              (!roles.isEmpty() ? " and (authz.role_id in (select id from roles where name in (:roles)))" : ""),
          parameters, GROUP_MAPPER);
  ...
  public Set<Member> getMembersWhereUserIsInRoles(User user, List<String> roles) {
    ...
    try {
      return new HashSet<>(namedParameterJdbcTemplate.query(
          "select " + memberMappingSelectQuery + " from authz join members on authz.member_id=members.id " +
              " left outer join groups_members on groups_members.group_id=authz.authorized_group_id " +
  ...
  public Set<SecurityTeam> getSecurityTeamsWhereUserIsInRoles(User user, List<String> roles) {
    ...
    try {
      return new HashSet<>(namedParameterJdbcTemplate.query("select " + securityTeamMappingSelectQuery +
              " from authz join security_teams on authz.security_team_id=security_teams.id " +
              " left outer join groups_members on groups_members.group_id=authz.authorized_group_id " +
              " left outer join members on members.id=groups_members.member_id " +
              " where (authz.user_id=:uid or members.user_id=:uid) and authz.role_id in " +
              "(select id from roles where name in (:roles))",
