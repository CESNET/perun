 ...
 */
public class urn_perun_user_facility_attribute_def_virt_UID extends UserFacilityVirtualAttributesModuleAbstract
  ...
  public void checkAttributeSemantics(PerunSessionImpl sess, User user, Facility facility, Attribute attribute)
    ...
    try {
      Attribute uidNamespaceAttribute = sess.getPerunBl().getAttributesManagerBl()
          .getAttribute(sess, facility, AttributesManager.NS_FACILITY_ATTR_DEF + ":uid-namespace");

      Attribute uidAttribute;
      if (uidNamespaceAttribute.getValue() != null) {
        // Get the u:uid-namespace[uidNamespaceAttribute]
        uidAttribute = sess.getPerunBl().getAttributesManagerBl().getAttribute(sess, user,
            AttributesManager.NS_USER_ATTR_DEF + ":uid-namespace:" + uidNamespaceAttribute.getValue());
        ...
        throw new WrongReferenceAttributeValueException(attribute, null, user, facility,
            "Required facility attribute uid-namespace value is null.");
      }
  ...
  public Attribute fillAttribute(PerunSessionImpl sess, User user, Facility facility, AttributeDefinition attribute)
    ...
    try {
      ...
      if (uidNamespaceAttribute.getValue() != null) {
        // Get the u:uid-namespace[uidNamespaceAttribute]
        Attribute uidAttribute = sess.getPerunBl().getAttributesManagerBl().getAttribute(sess, user,
            AttributesManager.NS_USER_ATTR_DEF + ":uid-namespace:" + uidNamespaceAttribute.getValue());
  ...
  public boolean setAttributeValue(PerunSessionImpl sess, User user, Facility facility, Attribute attribute)
    ...
    try {
      ...
      userUidAttributeDefinition = sess.getPerunBl().getAttributesManagerBl().getAttributeDefinition(sess,
          AttributesManager.NS_USER_ATTR_DEF + ":uid-namespace:" + uidNamespaceAttribute.getValue());
    } catch (AttributeNotExistsException e) {
