...
public class ConsentsManagerImpl implements ConsentsManagerImplApi {
  ...
  protected static final String CONSENT_HUB_MAPPING_SELECT_QUERY =
      "consent_hubs.id as consent_hubs_id, consent_hubs.name as consent_hubs_name, consent_hubs.enforce_consents as " +
      "consent_hubs_enforce_consents, " +
      "consent_hubs.created_at as consent_hubs_created_at, consent_hubs.created_by as consent_hubs_created_by, " +
      "consent_hubs.modified_at as consent_hubs_modified_at, " +
      "consent_hubs.modified_by as consent_hubs_modified_by, consent_hubs.created_by_uid as " +
      "consent_hubs_created_by_uid, consent_hubs.modified_by_uid as consent_hubs_modified_by_uid";
  ...
  private static final String CONSENT_MAPPING_SELECT_QUERY =
      " consents.id as consents_id, consents.user_id as consents_user_id, consents.consent_hub_id as " +
      "consents_consent_hub_id, " +
      "consents.status as consents_status, consents.created_at as consents_created_at, consents.created_by as " +
      "consents_created_by, consents.modified_by as consents_modified_by, consents.modified_at as " +
      "consents_modified_at, " +
      "consents.modified_by_uid as consents_modified_by_uid, consents.created_by_uid as consents_created_by_uid ";
  // Consent mapper
  private static final RowMapper<Consent> CONSENT_MAPPER = (rs, rowNum) -> {
  ...
  private static final RowMapper<Consent> CONSENT_MAPPER_WITHOUT_HUB = (rs, rowNum) -> {
    Consent c = new Consent();
    c.setId(rs.getInt("consents_id"));
    c.setUserId(rs.getInt("consents_user_id"));
  ...
  public void addFacility(PerunSession sess, ConsentHub consentHub, Facility facility) throws FacilityAlreadyAssigned {
    try {
      jdbc.update("INSERT INTO consent_hubs_facilities(" + "consent_hub_id, " + "facility_id, " + "created_at, " +
                  "created_by, " + "modified_at, " + "modified_by, " + "created_by_uid, " +
                  "modified_by_uid) VALUES(?,?," + Compatibility.getSysdate() + ",?," + Compatibility.getSysdate() +
                  ",?,?,?)", consentHub.getId(), facility.getId(), sess.getPerunPrincipal().getActor(),
  ...
  public void changeConsentStatus(PerunSession sess, Consent consent) {
    try {
      jdbc.update("UPDATE consents SET " + "status=?::consent_status , " + "modified_by=?, " + "modified_by_uid=?, " +
                  "modified_at= " + Compatibility.getSysdate() + " WHERE id=?", consent.getStatus().toString(),
          sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getUserId(), consent.getId());
    } catch (RuntimeException e) {
      throw new InternalErrorException(e);
    }
  }
  ...
  public Consent createConsent(PerunSession perunSession, Consent consent) throws ConsentExistsException {
    ...
    try {
      ...
      jdbc.update("insert into consents(id, user_id, consent_hub_id, status, created_at, created_by, modified_at, " +
                  "modified_by, created_by_uid, modified_by_uid) " + "values (?,?,?,?::consent_status," +
                  Compatibility.getSysdate() + ",?," + Compatibility.getSysdate() + ",?,?,?)", newId,
          consent.getUserId(), consent.getConsentHub().getId(), ConsentStatus.UNSIGNED.toString(),
  ...
  public ConsentHub createConsentHub(PerunSession sess, ConsentHub consentHub) {
    try {
      ...
      jdbc.update("insert into consent_hubs(id,name,enforce_consents,created_by,created_at,modified_by,modified_at," +
                  "created_by_uid,modified_by_uid) " + "values (?,?,?,?," + Compatibility.getSysdate() + ",?," +
                  Compatibility.getSysdate() + ",?,?)", id, consentHub.getName(), consentHub.isEnforceConsents(),
  ...
  public List<ConsentHub> getAllConsentHubs(PerunSession sess) {
    try {
      List<ConsentHub> consentHubs =
          jdbc.query("select " + CONSENT_HUB_MAPPING_SELECT_QUERY + " from consent_hubs", CONSENT_HUB_MAPPER);
  ...
  private List<AttributeDefinition> getAttrDefsForConsent(PerunSession sess, int consentId) {
    try {
      return jdbc.query("select " + AttributesManagerImpl.ATTRIBUTE_DEFINITION_MAPPING_SELECT_QUERY +
                        " from attr_names join consent_attr_defs on attr_names.id = consent_attr_defs.attr_id" +
                        " where consent_attr_defs.consent_id=?", AttributesManagerImpl.ATTRIBUTE_DEFINITION_MAPPER,
          consentId);
    } catch (EmptyResultDataAccessException ex) {
  ...
  public Consent getConsentById(PerunSession sess, int id) throws ConsentNotExistsException {
    try {
      Consent consent = jdbc.queryForObject(
          "select " + CONSENT_MAPPING_SELECT_QUERY + ", " + CONSENT_HUB_MAPPING_SELECT_QUERY +
          " from consents left join consent_hubs on consents.consent_hub_id=consent_hubs.id" + " where consents.id=? ",
          CONSENT_MAPPER, id);
      if (consent != null) {
  ...
  public Consent getConsentForUserAndConsentHub(PerunSession sess, int userId, int consentHubId, ConsentStatus status)
    ...
    try {
      Consent consent = jdbc.queryForObject("select " + CONSENT_MAPPING_SELECT_QUERY +
                                            " from consents where consents.user_id=? and consents.consent_hub_id=? " +
                                            "and consents" +
                                            ".status=?::consent_status ", CONSENT_MAPPER_WITHOUT_HUB, userId,
          consentHubId, status.toString());
  ...
  public ConsentHub getConsentHubByFacility(PerunSession sess, int facilityId) throws ConsentHubNotExistsException {
    try {
      ConsentHub consentHub = jdbc.queryForObject("select " + CONSENT_HUB_MAPPING_SELECT_QUERY +
                                                  " from consent_hubs join consent_hubs_facilities on consent_hubs" +
                                                  ".id=consent_hubs_facilities" +
                                                  ".consent_hub_id where consent_hubs_facilities.facility_id=?",
  ...
  public ConsentHub getConsentHubById(PerunSession sess, int id) throws ConsentHubNotExistsException {
    try {
      ConsentHub consentHub =
          jdbc.queryForObject("select " + CONSENT_HUB_MAPPING_SELECT_QUERY + " from consent_hubs where id=?",
  ...
  public ConsentHub getConsentHubByName(PerunSession sess, String name) throws ConsentHubNotExistsException {
    try {
      ConsentHub consentHub =
          jdbc.queryForObject("select " + CONSENT_HUB_MAPPING_SELECT_QUERY + " from consent_hubs where name=?",
  ...
  public List<ConsentHub> getConsentHubsByService(PerunSession session, int serviceId) {
    try {
      List<ConsentHub> consentHubs = jdbc.query("select " + CONSENT_HUB_MAPPING_SELECT_QUERY + " from consent_hubs" +
                                                " join consent_hubs_facilities on consent_hubs" +
                                                ".id=consent_hubs_facilities.consent_hub_id" +
                                                " join resources on resources.facility_id = consent_hubs_facilities" +
                                                ".facility_id" +
                                                " join resource_services on resource_services.resource_id = resources" +
                                                ".id" +
                                                " join services on services.id = resource_services.service_id" +
                                                " where services.id=?", CONSENT_HUB_MAPPER, serviceId);
  ...
  public List<Consent> getConsentsForConsentHub(PerunSession sess, int id, ConsentStatus status) {
    try {
      List<Consent> consents = jdbc.query("select " + CONSENT_MAPPING_SELECT_QUERY +
                                          " from consents where consents.consent_hub_id=? and consents" +
  ...
  public List<Consent> getConsentsForConsentHub(PerunSession sess, int id) {
    try {
      List<Consent> consents =
          jdbc.query("select " + CONSENT_MAPPING_SELECT_QUERY + " from consents where consents.consent_hub_id=? ",
  ...
  public List<Consent> getConsentsForUser(PerunSession sess, int id, ConsentStatus status) {
    try {
      List<Consent> consents = jdbc.query("select " + CONSENT_MAPPING_SELECT_QUERY + ", " +
                                          ...
                                          " from consents left join consent_hubs on consents" +
                                          ".consent_hub_id=consent_hubs.id " +
                                          " where consents.user_id=? and consents.status=?::consent_status ",
          CONSENT_MAPPER, id, status.toString());
      for (Consent consent : consents) {
        if (consent != null) {
          consent.getConsentHub().setFacilities(getFacilitiesForConsentHub(consent.getConsentHub()));
          consent.setAttributes(getAttrDefsForConsent(sess, consent.getId()));
  ...
  public List<Consent> getConsentsForUserAndConsentHub(PerunSession sess, int userId, int consentHubId) {
    try {
      List<Consent> consents =
          jdbc.query("select " + CONSENT_MAPPING_SELECT_QUERY + " from consents where user_id=? and consent_hub_id=?",
  ...
  public List<Facility> getFacilitiesForConsentHub(ConsentHub consentHub) {
    try {
      return jdbc.query("select " + FacilitiesManagerImpl.FACILITY_MAPPING_SELECT_QUERY +
                        " from facilities join consent_hubs_facilities on facilities.id=consent_hubs_facilities" +
                        ".facility_id" +
                        " where consent_hubs_facilities.consent_hub_id=?", FacilitiesManagerImpl.FACILITY_MAPPER,
          consentHub.getId());
    } catch (RuntimeException ex) {
      throw new InternalErrorException(ex);
    }
  ...
  public void updateConsentHub(PerunSession sess, ConsentHub consentHub) throws ConsentHubExistsException {
    try {
      jdbc.update(
          "update consent_hubs set name=?, enforce_consents=?, modified_by=?, modified_by_uid=?," + " modified_at=" +
          Compatibility.getSysdate() + " where id=?", consentHub.getName(), consentHub.isEnforceConsents(),
