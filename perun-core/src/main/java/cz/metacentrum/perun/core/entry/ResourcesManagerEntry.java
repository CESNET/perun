...
public class ResourcesManagerEntry implements ResourcesManager {
  ...
  public void activateGroupResourceAssignment(PerunSession sess, Group group, Resource resource, boolean async)
    ...
    // Authorization
    if (!AuthzResolver.authorizedInternal(sess, "activateGroupResourceAssignment_Group_Resource_boolean_policy", group,
        resource)) {
  ...
  @Override
  public void addResourceSelfServiceUser(PerunSession sess, Resource resource, User user)
    ...
    //Authorization
    if (!AuthzResolver.authorizedInternal(sess, "addResourceSelfServiceUser_Resource_User_policy",
  ...
  public void assignGroupToResource(PerunSession sess, Group group, Resource resource, boolean async,
    ...
    // Authorization
    if (!AuthzResolver.authorizedInternal(sess, "assignGroupToResource_Group_Resource_policy",
  ...
  public void assignGroupToResources(PerunSession perunSession, Group group, List<Resource> resources, boolean async,
    ...
    for (Resource resource : resources) {
      if (!AuthzResolver.authorizedInternal(perunSession, "assignGroupToResources_Group_List<Resource>_policy",
          resource, group)) {
        throw new PrivilegeException(perunSession, "assignGroupToResources");
      }
    }

    getResourcesManagerBl().assignGroupToResources(perunSession, group, resources, async, assignInactive,
        autoAssignSubgroups);
  }

  @Override
  public void assignGroupsToResource(PerunSession perunSession, List<Group> groups, Resource resource, boolean async,
                                     boolean assignInactive, boolean autoAssignSubgroups)
      throws PrivilegeException, GroupNotExistsException, ResourceNotExistsException, WrongAttributeValueException,
      WrongReferenceAttributeValueException, GroupResourceMismatchException {
    Utils.checkPerunSession(perunSession);
    ...
    for (Group group : groups) {
      if (!AuthzResolver.authorizedInternal(perunSession, "assignGroupsToResource_List<Group>_Resource_policy", group,
          resource)) {
        throw new PrivilegeException(perunSession, "assignGroupsToResource");
      }
    }

    getResourcesManagerBl().assignGroupsToResource(perunSession, groups, resource, async, assignInactive,
        autoAssignSubgroups);
  }
  ...
  public void assignResourceTagToResource(PerunSession perunSession, ResourceTag resourceTag, Resource resource)
    ...
    if (resourceTag.getVoId() != resource.getVoId()) {
      throw new ConsistencyErrorException("ResourceTag is from other Vo than Resource to which you want to assign it.");
    }

    // Authorization
    if (!AuthzResolver.authorizedInternal(perunSession, "assignResourceTagToResource_ResourceTag_Resource_policy",
        Arrays.asList(resource, resourceTag))) {
  ...
  public void assignResourceTagsToResource(PerunSession perunSession, List<ResourceTag> resourceTags, Resource resource)
    ...
    for (ResourceTag tag : resourceTags) {
      ...
      if (tag.getVoId() != resource.getVoId()) {
        throw new ConsistencyErrorException(
            "ResourceTag is from other Vo than Resource to which you want to assign it.");
      }
    }
  ...
  public void assignService(PerunSession sess, Resource resource, Service service)
    ...
    // Authorization
    if (!AuthzResolver.authorizedInternal(sess, "assignService_Resource_Service_policy",
        Arrays.asList(resource, service))) {
  ...
  public void copyAttributes(PerunSession sess, Resource sourceResource, Resource destinationResource)
    ...
    if (!AuthzResolver.authorizedInternal(sess, "source-copyAttributes_Resource_Resource_policy", sourceResource) ||
        !AuthzResolver.authorizedInternal(sess, "destination-copyAttributes_Resource_Resource_policy",
            destinationResource)) {
  ...
  public void copyGroups(PerunSession sess, Resource sourceResource, Resource destinationResource)
    ...
    for (Group group : perunBl.getResourcesManager().getAssignedGroups(sess, sourceResource)) {
      if (!AuthzResolver.authorizedInternal(sess, "source-copyGroups_Resource_Resource_policy", sourceResource,
          group)) {
        throw new PrivilegeException(sess, "copyGroups");
      }
    }
    if (!AuthzResolver.authorizedInternal(sess, "destination-copyGroups_Resource_Resource_policy",
        destinationResource)) {
      throw new PrivilegeException(sess, "copyGroups");
    }
  ...
  public Resource copyResource(PerunSession sess, Resource templateResource, Resource destinationResource,
    ...
    //Authorization
    if (!AuthzResolver.authorizedInternal(sess, "source-copyResource_Resource_Resource_boolean_policy",
        templateResource) ||
        !AuthzResolver.authorizedInternal(sess, "destination-copyResource_Resource_Resource_boolean_policy",
            destinationResource)) {
  ...
  public void copyServices(PerunSession sess, Resource sourceResource, Resource destinationResource)
    ...
    if (!AuthzResolver.authorizedInternal(sess, "source-copyServices_Resource_Resource_policy", sourceResource) ||
        !AuthzResolver.authorizedInternal(sess, "destination-copyServices_Resource_Resource_policy",
            destinationResource)) {
  ...
  public Resource createResource(PerunSession sess, Resource resource, Vo vo, Facility facility)
    ...
    // Authorization
    if (!AuthzResolver.authorizedInternal(sess, "createResource_Resource_Vo_Facility_policy",
        Arrays.asList(vo, facility))) {
  ...
  public void deactivateGroupResourceAssignment(PerunSession sess, Group group, Resource resource)
    ...
    // Authorization
    if (!AuthzResolver.authorizedInternal(sess, "deactivateGroupResourceAssignment_Group_Resource_policy", group,
        resource)) {
  ...
  public void deleteAllResourcesTagsForVo(PerunSession perunSession, Vo vo)
      throws PrivilegeException, VoNotExistsException, ResourceTagAlreadyAssignedException {
    Utils.notNull(perunSession, "perunSession");
    getPerunBl().getVosManagerBl().checkVoExists(perunSession, vo);
  ...
  public List<Resource> getAllResourcesByResourceTag(PerunSession perunSession, ResourceTag resourceTag)
    ...
    // Authorization
    if (!AuthzResolver.authorizedInternal(perunSession, "getAllResourcesByResourceTag_ResourceTag_policy",
        resourceTag)) {
  ...
  public List<Group> getAssignedGroups(PerunSession sess, Resource resource)
    ...
    assignedGroups.removeIf(
        assignedGroup -> !AuthzResolver.authorizedInternal(sess, "filter-getAssignedGroups_Resource_policy",
            assignedGroup, resource));
  ...
  public List<Group> getAssignedGroups(PerunSession sess, Resource resource, Member member)
    ...
    // Authorization
    if (!AuthzResolver.authorizedInternal(sess, "getAssignedGroups_Resource_Member_policy",
        Arrays.asList(resource, member))) {
      throw new PrivilegeException(sess, "getAssignedGroups");
    }
  ...
  public List<Resource> getAssignedResources(PerunSession sess, Group group)
    ...
    return getResourcesManagerBl().getAssignedResources(sess, group).stream().filter(
            resource -> AuthzResolver.authorizedInternal(sess, "filter-getAssignedResources_Group_policy", group,
                resource))
  ...
  public List<Resource> getAssignedResources(PerunSession sess, Member member, Service service)
    ...
    // Authorization
    if (!AuthzResolver.authorizedInternal(sess, "getAssignedResources_Member_Service_policy",
        Arrays.asList(member, service))) {
      throw new PrivilegeException(sess, "getAssignedResources");
    }
  ...
  public List<RichResource> getAssignedRichResources(PerunSession sess, Group group)
    ...
    return getResourcesManagerBl().getAssignedRichResources(sess, group).stream().filter(
        resource -> AuthzResolver.authorizedInternal(sess, "filter-getAssignedRichResources_Group_policy", group,
  ...
  public List<RichResource> getAssignedRichResources(PerunSession sess, Member member, Service service)
    ...
    // Authorization
    if (!AuthzResolver.authorizedInternal(sess, "getAssignedRichResources_Member_Service_policy",
        Arrays.asList(member, service))) {
      throw new PrivilegeException(sess, "getAssignedRichResources");
    }
  ...
  @Override
  public BanOnResource getBan(PerunSession sess, int memberId, int resourceId)
  ...
  @Override
  public BanOnResource getBanById(PerunSession sess, int banId)
  ...
  public List<BanOnResource> getBansForMember(PerunSession sess, int memberId)
    ...
    while (iterator.hasNext()) {
      ...
      if (!AuthzResolver.authorizedInternal(sess, "getBansForMember_int_policy",
          Arrays.asList(banForFiltering, resource, member))) {
        iterator.remove();
      }
    }
  ...
  @Override
  public List<EnrichedBanOnResource> getEnrichedBansForResource(PerunSession sess, int resourceId,
  ...
  @Override
  public List<EnrichedBanOnResource> getEnrichedBansForUser(PerunSession sess, int userId, List<String> attrNames)
  ...
  public EnrichedResource getEnrichedResourceById(PerunSession sess, int id, List<String> attrNames)
    ...
    EnrichedResource enrichedResource = resourcesManagerBl.getEnrichedResourceById(sess, id, attrNames);
    ...
    return resourcesManagerBl.filterOnlyAllowedAttributes(sess, enrichedResource);
  }

  @Override
  public List<EnrichedResource> getEnrichedResourcesForFacility(PerunSession sess, Facility facility,
                                                                List<String> attrNames)
    ...
    return getResourcesManagerBl().getEnrichedRichResourcesForFacility(sess, facility, attrNames).stream().filter(
            enrichedResource -> AuthzResolver.authorizedInternal(sess,
                "filter" + "-getEnrichedResourcesForFacility_Facility_policy",
                facility, enrichedResource.getResource()))
        .map(enrichedResource -> getResourcesManagerBl().filterOnlyAllowedAttributes(sess, enrichedResource))
        .collect(Collectors.toList());
  }

  @Override
  public List<EnrichedResource> getEnrichedResourcesForVo(PerunSession sess, Vo vo, List<String> attrNames)
    ...
    return getResourcesManagerBl().getEnrichedRichResourcesForVo(sess, vo, attrNames).stream().filter(
            enrichedResource -> AuthzResolver.authorizedInternal(sess, "filter-getEnrichedResourcesForVo_Vo_policy", vo,
                enrichedResource.getResource()))
        .map(enrichedResource -> getResourcesManagerBl().filterOnlyAllowedAttributes(sess, enrichedResource))
        .collect(Collectors.toList());
  }
  ...
  public List<AssignedGroup> getGroupAssignments(PerunSession sess, Resource resource, List<String> attrNames)
    ...
    List<AssignedGroup> filteredGroups = getResourcesManagerBl().getGroupAssignments(sess, resource, attrNames).stream()
        .filter(assignedGroup -> AuthzResolver.authorizedInternal(sess, "filter-getGroupAssignments_Resource_policy",
            assignedGroup.getEnrichedGroup().getGroup(), resource)).collect(Collectors.toList());

    filteredGroups.forEach(assignedGroup -> assignedGroup.setEnrichedGroup(
        getPerunBl().getGroupsManagerBl().filterOnlyAllowedAttributes(sess, assignedGroup.getEnrichedGroup())));
  ...
  public List<AssignedResource> getResourceAssignments(PerunSession sess, Group group, List<String> attrNames)
    ...
    List<AssignedResource> filteredResources =
        getResourcesManagerBl().getResourceAssignments(sess, group, attrNames).stream().filter(
            assignedResource -> AuthzResolver.authorizedInternal(sess, "filter-getResourceAssignments_Group_policy",
                assignedResource.getEnrichedResource().getResource(), group)).collect(Collectors.toList());

    filteredResources.forEach(assignedResource -> assignedResource.setEnrichedResource(
        getResourcesManagerBl().filterOnlyAllowedAttributes(sess, assignedResource.getEnrichedResource())));
  ...
  public Resource getResourceByName(PerunSession sess, Vo vo, Facility facility, String name)
    ...
    // Authorization
    if (!AuthzResolver.authorizedInternal(sess, "getResourceByName_Vo_Facility_String_policy",
        Arrays.asList(resource, vo, facility))) {
  ...
  public List<Resource> getResourcesByIds(PerunSession sess, List<Integer> ids) throws PrivilegeException {
    ...
    resources.removeIf(
        resource -> !AuthzResolver.authorizedInternal(sess, "filter-getResourcesByIds_List<Integer>_policy", resource));
  ...
  public List<Resource> getResourcesWhereGroupIsAdmin(PerunSession sess, Facility facility, Vo vo,
    ...
    //Authorization
    if (!AuthzResolver.authorizedInternal(sess, "getResourcesWhereGroupIsAdmin_Facility_Vo_Group_policy",
        Arrays.asList(facility, vo)) && !AuthzResolver.authorizedInternal(sess,
        "authorizedGroup-getResourcesWhereGroupIsAdmin_Facility_Vo_Group_policy", authorizedGroup)) {
      throw new PrivilegeException(sess, "getResourcesByResourceManager");
    }
    List<Resource> resources =
        getResourcesManagerBl().getResourcesWhereGroupIsAdmin(sess, facility, vo, authorizedGroup);
    resources.removeIf(resource ->
        !AuthzResolver.authorizedInternal(sess, "filter-getResourcesWhereGroupIsAdmin_Facility_Vo_Group_policy",
            Arrays.asList(vo, resource, facility)) && !AuthzResolver.authorizedInternal(sess,
            "filter_authorizedGroup-getResourcesWhereGroupIsAdmin_Facility_Vo_Group_policy", authorizedGroup));
  ...
  public List<Resource> getResourcesWhereUserIsAdmin(PerunSession sess, Facility facility, Vo vo, User authorizedUser)
    ...
    //Authorization
    if (!AuthzResolver.authorizedInternal(sess, "getResourcesWhereUserIsAdmin_Facility_Vo_User_policy", facility, vo,
        authorizedUser)) {
      throw new PrivilegeException(sess, "getResourcesByResourceManager");
    }
    List<Resource> resources = getResourcesManagerBl().getResourcesWhereUserIsAdmin(sess, facility, vo, authorizedUser);
    resources.removeIf(resource -> !AuthzResolver.authorizedInternal(sess,
        "filter-getResourcesWhereUserIsAdmin_Facility_Vo_User_policy",
        Arrays.asList(vo, facility, resource, authorizedUser)));
  ...
  public List<Resource> getResourcesWhereUserIsAdmin(PerunSession sess, Vo vo, User authorizedUser)
    ...
    resources.removeIf(
        resource -> !AuthzResolver.authorizedInternal(sess, "filter-getResourcesWhereUserIsAdmin_Vo_User_policy",
            Arrays.asList(vo, resource, authorizedUser)));
  ...
  public List<RichUser> getRichAdmins(PerunSession perunSession, Resource resource, List<String> specificAttributes,
    ...
    // Authorization
    if (!AuthzResolver.authorizedInternal(perunSession, "getRichAdmins_Resource_List<String>_boolean_boolean_policy",
        resource)) {
    ...
    return getPerunBl().getUsersManagerBl().filterOnlyAllowedAttributes(perunSession,
        getResourcesManagerBl().getRichAdmins(perunSession, resource, specificAttributes, allUserAttributes,
            onlyDirectAdmins));
  }

  @Override
  public RichResource getRichResourceById(PerunSession sess, int id)
      throws PrivilegeException, ResourceNotExistsException {
    Utils.checkPerunSession(sess);
  ...
  public List<RichResource> getRichResourcesByIds(PerunSession sess, List<Integer> ids) throws PrivilegeException {
    ...
    richResources.removeIf(
        richResource -> !AuthzResolver.authorizedInternal(sess, "filter-getRichResourcesByIds_List<Integer>_policy",
            richResource));
  ...
  @Override
  public void removeAdmin(PerunSession sess, Resource resource, Group group)
  ...
  public void removeAllResourcesTagFromResource(PerunSession perunSession, Resource resource)
    ...
    // Authorization
    if (!AuthzResolver.authorizedInternal(perunSession, "removeAllResourcesTagFromResource_Resource_policy",
        resource)) {
  ...
  @Override
  public void removeBan(PerunSession sess, int memberId, int resourceId)
      throws BanNotExistsException, PrivilegeException, ResourceNotExistsException {
    Utils.checkPerunSession(sess);
  ...
  public void removeGroupFromResource(PerunSession sess, Group group, Resource resource)
    ...
    // Authorization
    if (!AuthzResolver.authorizedInternal(sess, "removeGroupFromResource_Group_Resource_policy",
        Arrays.asList(group, resource))) {
  ...
  public void removeGroupFromResources(PerunSession perunSession, Group group, List<Resource> resources)
    ...
    // Authorization
    for (Resource resource : resources) {
      if (!AuthzResolver.authorizedInternal(perunSession, "removeGroupFromResources_Group_List<Resource>_policy",
          resource, group)) {
  ...
  public void removeGroupsFromResource(PerunSession perunSession, List<Group> groups, Resource resource)
    ...
    // Authorization
    for (Group group : groups) {
      if (!AuthzResolver.authorizedInternal(perunSession, "removeGroupsFromResource_List<Group>_Resource_policy", group,
          resource)) {
  ...
  public void removeResourceSelfServiceUser(PerunSession sess, Resource resource, User user)
    ...
    //Authorization
    if (!AuthzResolver.authorizedInternal(sess, "removeResourceSelfServiceUser_Resource_User_policy",
        Arrays.asList(resource, user))) {
  ...
  public void removeResourceTagFromResource(PerunSession perunSession, ResourceTag resourceTag, Resource resource)
    ...
    if (resourceTag.getVoId() != resource.getVoId()) {
      throw new ConsistencyErrorException(
          "ResourceTag is from other Vo than Resource to which you want to remove from.");
    }

    // Authorization
    if (!AuthzResolver.authorizedInternal(perunSession, "removeResourceTagFromResource_ResourceTag_Resource_policy",
        Arrays.asList(resource, resourceTag))) {
  ...
  @Override
  public void removeService(PerunSession sess, Resource resource, Service service)
    ...
    // Authorization
    if (!AuthzResolver.authorizedInternal(sess, "removeService_Resource_Service_policy",
        Arrays.asList(resource, service))) {
  ...
  @Override
  public void removeService(PerunSession perunSession, List<Resource> resources, Service service)
      throws PrivilegeException, ResourceNotExistsException, ServiceNotExistsException, ServiceNotAssignedException,
    ...
    for (Resource resource : resources) {
      ...
      // Authorization
      if (!AuthzResolver.authorizedInternal(perunSession, "removeService_Resource_Service_policy",
          Arrays.asList(resource, service))) {
        throw new PrivilegeException(perunSession, "removeServices");
      }

      if (facilityId != resource.getFacilityId()) {
        throw new FacilityMismatchException("Resources " + resource.getName() + " and " + resources.get(0).getName() +
                                            " are from different facilities!");
      }
  ...
  public BanOnResource updateBan(PerunSession sess, BanOnResource banOnResource)
    ...
    // Authorization
    if (!AuthzResolver.authorizedInternal(sess, "updateBan_BanOnResource_policy",
        Arrays.asList(banOnResource, member, resource))) {
