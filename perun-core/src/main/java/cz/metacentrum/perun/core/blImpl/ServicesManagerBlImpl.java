...
public class ServicesManagerBlImpl implements ServicesManagerBl {
    ...
    if (!getServicesManagerImpl().destinationExists(sess, destination)) {
      try {
        ...
        Destination existingDestination =
            getServicesManagerImpl().getDestination(sess, destination.getDestination(), destination.getType());
  ...
  public Destination addDestination(PerunSession perunSession, List<Service> services, Facility facility,
    ...
    if (!getServicesManagerImpl().destinationExists(perunSession, destination)) {
      try {
        //Try to get the destination without id
        Destination existingDestination =
            getServicesManagerImpl().getDestination(perunSession, destination.getDestination(), destination.getType());
        // pass new propagation type from API to object retrieved from DB,
        // since it always contains PARALLEL type
        existingDestination.setPropagationType(destination.getPropagationType());
        destination = existingDestination;
  ...
  private Destination addDestinationEvenIfAlreadyExists(PerunSession sess, Service service, Facility facility,
                                                        Destination destination) {
    if (!getServicesManagerImpl().destinationExists(sess, destination)) {
      try {
        //Try to get the destination without id
        ...
        try {
          ...
          try {
            ...
            throw new ConsistencyErrorException(
                "Destination seems to exists and not exists in the same time. There is some other problem." + exep);
          }
        }
      }
    }
  ...
  /**
   * Checks, that adding new required attribute to service which would require renewing user consents (user-related
   * attributes on consents-requiring instances) is done for globally disabled service. Does not throw exception, if all
   * facilities with given service are excluded from consents logic.
   *
   * @throws ServiceAttributesCannotExtend if trying to add attribute which would invalidate consents
   */
  private void checkCanAddAttribute(PerunSession sess, Service service, AttributeDefinition attribute)
    ...
    List<Integer> facilitiesIds =
        perunBl.getServicesManagerBl().getAssignedResources(sess, service).stream().map(Resource::getFacilityId)
            .distinct().toList();
  ...
  public void deleteDestination(PerunSession sess, Destination destination)
    ...
    List<TaskResult> taskResults = getPerunBl().getTasksManagerBl()
        .getTaskResultsByDestinations(sess, Collections.singletonList(destination.getDestination()));
   ...
   */
  @Override
  public void deleteService(PerunSession perunSession, Service service, boolean forceFlag)
    ...
    if (forceFlag) {
      ...
      for (Resource resource : assignedResources) {
        try {
          ...
          Facility facility =
              getPerunBl().getFacilitiesManagerBl().getFacilityById(perunSession, resource.getFacilityId());
  ...
  public void deleteServicesPackage(PerunSession sess, ServicesPackage servicesPackage) throws RelationExistsException {
    if (getServicesFromServicesPackage(sess, servicesPackage).isEmpty()) {
      ...
      throw new RelationExistsException(
          "There is one or more services in the services package. ServicesPackage=\"" + servicesPackage + "\"");
    }
  }
  ...
  public boolean forceServicePropagation(PerunSession sess, Facility facility, Service service) {
    ...
    //Local
    if (getServicesManagerImpl().isServiceBlockedOnFacility(service.getId(), facility.getId())) {
      return false;
    }
  ...
  public List<ServiceForGUI> getFacilityAssignedServicesForGUI(PerunSession perunSession, Facility facility) {
    ...
    for (Service service : services) {
      ...
      newService.setAllowedOnFacility(
          !getServicesManagerImpl().isServiceBlockedOnFacility(service.getId(), facility.getId()));
  ...
  public HashedGenData getHashedDataWithGroups(PerunSession sess, Service service, Facility facility,
    ...
    HashedDataGenerator hashedDataGenerator =
        new GroupsHashedDataGenerator.Builder().sess((PerunSessionImpl) sess).service(service).facility(facility)
            .filterExpiredMembers(!service.isUseExpiredMembers()).consentEval(consentEval).taskRunId(taskRunId).build();
  ...
  public boolean planServicePropagation(PerunSession perunSession, Facility facility, Service service) {
    ...
    //Call log method out of transaction
    perunSession.getPerun().getAuditer()
        .log(perunSession, new PropagationPlannedOnFacilityAndService(facility, service));
    return true;
  }
