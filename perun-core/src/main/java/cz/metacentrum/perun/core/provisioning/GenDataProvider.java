 ...
 * <p>
 * Attributes, that can be hashed, must be loaded first via load* methods. E.g.: to be able to call
 * getResourceAttributesHashes, one must first call loadResourceSpecificAttributes.
 * <p>
 * IMPORTANT: this components has a STATE! The order of load methods is important. E.g.: loadResourceSpecificAttributes
 * will overwrite data for previously loaded resource.
 *
...
public interface GenDataProvider {
  ...
  /**
   * Returns map of all loaded attributes grouped by their hashes. Returns only non-empty lists, and only lists, for
   * which their hashes has been returned, by some get.*attributesHashes method.
   *
  ...
  /**
   * Return all hashes for given resource attributes. If addVoAttributes is true, also adds a hash for resource's vo
   * attributes, if they are not empty.
   *
  ...
  /**
   * Loads Group and Group-Resource attributes. Group attributes are loaded only for groups that has not been already
   * loaded.
   *
   * @param resource resource
   * @param groups   groups
  ...
  /**
   * Loads Resource and Member specific attributes. Resouce, Member, User, User-Facility (if not already loaded).
   * Resource-Member (always) Vo - if specified by loadVoAttributes
   *
   * @param resource         resource
   * @param members          members
