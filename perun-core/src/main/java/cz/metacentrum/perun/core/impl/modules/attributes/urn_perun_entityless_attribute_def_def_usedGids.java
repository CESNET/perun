 ...
 */
public class urn_perun_entityless_attribute_def_def_usedGids extends EntitylessAttributesModuleAbstract
    ...
    for (String mapKey : mapKeys) {
      ...
      if (!keyMatcher.matches()) {
        throw new WrongAttributeValueException(attribute, key,
            "Key in usedGids can be only in format 'Rx', 'Gx', 'Dx' where 'x' is positive integer.");
      }

      //Test value
      String value = map.get(mapKey);
      if (value == null) {
        throw new WrongAttributeValueException(attribute, key, "Value in usedGids can't be null.");
      }
