...
public class urn_perun_member_resource_attribute_def_virt_isBanned extends MemberResourceVirtualAttributesModuleAbstract
    implements MemberResourceVirtualAttributesModuleImplApi {

  private static final String A_MR_V_isBanned = AttributesManager.NS_MEMBER_RESOURCE_ATTR_VIRT + ":isBanned";
  private static final Logger LOG =
      LoggerFactory.getLogger(urn_perun_member_resource_attribute_def_virt_isBanned.class);
   ...
   */
  private List<Pair<Resource, Member>> getAffectedMemberResourceObjects(PerunSessionImpl perunSession, User user,
                                                                        Facility facility) {
    ...
    for (Resource resource : assignedResources) {
      List<Member> assignedMembers =
          perunSession.getPerunBl().getResourcesManagerBl().getAssignedMembers(perunSession, resource);
  ...
  private List<AuditEvent> resolveBanChangedForFacility(PerunSessionImpl perunSession, int userId, int facilityId) {
    ...
    try {
      ...
      List<Pair<Resource, Member>> listOfAffectedObjects =
          getAffectedMemberResourceObjects(perunSession, user, facility);

      for (Pair<Resource, Member> affectedObjects : listOfAffectedObjects) {
        try {
          AttributeDefinition attributeDefinition =
              perunSession.getPerunBl().getAttributesManagerBl().getAttributeDefinition(perunSession, A_MR_V_isBanned);
          resolvingMessages.add(
              new AttributeChangedForResourceAndMember(new Attribute(attributeDefinition), affectedObjects.getLeft(),
                  affectedObjects.getRight()));
  ...
  private List<AuditEvent> resolveBanChangedForResource(PerunSessionImpl perunSession, int memberId, int resourceId)
    ...
    try {
      ...
      AttributeDefinition attributeDefinition =
          perunSession.getPerunBl().getAttributesManagerBl().getAttributeDefinition(perunSession, A_MR_V_isBanned);
      resolvingMessages.add(
          new AttributeChangedForResourceAndMember(new Attribute(attributeDefinition), resource, member));
  ...
  public List<AuditEvent> resolveVirtualAttributeValueChange(PerunSessionImpl perunSession, AuditEvent message)
    ...
    if (message instanceof BanSetForResource) {
      return resolveBanChangedForResource(perunSession, ((BanSetForResource) message).getMemberId(),
          ((BanSetForResource) message).getResourceId());

    } else if (message instanceof BanRemovedForResource) {
      return resolveBanChangedForResource(perunSession, ((BanRemovedForResource) message).getMemberId(),
          ((BanRemovedForResource) message).getResourceId());

    } else if (message instanceof BanUpdatedForResource) {
      return resolveBanChangedForResource(perunSession, ((BanUpdatedForResource) message).getMemberId(),
          ((BanUpdatedForResource) message).getResourceId());

    } else if (message instanceof BanSetForFacility) {
      return resolveBanChangedForFacility(perunSession, ((BanSetForFacility) message).getUserId(),
          ((BanSetForFacility) message).getFacilityId());

    } else if (message instanceof BanRemovedForFacility) {
      return resolveBanChangedForFacility(perunSession, ((BanRemovedForFacility) message).getUserId(),
          ((BanRemovedForFacility) message).getFacilityId());

    } else if (message instanceof BanUpdatedForFacility) {
      return resolveBanChangedForFacility(perunSession, ((BanUpdatedForFacility) message).getUserId(),
