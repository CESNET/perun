...
public class urn_perun_member_resource_attribute_def_virt_fileQuotas
    extends MemberResourceVirtualAttributesModuleAbstract {
  ...
  public static final String A_MR_fileQuotasOverride =
      AttributesManager.NS_MEMBER_RESOURCE_ATTR_DEF + ":fileQuotasOverride";
  ...
  public Attribute getAttributeValue(PerunSessionImpl sess, Member member, Resource resource,
    ...
    if (resourceQuotas == null || resourceQuotas.getValue() == null) {
      ...
      try {
        resourceTransferedQuotas =
            sess.getPerunBl().getModulesUtilsBl().checkAndTransferQuotas(resourceQuotas, resource, null, false);
    ...
    if (memberQuotas == null || memberQuotas.getValue() == null) {
      ...
      try {
        memberTransferedQuotas =
            sess.getPerunBl().getModulesUtilsBl().checkAndTransferQuotas(memberQuotas, resource, member, false);
      } catch (WrongAttributeValueException ex) {
        throw new ConsistencyErrorException(
            "Quotas on resource " + resource + " for member " + member + " are in bad format.", ex);
      }
    }
    ...
    try {
      memberQuotasOverride =
          sess.getPerunBl().getAttributesManagerBl().getAttribute(sess, member, resource, A_MR_fileQuotasOverride);
    } catch (AttributeNotExistsException ex) {
      throw new ConsistencyErrorException(ex);
    } catch (WrongAttributeAssignmentException | MemberResourceMismatchException ex) {
      throw new InternalErrorException(ex);
    ...
    if (memberQuotasOverride == null || memberQuotasOverride.getValue() == null) {
      ...
      try {
        memberTransferedQuotasOverride =
            sess.getPerunBl().getModulesUtilsBl().checkAndTransferQuotas(memberQuotasOverride, resource, member, false);
      } catch (WrongAttributeValueException ex) {
        throw new ConsistencyErrorException(
            "Override quotas on resource " + resource + " for member " + member + " are in bad format.", ex);
      }
    }

    //Merge quotas for member on resource
    Map<String, Pair<BigDecimal, BigDecimal>> finalMemberResourceQuotas = sess.getPerunBl().getModulesUtilsBl()
        .mergeMemberAndResourceTransferredQuotas(resourceTransferedQuotas, memberTransferedQuotas,
            memberTransferedQuotasOverride);

    //set final value to attribute
    attribute.setValue(
        sess.getPerunBl().getModulesUtilsBl().transferQuotasBackToAttributeValue(finalMemberResourceQuotas, false));
  ...
  public AttributeDefinition getAttributeDefinition() {
    ...
    attr.setDescription(
        "Every record is the path (to volume) and the quota in format 'SoftQuota:HardQuota' in (M, G, T, ...), G is default. Example: '10G:20T'. For every volume count final value of file quotas for this member on this resource.");
    return attr;
  }
}
