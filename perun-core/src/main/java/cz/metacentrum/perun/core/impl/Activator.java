 ...
 */
public class Activator implements ApplicationListener<ContextRefreshedEvent> {
	...
	public Activator(PerunBl perunBl) {
		this.perunBl = perunBl;
		this.sess = perunBl.getPerunSession(
			new PerunPrincipal("perunActivator", ExtSourcesManager.EXTSOURCE_NAME_INTERNAL, ExtSourcesManager.EXTSOURCE_INTERNAL),
			new PerunClient());
	}
	...
	private void activateGroupResourceAssignments() {
		try {
			log.debug("Activator starting to activate group-resource assignments in PROCESSING or FAILED state.");
