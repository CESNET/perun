...
/**
 * Get all records (as list) of attribute group:def:sponsorOrganizationIdentifier from any Group where user is valid
 * Member.
 *
...
public class urn_perun_user_attribute_def_virt_sponsoredMembershipInOrganizations
  ...
  /**
   * Return attribute organization identifier for group. Return null if attribute definition does not exist.
   *
   * @param sess
  ...
  private List<String> getSponsorOrganizationIdentifiersFromGroups(PerunSessionImpl sess, User user) {
    ...
    return membersManagerBl.getMembersByUserWithStatus(sess, user, Status.VALID).stream()
        ...
        .filter(Objects::nonNull).map(Attribute::valueAsString).filter(Objects::nonNull).distinct()
