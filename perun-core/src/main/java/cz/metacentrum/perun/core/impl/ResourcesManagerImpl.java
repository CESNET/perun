...
public class ResourcesManagerImpl implements ResourcesManagerImplApi {
  ...
  protected static final String RESOURCE_MAPPING_SELECT_QUERY =
      "resources.id as resources_id, resources.uu_id as resources_uu_id," +
      " resources.facility_id as resources_facility_id, " +
      "resources.name as resources_name, resources.dsc as resources_dsc, resources.vo_id as resources_vo_id, " +
      "resources.created_at as resources_created_at, resources.created_by as resources_created_by," +
      " resources.modified_by as resources_modified_by, resources.modified_at as resources_modified_at," +
      " resources.modified_by_uid as resources_modified_by_uid," +
      " resources.created_by_uid as resources_created_by_uid";
  protected static final String ASSIGNED_RESOURCE_MAPPING_SELECT_QUERY =
      RESOURCE_MAPPING_SELECT_QUERY + ", groups_resources_state.status as groups_resources_state_status," +
      " groups_resources_state.failure_cause as groups_resources_state_failure_cause" +
      ", groups_resources_automatic.source_group_id as groups_resources_automatic_source_group_id," +
      " groups_resources_automatic.auto_assign_subgroups as auto_assign_subgroups, " +
      FacilitiesManagerImpl.FACILITY_MAPPING_SELECT_QUERY;
  protected static final String GROUP_RESOURCE_ASSIGNMENT_MAPPING_SELECT_QUERY =
      RESOURCE_MAPPING_SELECT_QUERY + ", groups_resources_state.status as groups_resources_state_status," +
      " groups_resources_state.failure_cause as groups_resources_state_failure_cause" + ", " +
      GroupsManagerImpl.GROUP_MAPPING_SELECT_QUERY;
  protected static final String RESOURCE_TAG_MAPPING_SELECT_QUERY =
      "res_tags.id as res_tags_id, res_tags.vo_id as res_tags_vo_id, res_tags.tag_name as res_tags_tag_name, " +
      "res_tags.created_at as res_tags_created_at, res_tags.created_by as res_tags_created_by," +
      " res_tags.modified_by as res_tags_modified_by, " +
      "res_tags.modified_at as res_tags_modified_at, res_tags.modified_by_uid as res_tags_modified_by_uid," +
      " res_tags.created_by_uid as res_tags_created_by_uid";
  protected static final String BAN_ON_RESOURCE_MAPPING_SELECT_QUERY =
      "resources_bans.id as res_bans_id, resources_bans.description as res_bans_description, " +
      "resources_bans.member_id as res_bans_member_id, resources_bans.resource_id as res_bans_resource_id," +
      " resources_bans.banned_to as res_bans_validity_to, " +
      "resources_bans.created_at as res_bans_created_at, resources_bans.created_by as res_bans_created_by, " +
      "resources_bans.modified_at as res_bans_modified_at, " + "resources_bans.modified_by as res_bans_modified_by," +
      " resources_bans.created_by_uid as res_bans_created_by_uid," +
      " resources_bans.modified_by_uid as res_bans_modified_by_uid";
  ...
  static final Logger LOG = LoggerFactory.getLogger(ResourcesManagerImpl.class);
  ...
  public void assignAutomaticGroupToResource(PerunSession sess, Group group, Resource resource, Group sourceGroup)
    ...
    try {
      if (1 == jdbc.queryForInt(
        ...
        jdbc.update("insert into groups_resources_automatic (group_id, resource_id, source_group_id, modified_by," +
                    " modified_at, created_by, created_at, created_by_uid, modified_by_uid) " + "values (?,?,?,?," +
  ...
  public void assignGroupToResource(PerunSession sess, Group group, Resource resource, boolean autoAssignSubgroups)
    ...
    try {
      if (1 ==
        ...
        jdbc.update("insert into groups_resources (group_id, resource_id, modified_by, modified_at, created_by," +
                    " created_at, created_by_uid, modified_by_uid, auto_assign_subgroups) " + "values (?,?,?," +
                    Compatibility.getSysdate() + ",?," + Compatibility.getSysdate() + ",?,?,?)", group.getId(),
            resource.getId(), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getActor(),
            sess.getPerunPrincipal().getUserId(), sess.getPerunPrincipal().getUserId(), autoAssignSubgroups);
      }
    } catch (RuntimeException ex) {
      throw new InternalErrorException(ex);
    }
  ...
  public void assignService(PerunSession sess, Resource resource, Service service)
    ...
    try {
      ...
      jdbc.update("insert into resource_services(service_id, resource_id, created_by,created_at,modified_by," +
                  "modified_at,created_by_uid, modified_by_uid) " + "values (?,?,?," + Compatibility.getSysdate() +
                  ",?," + Compatibility.getSysdate() + ",?,?)", service.getId(), resource.getId(),
  ...
  public Resource createResource(PerunSession sess, Vo vo, Resource resource, Facility facility) {
    ...
    try {
      ...
      jdbc.update("insert into resources(id, name, dsc, facility_id, vo_id, created_by, created_at,modified_by," +
                  "modified_at,created_by_uid,modified_by_uid) " + "values (?,?,?,?,?,?," + Compatibility.getSysdate() +
                  ",?," + Compatibility.getSysdate() + ",?,?)", newId, resource.getName(), resource.getDescription(),
          facility.getId(), vo.getId(), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getActor(),
          sess.getPerunPrincipal().getUserId(), sess.getPerunPrincipal().getUserId());
    } catch (RuntimeException ex) {
      throw new InternalErrorException(ex);
    }
  ...
  public ResourceTag createResourceTag(PerunSession perunSession, ResourceTag resourceTag, Vo vo) {
    try {
      ...
      jdbc.update("insert into res_tags(id, vo_id, tag_name, created_by, created_at,modified_by,modified_at," +
                  "created_by_uid,modified_by_uid) " + "values (?,?,?,?," + Compatibility.getSysdate() + ",?," +
                  Compatibility.getSysdate() + ",?,?)", newId, vo.getId(), resourceTag.getTagName(),
    ...
    try {
      ...
      for (Group authorizedGroup : listOfGroupAdmins) {
        setOfAdmins.addAll(jdbc.query("select " + UsersManagerImpl.USER_MAPPING_SELECT_QUERY +
                                      ...
                                      " and groups_members.source_group_status=? where groups_members.group_id=? and " +
                                      "members.status=?", UsersManagerImpl.USER_MAPPER,
            MemberGroupStatus.VALID.getCode(), authorizedGroup.getId(), Status.VALID.getCode()));
      }
  ...
  public List<BanOnResource> getAllExpiredBansOnResources(PerunSession sess) {
    try {
      return jdbc.query("select " + BAN_ON_RESOURCE_MAPPING_SELECT_QUERY + " from resources_bans where banned_to < " +
  ...
  public List<Resource> getAllResources(PerunSession sess) {
    try {
      return jdbc.query("select " + RESOURCE_MAPPING_SELECT_QUERY + " from resources", RESOURCE_MAPPER);
    } catch (RuntimeException e) {
      throw new InternalErrorException(e);
    }
  }
  ...
  public List<Resource> getAllResourcesByResourceTag(PerunSession perunSession, ResourceTag resourceTag) {
    try {
      return jdbc.query("select " + RESOURCE_MAPPING_SELECT_QUERY + " from resources" +
                        " join tags_resources on resources.id=tags_resources.resource_id" +
                        " where tags_resources.tag_id=?", RESOURCE_MAPPER, resourceTag.getId());
    } catch (EmptyResultDataAccessException e) {
      return new ArrayList<>();
    } catch (RuntimeException e) {
      throw new InternalErrorException(e);
  ...
  public List<ResourceTag> getAllResourcesTagsForResource(PerunSession perunSession, Resource resource) {
    try {
      return jdbc.query("select " + RESOURCE_TAG_MAPPING_SELECT_QUERY + " from tags_resources" +
                        " join res_tags on tags_resources.tag_id=res_tags.id" + " where tags_resources.resource_id=?",
  ...
  public List<ResourceTag> getAllResourcesTagsForVo(PerunSession perunSession, Vo vo) {
    try {
      return jdbc.query(
          "select " + RESOURCE_TAG_MAPPING_SELECT_QUERY + " from vos" + " join res_tags on vos.id=res_tags.vo_id" +
          " where res_tags.vo_id=?", RESOURCE_TAG_MAPPER, vo.getId());
    } catch (EmptyResultDataAccessException e) {
      return new ArrayList<>();
    } catch (RuntimeException e) {
      throw new InternalErrorException(e);
  ...
  public List<Member> getAllowedMembers(PerunSession sess, Resource resource) {
    try {
      return jdbc.query(
          "select distinct " + MembersManagerImpl.MEMBER_MAPPING_SELECT_QUERY + " from groups_resources_state" +
          ...
          " where groups_resources_state.resource_id=? and members.status!=? and members.status!=?", MEMBER_MAPPER,
          GroupResourceStatus.ACTIVE.toString(), resource.getId(), Status.INVALID.getCode(), Status.DISABLED.getCode());
    } catch (EmptyResultDataAccessException e) {
      return new ArrayList<>();
    } catch (RuntimeException e) {
      throw new InternalErrorException(e);
  ...
  public List<Member> getAllowedMembersNotExpiredInGroup(PerunSession sess, Resource resource) {
    try {
      ...
      return jdbc.query(
          ...
          " join groups on groups_resources_state.group_id=groups.id" +
          " join groups_members on groups.id=groups_members.group_id" +
          " join members on groups_members.member_id=members.id " + " where groups_resources_state.resource_id=?" +
          " and groups_resources_state.status=?::group_resource_status " +
          ...
          GroupResourceStatus.ACTIVE.toString(), Status.INVALID.getCode(), Status.DISABLED.getCode(),
  ...
  public List<Resource> getAllowedResources(PerunSession sess, Facility facility, User user) {
    try {
      return jdbc.query("select distinct " + ResourcesManagerImpl.RESOURCE_MAPPING_SELECT_QUERY + " from resources" +
                        ...
                        " where resources.facility_id=? and members.user_id=? and members.status!=? and members" +
                        ".status!=?", RESOURCE_MAPPER, GroupResourceStatus.ACTIVE.toString(), facility.getId(),
          user.getId(), Status.INVALID.getCode(), Status.DISABLED.getCode());
    } catch (EmptyResultDataAccessException e) {
      return new ArrayList<>();
    } catch (RuntimeException e) {
      throw new InternalErrorException(e);
  ...
  public List<User> getAllowedUsers(PerunSession sess, Resource resource) {
    try {
      return jdbc.query("select distinct " + UsersManagerImpl.USER_MAPPING_SELECT_QUERY +
                        ...
                        ".status=?::group_resource_status" + " and members.status!=? and members.status!=?",
  ...
  public List<User> getAllowedUsersNotExpiredInGroup(PerunSession sess, Resource resource) {
    try {
      return jdbc.query("select distinct " + UsersManagerImpl.USER_MAPPING_SELECT_QUERY +
                        ...
                        " join groups_members on groups.id=groups_members.group_id" +
                        " join members on groups_members.member_id=members.id" +
                        " join users on users.id=members.user_id" + " where groups_resources_state.resource_id=?" +
                        " and groups_resources_state.status=?::group_resource_status " +
                        " and members.status!=? and members.status!=? and groups_members.source_group_status =?",
          UsersManagerImpl.USER_MAPPER, resource.getId(), GroupResourceStatus.ACTIVE.toString(),
  ...
  public List<AssignedMember> getAssignedMembersWithStatus(PerunSession sess, Resource resource) {
    try {
      // we do include all group statuses for such members
      return jdbc.query("select distinct " + MembersManagerImpl.GROUPS_ASSIGNED_MEMBERS_MAPPING_SELECT_QUERY +
  ...
  public List<Resource> getAssignedResources(PerunSession sess, Group group) {
    try {
      return jdbc.query("select " + RESOURCE_MAPPING_SELECT_QUERY + " from resources" +
                        ...
                        " where groups_resources_state.group_id=? and groups_resources_state" +
                        ".status=?::group_resource_status", RESOURCE_MAPPER, group.getId(),
          GroupResourceStatus.ACTIVE.toString());
    } catch (EmptyResultDataAccessException e) {
      return new ArrayList<>();
    } catch (RuntimeException e) {
  ...
  public List<Resource> getAssignedResources(PerunSession sess, Member member) {
    try {
      return jdbc.query("select distinct " + RESOURCE_MAPPING_SELECT_QUERY + " from resources" +
                        " join groups_resources_state on resources.id=groups_resources_state.resource_id" +
                        " and groups_resources_state.status=?::group_resource_status " +
                        " join groups on groups_resources_state.group_id=groups.id" +
                        " join groups_members on groups.id=groups_members.group_id " +
                        " where groups_members.member_id=?", RESOURCE_MAPPER, GroupResourceStatus.ACTIVE.toString(),
          member.getId());
    } catch (EmptyResultDataAccessException e) {
      return new ArrayList<>();
    } catch (RuntimeException e) {
  ...
  public List<Resource> getAssignedResources(PerunSession sess, Member member, Service service) {
    try {
      return jdbc.query("select distinct " + RESOURCE_MAPPING_SELECT_QUERY + " from resources" +
                        ...
                        " where groups_members.member_id=? and resource_services.service_id=?", RESOURCE_MAPPER,
          GroupResourceStatus.ACTIVE.toString(), member.getId(), service.getId());
    } catch (EmptyResultDataAccessException e) {
      return new ArrayList<>();
    } catch (RuntimeException e) {
      throw new InternalErrorException(e);
  ...
  public List<Resource> getAssignedResources(PerunSession sess, User user, Vo vo) {
    try {
      ...
      return jdbc.query("select distinct " + RESOURCE_MAPPING_SELECT_QUERY + " from resources" +
                        ...
                        " join groups_members on groups.id=groups_members.group_id" +
                        " join members on groups_members.member_id=members.id" +
                        " where members.user_id=? and members.vo_id=?", RESOURCE_MAPPER,
          GroupResourceStatus.ACTIVE.toString(), user.getId(), vo.getId());
    } catch (EmptyResultDataAccessException e) {
      return new ArrayList<>();
    } catch (RuntimeException e) {
      throw new InternalErrorException(e);
  ...
  public List<AssignedResource> getAssignedResourcesWithStatus(PerunSession sess, Member member) {
    try {
      List<AssignedResource> resources = jdbc.query(
          "select distinct " + ASSIGNED_RESOURCE_MAPPING_SELECT_QUERY + " from resources" +
          " join (SELECT group_id, resource_id, NULL as source_group_id, auto_assign_subgroups" +
          " FROM groups_resources" + " UNION" +
          " SELECT group_id, resource_id, source_group_id, true as auto_assign_subgroups" +
          " FROM groups_resources_automatic)" +
          " groups_resources_automatic on groups_resources_automatic.resource_id = resources.id" +
          " join groups_resources_state" +
          " on groups_resources_automatic.resource_id=groups_resources_state.resource_id" +
          " and groups_resources_automatic.group_id=groups_resources_state.group_id" +
          " join facilities on resources.facility_id=facilities.id" +
          " join groups on groups_resources_state.group_id=groups.id" +
          " join groups_members on groups.id=groups_members.group_id" + " where groups_members.member_id=?",
          ASSIGNED_RESOURCE_MAPPER, member.getId());

      resources.removeIf(r1 -> resources.stream().anyMatch(
          r2 -> r2.getEnrichedResource().equals(r1.getEnrichedResource()) &&
  ...
  public List<RichResource> getAssignedRichResources(PerunSession sess, Group group) {
    ...
    try {
      ...
      return jdbc.query(
          "select distinct " + RESOURCE_MAPPING_SELECT_QUERY + ", " + VosManagerImpl.VO_MAPPING_SELECT_QUERY + ", " +
          FacilitiesManagerImpl.FACILITY_MAPPING_SELECT_QUERY + ", " + RESOURCE_TAG_MAPPING_SELECT_QUERY +
          ...
          " left outer join res_tags on tags_resources.tag_id=res_tags.id" + " where groups_resources_state.group_id=?",
  ...
  public List<RichResource> getAssignedRichResources(PerunSession sess, Member member) {
    try {
      return jdbc.query(
          ...
          " join groups_members on groups.id=groups_members.group_id" +
          " left outer join tags_resources on resources.id=tags_resources.resource_id" +
          " left outer join res_tags on tags_resources.tag_id=res_tags.id" + " where groups_members.member_id=?",
  ...
  public List<RichResource> getAssignedRichResources(PerunSession sess, Member member, Service service) {
    try {
      return jdbc.query(
          ...
          " where groups_members.member_id=? and resource_services.service_id=?", RICH_RESOURCE_WITH_TAGS_EXTRACTOR,
          GroupResourceStatus.ACTIVE.toString(), member.getId(), service.getId());
    } catch (EmptyResultDataAccessException e) {
      return new ArrayList<>();
    } catch (RuntimeException e) {
  ...
  public List<Service> getAssignedServices(PerunSession sess, Resource resource) {
    try {
      return jdbc.query("select " + ServicesManagerImpl.SERVICE_MAPPING_SELECT_QUERY + " from services" +
                        " join resource_services on services.id=resource_services.service_id and resource_services" +
                        ".resource_id=?", ServicesManagerImpl.SERVICE_MAPPER, resource.getId());
    } catch (RuntimeException e) {
      throw new InternalErrorException(e);
    }
  }
  ...
  public List<User> getAssignedUsers(PerunSession sess, Resource resource) {
    try {
      return jdbc.query("select distinct " + UsersManagerImpl.USER_MAPPING_SELECT_QUERY +
                        ...
                        " join users on users.id=members.user_id" +
                        " where groups_resources_state.resource_id=? and groups_resources_state" +
                        ".status=?::group_resource_status", UsersManagerImpl.USER_MAPPER, resource.getId(),
          GroupResourceStatus.ACTIVE.toString());
    } catch (EmptyResultDataAccessException e) {
      return new ArrayList<>();
    } catch (RuntimeException e) {
  ...
  public List<Resource> getAssociatedResources(PerunSession sess, Group group) {
    try {
      return jdbc.query("select " + RESOURCE_MAPPING_SELECT_QUERY + " from resources" +
                        " join groups_resources_state on resources.id=groups_resources_state.resource_id" +
                        " where groups_resources_state.group_id=?", RESOURCE_MAPPER, group.getId());
    } catch (EmptyResultDataAccessException e) {
      return new ArrayList<>();
    } catch (RuntimeException e) {
      throw new InternalErrorException(e);
  ...
  public BanOnResource getBan(PerunSession sess, int memberId, int resourceId) throws BanNotExistsException {
    try {
      return jdbc.queryForObject(
          "select " + BAN_ON_RESOURCE_MAPPING_SELECT_QUERY + " from resources_bans where member_id=? and resource_id=?",
  ...
  public BanOnResource getBanById(PerunSession sess, int banId) throws BanNotExistsException {
    try {
      return jdbc.queryForObject("select " + BAN_ON_RESOURCE_MAPPING_SELECT_QUERY + " from resources_bans where id=? ",
  ...
  public List<BanOnResource> getBansForMember(PerunSession sess, int memberId) {
    try {
      return jdbc.query("select " + BAN_ON_RESOURCE_MAPPING_SELECT_QUERY + " from resources_bans where member_id=?",
  ...
  public List<BanOnResource> getBansForResource(PerunSession sess, int resourceId) {
    try {
      return jdbc.query("select " + BAN_ON_RESOURCE_MAPPING_SELECT_QUERY + " from resources_bans where resource_id=?",
  ...
  public List<AssignedGroup> getGroupAssignments(PerunSession sess, Resource resource) {
    try {
      return jdbc.query("select " + GroupsManagerImpl.ASSIGNED_GROUP_MAPPING_SELECT_QUERY + " from groups join " +
                        " (SELECT group_id, resource_id, NULL as source_group_id, auto_assign_subgroups FROM " +
                        "groups_resources" + " UNION" +
                        " SELECT group_id, resource_id, source_group_id, true as auto_assign_subgroups FROM " +
                        "groups_resources_automatic)" +
                        " groups_resources_automatic on groups_resources_automatic.group_id = groups.id" +
                        " join groups_resources_state on groups_resources_automatic" +
                        ".resource_id=groups_resources_state.resource_id " +
                        "and groups_resources_automatic.group_id=groups_resources_state.group_id" +
                        " where groups_resources_state.resource_id=?", GroupsManagerImpl.ASSIGNED_GROUP_MAPPER,
          resource.getId());
    } catch (RuntimeException e) {
      throw new InternalErrorException(e);
    }
  ...
  public List<GroupResourceAssignment> getGroupResourceAssignments(PerunSession sess,
    ...
    try {
      ...
      String statusesSql =
          statuses.stream().map(status -> "'" + status.toString() + "'").collect(Collectors.joining(", "));

      return jdbc.query("select " + GROUP_RESOURCE_ASSIGNMENT_MAPPING_SELECT_QUERY + " from groups " +
                        ...
                        " where groups_resources_state.status in (" + statusesSql + ")",
  ...
  public List<AssignedResource> getResourceAssignments(PerunSession sess, Group group) {
    try {
      return jdbc.query("select " + ASSIGNED_RESOURCE_MAPPING_SELECT_QUERY + " from resources" +
                        " join (SELECT group_id, resource_id, NULL as source_group_id, auto_assign_subgroups FROM " +
                        "groups_resources" + " UNION" +
                        " SELECT group_id, resource_id, source_group_id, true as auto_assign_subgroups FROM " +
                        "groups_resources_automatic)" +
                        " groups_resources_automatic on groups_resources_automatic.resource_id = resources.id" +
                        " join groups_resources_state on groups_resources_automatic" +
                        ".resource_id=groups_resources_state" +
                        ".resource_id and groups_resources_automatic.group_id=groups_resources_state.group_id" +
                        " join facilities on resources.facility_id=facilities.id" +
  ...
  public Resource getResourceById(PerunSession sess, int id) throws ResourceNotExistsException {
    try {
      return jdbc.queryForObject("select " + RESOURCE_MAPPING_SELECT_QUERY + " from resources where resources.id=?",
          RESOURCE_MAPPER, id);
    } catch (EmptyResultDataAccessException e) {
  ...
  public Resource getResourceByName(PerunSession sess, Vo vo, Facility facility, String name)
    ...
    try {
      return jdbc.queryForObject("select " + RESOURCE_MAPPING_SELECT_QUERY +
  ...
  public List<Resource> getResources(PerunSession sess, Vo vo) {
    try {
      return jdbc.query("select " + RESOURCE_MAPPING_SELECT_QUERY + " from resources where resources.vo_id=?",
  ...
  public List<Resource> getResources(PerunSession sess, User user, List<Status> memberStatuses,
    ...
    String groupResourceStatusesSql =
        groupResourceStatuses.stream().map(status -> "'" + status.toString() + "'").collect(Collectors.joining(", "));
    ...
    try {
      ...
      return namedParameterJdbcTemplate.query(
          "select distinct " + ResourcesManagerImpl.RESOURCE_MAPPING_SELECT_QUERY + " from resources" +
          " join groups_resources_state on groups_resources_state.resource_id=resources.id" +
          " and groups_resources_state.status in (" + groupResourceStatusesSql + ")" +
          " join groups_members on groups_members.group_id=groups_resources_state.group_id" +
          " and groups_members.source_group_status in (:groupStatuses)" +
          " join members on members.id=groups_members.member_id" +
  ...
  public List<Resource> getResourcesByAttribute(PerunSession sess, Attribute attribute) {
    try {
      return jdbc.query("select " + RESOURCE_MAPPING_SELECT_QUERY + " from resources join " +
                        "resource_attr_values on resources.id=resource_attr_values.resource_id " +
                        "where resource_attr_values.attr_id=? and resource_attr_values.attr_value=?", RESOURCE_MAPPER,
          attribute.getId(), BeansUtils.attributeValueToString(attribute));
    } catch (EmptyResultDataAccessException e) {
      return new ArrayList<>();
    } catch (RuntimeException e) {
      throw new InternalErrorException(e);
  ...
  public List<Resource> getResourcesByIds(PerunSession sess, List<Integer> ids) {
    try {
      return jdbc.execute("select " + RESOURCE_MAPPING_SELECT_QUERY + " from resources where id " +
  ...
  public int getResourcesCount(PerunSession perunSession, Vo vo) {
    try {
      return jdbc.queryForInt("select count(*) from resources where resources.vo_id=?", vo.getId());
    } catch (RuntimeException e) {
      throw new InternalErrorException(e);
    }
  }
  ...
  public List<Resource> getResourcesWhereGroupIsAdmin(PerunSession sess, Facility facility, Vo vo,
    ...
    try {
      return jdbc.query("select distinct " + ResourcesManagerImpl.RESOURCE_MAPPING_SELECT_QUERY + " from resources " +
                        ...
                        " where resources.facility_id=? and resources.vo_id=? and authz.authorized_group_id=? and " +
                        "authz" + ".role_id=(select id from roles where name=?)", RESOURCE_MAPPER, facility.getId(),
          vo.getId(), authorizedGroup.getId(), Role.RESOURCEADMIN.toLowerCase());
    } catch (EmptyResultDataAccessException e) {
      return new ArrayList<>();
    } catch (RuntimeException e) {
      throw new InternalErrorException(e);
  ...
  public List<Resource> getResourcesWhereUserIsAdmin(PerunSession sess, User user) {
    try {
      return jdbc.query("select " + RESOURCE_MAPPING_SELECT_QUERY + " from resources " +
                        ...
                        " where (authz.user_id=? or members.user_id=?) and authz.role_id=(select id from roles where " +
                        "name=?)" + " and (members.status=? or members.status is null)", RESOURCE_MAPPER,
          MemberGroupStatus.VALID.getCode(), user.getId(), user.getId(), Role.RESOURCEADMIN.toLowerCase(),
  ...
  public List<Resource> getResourcesWhereUserIsAdmin(PerunSession sess, Facility facility, Vo vo, User authorizedUser) {
    try {
      return jdbc.query("select distinct " + ResourcesManagerImpl.RESOURCE_MAPPING_SELECT_QUERY + " from resources " +
                        " left outer join authz on authz.resource_id=resources.id " +
                        " left outer join groups_members on groups_members.group_id=authz.authorized_group_id and " +
                        "groups_members.source_group_status=?" +
                        " left outer join members on members.id=groups_members" + ".member_id" +
                        " where resources.facility_id=? and resources.vo_id=? and (authz.user_id=? or members" +
                        ".user_id=?) " +
                        " and authz.role_id=(select id from roles where name=?) and (members.status=? or members" +
                        ".status is " + "null) ", RESOURCE_MAPPER, MemberGroupStatus.VALID.getCode(), facility.getId(),
          vo.getId(), authorizedUser.getId(), authorizedUser.getId(), Role.RESOURCEADMIN.toLowerCase(),
          Status.VALID.getCode());
    } catch (EmptyResultDataAccessException e) {
      return new ArrayList<>();
    } catch (RuntimeException e) {
  ...
  public List<Resource> getResourcesWhereUserIsAdmin(PerunSession sess, Vo vo, User authorizedUser) {
    try {
      return jdbc.query("select distinct " + ResourcesManagerImpl.RESOURCE_MAPPING_SELECT_QUERY + " from resources " +
                        ...
                        " and authz.role_id=(select id from roles where name=?) and (members.status=? or members" +
                        ".status is " + "null) ", RESOURCE_MAPPER, MemberGroupStatus.VALID.getCode(), vo.getId(),
          authorizedUser.getId(), authorizedUser.getId(), Role.RESOURCEADMIN.toLowerCase(), Status.VALID.getCode());
    } catch (EmptyResultDataAccessException e) {
      return new ArrayList<>();
    } catch (RuntimeException e) {
      throw new InternalErrorException(e);
  ...
  public RichResource getRichResourceById(PerunSession sess, int id) throws ResourceNotExistsException {
    try {
      return jdbc.queryForObject(
          "select " + RESOURCE_MAPPING_SELECT_QUERY + ", " + VosManagerImpl.VO_MAPPING_SELECT_QUERY + ", " +
          FacilitiesManagerImpl.FACILITY_MAPPING_SELECT_QUERY + ", " + RESOURCE_TAG_MAPPING_SELECT_QUERY +
          " from resources join vos on resources.vo_id=vos.id " +
          "join facilities on resources.facility_id=facilities.id" +
          " left outer join tags_resources on resources.id=tags_resources.resource_id" +
          " left outer join res_tags on tags_resources.tag_id=res_tags.id where resources.id=?",
  ...
  public List<RichResource> getRichResources(PerunSession sess, Vo vo) {
    try {
      return jdbc.query(
          ...
          " join facilities on resources.facility_id=facilities.id" +
          " left outer join tags_resources on resources.id=tags_resources.resource_id" +
          " left outer join res_tags on tags_resources.tag_id=res_tags.id" + " where resources.vo_id=?",
  ...
  public List<RichResource> getRichResourcesWithMemberAndAttribute(PerunSession perunSession, Member member,
    ...
    try {
      return jdbc.query(
          ...
          "left outer join tags_resources on resources.id=tags_resources.resource_id " +
          "left outer join res_tags on tags_resources.tag_id=res_tags.id " +
          "where service_required_attrs.attr_id=? and groups_members.member_id=?", RICH_RESOURCE_WITH_TAGS_EXTRACTOR,
  ...
  public boolean groupResourceAssignmentExists(PerunSession sess, Resource resource, Group group) {
    try {
      return 1 ==
             jdbc.queryForInt("select count(1) from groups_resources_state where group_id=?" + " and resource_id=?",
  ...
  public boolean isGroupAssigned(PerunSession sess, Resource resource, Group group) {
    try {
      return 1 == jdbc.queryForInt("select count(1) from groups_resources_state where group_id=? and resource_id=?" +
                                   " and status=?::group_resource_status", group.getId(), resource.getId(),
          GroupResourceStatus.ACTIVE.toString());
    } catch (RuntimeException e) {
      throw new InternalErrorException(e);
    }
  ...
  public boolean isUserAssigned(PerunSession sess, User user, Resource resource) {
    try {
      return (0 < jdbc.queryForInt("select count(*) from groups_resources_state" + " join groups" +
                                   " on groups_resources_state.group_id=groups.id" +
                                   " join groups_members on groups.id=groups_members.group_id" +
                                   " join members on groups_members.member_id=members.id" +
                                   " where groups_resources_state.resource_id=? and groups_resources_state" +
                                   ".status=?::group_resource_status" + " and members.user_id=?", resource.getId(),
          GroupResourceStatus.ACTIVE.toString(), user.getId()));
    } catch (RuntimeException e) {
      throw new InternalErrorException(e);
    }
  }
  ...
  public void removeAutomaticGroupFromResource(PerunSession perunSession, Group group, Resource resource,
    ...
    try {
      int numAffected = jdbc.update(
          "delete from groups_resources_automatic where group_id=? and resource_id=?" + " and source_group_id=?",
  ...
  public BanOnResource setBan(PerunSession sess, BanOnResource banOnResource) {
    try {
      ...
      jdbc.update(
          "insert into resources_bans(id, description, banned_to, member_id, resource_id, created_by, created_at," +
          "modified_by,modified_at,created_by_uid,modified_by_uid) " + "values (?,?," +
          (banOnResource.getValidityTo() != null ?
              "'" + Compatibility.getDate(banOnResource.getValidityTo().getTime()) + "'" : "DEFAULT") + ",?,?,?," +
          Compatibility.getSysdate() + ",?," + Compatibility.getSysdate() + ",?,?)", newId,
          banOnResource.getDescription(), banOnResource.getMemberId(), banOnResource.getResourceId(),
          sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getActor(),
          sess.getPerunPrincipal().getUserId(), sess.getPerunPrincipal().getUserId());
  ...
  public void setFailedGroupResourceAssignmentCause(PerunSession sess, Group group, Resource resource, String cause)
    ...
    try {
      int numAffected =
          jdbc.update("update groups_resources_state set failure_cause=?" + " where group_id=? and resource_id=?",
              cause, group.getId(), resource.getId());
      if (numAffected == 0) {
        throw new GroupNotDefinedOnResourceException(
            "Group " + group.getId() + " is not defined on resource " + resource.getId());
      }
  ...
  public void setGroupResourceStatus(PerunSession sess, Group group, Resource resource, GroupResourceStatus status)
    ...
    try {
      int numAffected = jdbc.update(
          "update groups_resources_state set status=?::group_resource_status" + " where group_id=? and resource_id=?",
          status.toString(), group.getId(), resource.getId());
      if (numAffected == 0) {
        throw new GroupNotDefinedOnResourceException(
            "Group " + group.getId() + " is not defined on resource " + resource.getId());
      }
  ...
  public BanOnResource updateBan(PerunSession sess, BanOnResource banOnResource) {
    try {
      jdbc.update("update resources_bans set description=?, banned_to=" + (banOnResource.getValidityTo() != null ?
          ...
          banOnResource.getDescription(), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getUserId(),
          banOnResource.getId());
    ...
    try {
      ...
      if (!resource.getName().equals(map.get("name")) && !resource.getDescription().equals(map.get("dsc"))) {
        jdbc.update("update resources set name=?, dsc=?, modified_by=?, modified_by_uid=?, modified_at=" +
                    Compatibility.getSysdate() + "  where id=?", resource.getName(), resource.getDescription(),
        ...
        jdbc.update(
            ...
            "  where id=?", resource.getDescription(), sess.getPerunPrincipal().getActor(),
            sess.getPerunPrincipal().getUserId(), resource.getId());
        ...
        jdbc.update(
            ...
            "  where id=?", resource.getName(), sess.getPerunPrincipal().getActor(),
            sess.getPerunPrincipal().getUserId(), resource.getId());
      }
    ...
    try {
      ...
      if (!resourceTag.getTagName().equals(map.get("tag_name"))) {
        jdbc.update("update res_tags set tag_name=?, modified_by=?, modified_by_uid=?, modified_at=" +
                    Compatibility.getSysdate() + "  where id=?", resourceTag.getTagName(),
