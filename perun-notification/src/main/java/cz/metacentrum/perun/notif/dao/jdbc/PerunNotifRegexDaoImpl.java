...
public class PerunNotifRegexDaoImpl extends JdbcDaoSupport implements
	...
	public List<PerunNotifRegex> getAll() {
		...
		for (PerunNotifRegex regex : result) {
			List<PerunNotifObject> objects = this.getJdbcTemplate().query("SELECT * from pn_object object JOIN pn_regex_object bind ON object.id = bind.object_id WHERE bind.regex_id = ?", PerunNotifObject.PERUN_NOTIF_OBJECT, regex.getId());
	...
	public PerunNotifRegex getPerunNotifRegexById(int id) {
		...
		try {
			regex = this.getJdbcTemplate().queryForObject("SELECT * from pn_regex where id = ?", PerunNotifRegex.PERUN_NOTIF_REGEX, id);
	...
	public Set<PerunNotifRegex> getPerunNotifRegexForTemplateId(int id) {
		...
		List<PerunNotifRegex> regexes = this.getJdbcTemplate().query("SELECT * from pn_regex regex JOIN pn_template_regex bind ON regex.id=bind.regex_id WHERE bind.template_id = ?", PerunNotifRegex.PERUN_NOTIF_REGEX, id);
		if (regexes != null) {
	...
	public List<Integer> getTemplateIdsForRegexId(int id) {
		...
		return this.getJdbcTemplate().query("select template_id from pn_template_regex where regex_id = ?", (rs, rowNum) -> rs.getInt("template_id"), id);
	}
