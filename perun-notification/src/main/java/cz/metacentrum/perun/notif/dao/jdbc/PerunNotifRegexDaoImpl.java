...
public class PerunNotifRegexDaoImpl extends JdbcDaoSupport implements PerunNotifRegexDao {
  ...
  public List<PerunNotifRegex> getAll() {
    ...
    List<PerunNotifRegex> result =
        this.getJdbcTemplate().query("SELECT * from pn_regex ", PerunNotifRegex.PERUN_NOTIF_REGEX);
  ...
  public Set<PerunNotifRegex> getPerunNotifRegexForTemplateId(int id) {
    ...
    if (regexes != null) {
      for (PerunNotifRegex regex : regexes) {
        ...
        List<PerunNotifObject> objects = this.getJdbcTemplate().query(
            "SELECT * from pn_object object JOIN pn_regex_object bind ON object.id = bind.object_id WHERE bind" +
            ".regex_id = ?",
            PerunNotifObject.PERUN_NOTIF_OBJECT, regex.getId());
        regex.addObjects(objects);
      }
  ...
  public boolean isRegexRelation(int templateId, Integer regexId) {
    ...
    int countOfRelations = this.getJdbcTemplate()
        .queryForObject("select count (*) from pn_template_regex where template_id = ? AND regex_id = ?", Integer.class,
            templateId, regexId);
