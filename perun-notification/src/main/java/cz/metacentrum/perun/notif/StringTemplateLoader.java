...
/**
 * A {@link freemarker.cache.TemplateLoader} that uses a Map with Strings as its source of templates.
 * <p>
 * In most case the regular way of loading templates from files will be fine. However, there can be situations where you
 * don't want to or can't load a template from a file, e.g. if you have to deploy a single jar for JavaWebStart or if
 * they are contained within a database. A single template can be created manually e.g.
 * <pre>
 ...
 * will want to combine a <tt>StringTemplateLoader</tt> with another loader. You can do so using a
 * {@link freemarker.cache.MultiTemplateLoader}.
 *
...
public class StringTemplateLoader implements TemplateLoader {
  ...
  /**
   * Puts a template into the loader. A call to this method is identical to the call to the three-arg
  ...
  /**
   * Puts a template into the loader. The name can contain slashes to denote logical directory structure, but must not
   * start with a slash. If the method is called multiple times for the same name and with different last modified time,
   * the configuration's template cache will reload the template according to its own refresh settings (note that if the
   * refresh is disabled in the template cache, the template will not be reloaded). Also, since the cache uses
   * lastModified to trigger reloads, calling the method with different source and identical timestamp won't trigger
   * reloading.
   *
   * @param name           the name of the template.
   * @param templateSource the source code of the template.
