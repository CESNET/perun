...
/**
 * Exception thrown when trying to create new template message and a template message with the same template id and
...
public class NotifTemplateMessageAlreadyExistsException extends PerunException {
  ...
  public NotifTemplateMessageAlreadyExistsException(PerunNotifTemplateMessage message) {
    super("TemplateMessage with template id: " + message.getTemplateId() + " and locale: " + message.getLocale() +
          " already exists.");
  }
}
