...
public class RegistrarManagerImpl implements RegistrarManager {
	...
	/**
	 * Creates all necessary UserExtSources for new user
	 * based on his reserved logins in namespaces
	 * 
	 * @param app
	 * @param userId
	 * @param reservedLogin
	 * @param reservedNamespace
	 * @throws Exception
	 */
    @Deprecated
	protected void createUserExtSources(Application app, int userId, String reservedLogin, String reservedNamespace) throws Exception {
		// if applicant has a reserved login then create correspinding userExtSources
		if (reservedLogin != null && reservedNamespace != null) {
			//FIXME have configuration of the login-namespace and extSources somewhere else (configuration of the field USERNAME in the application?)

			User user = perun.getUsersManagerBl().getUserById(registrarSession, userId);
			if (reservedNamespace.equals("einfra")) {
				List<String> kerberosLogins = new ArrayList<String>();

				// Set META and EINFRA userExtSources
				ExtSource extSource = perun.getExtSourcesManagerBl().getExtSourceByName(registrarSession, "META");
				UserExtSource ues = new UserExtSource(extSource, reservedLogin + "@META");
				ues.setLoa(0);
				kerberosLogins.add(reservedLogin + "@META");

				perun.getUsersManagerBl().addUserExtSource(registrarSession, user, ues);

				extSource = perun.getExtSourcesManagerBl().getExtSourceByName(registrarSession, "EINFRA");
				ues = new UserExtSource(extSource, reservedLogin + "@EINFRA");
				ues.setLoa(0);
				kerberosLogins.add(reservedLogin + "@EINFRA");

				perun.getUsersManagerBl().addUserExtSource(registrarSession, user, ues);
				
				extSource = perun.getExtSourcesManagerBl().getExtSourceByName(registrarSession, "https://login.ics.muni.cz/idp/shibboleth");
				ues = new UserExtSource(extSource, reservedLogin + "@meta.cesnet.cz");
				ues.setLoa(0);
				...
				try {
					kerberosLoginsAttr = perun.getAttributesManagerBl().getAttribute(registrarSession, user, 
							AttributesManager.NS_USER_ATTR_DEF + ":" + "kerberosLogins");
					...
					AttributeDefinition kerberosLoginsAttrDef = perun.getAttributesManagerBl().getAttributeDefinition(registrarSession, 
							AttributesManager.NS_USER_ATTR_DEF + ":" + "kerberosLogins");
					kerberosLoginsAttr = new Attribute(kerberosLoginsAttrDef);
				}
				kerberosLoginsAttr.setValue(kerberosLogins);
				perun.getAttributesManagerBl().setAttribute(registrarSession, user, kerberosLoginsAttr);
			} else if (reservedNamespace.equals("egi-ui")) {
				List<String> kerberosLogins = new ArrayList<String>();

				ExtSource extSource = perun.getExtSourcesManagerBl().getExtSourceByName(registrarSession, "EGI");
				UserExtSource ues = new UserExtSource(extSource, reservedLogin + "@EGI");
				ues.setLoa(0);
				kerberosLogins.add(reservedLogin + "@EGI");
				...
				kerberosLoginsAttr.setValue(kerberosLogins);
				perun.getAttributesManagerBl().setAttribute(registrarSession, user, kerberosLoginsAttr);
			} else if (reservedNamespace.equals("sitola")) {
        List<String> kerberosLogins = new ArrayList<String>();

        ExtSource extSource = perun.getExtSourcesManagerBl().getExtSourceByName(registrarSession, "SITOLA.FI.MUNI.CZ");
        UserExtSource ues = new UserExtSource(extSource, reservedLogin + "@SITOLA.FI.MUNI.CZ");
        ues.setLoa(0);
        kerberosLogins.add(reservedLogin + "@SITOLA.FI.MUNI.CZ");
