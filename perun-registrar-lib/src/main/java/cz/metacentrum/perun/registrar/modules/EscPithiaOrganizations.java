    ...
    if (Application.AppType.INITIAL.equals(app.getType()) && app.getGroup() != null) {
      ...
      try {
        organizationAdminsGroup =
            perun.getGroupsManagerBl().createGroup(session, newOrganization, organizationAdminsGroup);
      } catch (GroupExistsException | GroupRelationNotAllowed | GroupRelationAlreadyExists e) {
      ...
      try {
        organizationMembersGroup =
            perun.getGroupsManagerBl().createGroup(session, newOrganization, organizationMembersGroup);
      } catch (GroupExistsException | GroupRelationNotAllowed | GroupRelationAlreadyExists e) {
      ...
      copyFormAndNotifications(session, newOrganization.getShortName(), getTemplateGroup(session, vo),
          organizationMembersGroup);
      copyFormAndNotifications(session, newOrganization.getShortName(), getAdminTemplateGroup(session, vo),
          organizationAdminsGroup);
      ...
      copySettings(perun, session, templateGroup, organizationMembersGroup,
          AttributesManager.NS_GROUP_ATTR_DEF + ":blockManualMemberAdding");
      // handle replace in attribute value
      Attribute attribute = perun.getAttributesManagerBl()
          .getAttribute(session, templateGroup, AttributesManager.NS_GROUP_ATTR_DEF + ":applicationURL");
      attribute.setValue(attribute.valueAsString()
          .replace("{groupName}", URLEncoder.encode(organizationMembersGroup.getName(), StandardCharsets.UTF_8)));
      ...
      copySettings(perun, session, adminTemplateGroup, organizationAdminsGroup,
          AttributesManager.NS_GROUP_ATTR_DEF + ":blockManualMemberAdding");
      // handle replace in attribute value
      Attribute attribute2 = perun.getAttributesManagerBl()
          .getAttribute(session, adminTemplateGroup, AttributesManager.NS_GROUP_ATTR_DEF + ":applicationURL");
      attribute2.setValue(attribute2.valueAsString()
          .replace("{groupName}", URLEncoder.encode(organizationAdminsGroup.getName(), StandardCharsets.UTF_8)));
