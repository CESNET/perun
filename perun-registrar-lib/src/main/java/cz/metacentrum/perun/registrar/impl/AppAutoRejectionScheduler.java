...
public class AppAutoRejectionScheduler {
  ...
  private static final String DEFAULT_VO_MSG_ADMIN = "Your application to VO " + VO_PLACEHOLDER +
                                                     " was automatically rejected, because admin didn't approve your " +
                                                     "application in a timely manner.";
  private static final String DEFAULT_GROUP_MSG_ADMIN = "Your application to group " + GROUP_PLACEHOLDER +
                                                        " was automatically rejected, because admin didn't approve " +
  ...
  private static final String DEFAULT_VO_MSG_MAIL = "Your application to VO " + VO_PLACEHOLDER + " was " +
                                                    "automatically rejected, because you didn't verify your email " +
                                                    "address.";
  private static final String DEFAULT_GROUP_MSG_MAIL = "Your application to group " + GROUP_PLACEHOLDER +
                                                       " was automatically rejected, because you didn't verify your " +
                                                       "email address.";
  ...
  /**
   * Perform check on applications state and expiration attribute and reject them, if it is necessary. Rejection is
   * based on current date and their value of application expiration.
   * <p>
  ...
  /**
   * Returns preferred language filled in the application. If there is no item with destination attribute
   * 'urn:perun:user:attribute-def:def:preferredLanguage filled, null is returned.
   *
   * @param application application, from which the preferred language should be returned
   * @return value filled by user, for application item mapping to preferredLanguage attribute, or null if the value is
   * empty of there is no such item
  ...
  /**
   * Returns reject message obtained from the given attribute, using the given key; or returns the default msg, if there
   * is no suitable message in the attribute.
   *
   ...
   * @return reject message, concerning the preferred language, obtained from the given attribute, or the given default
   * message
   */
  private String getRejectMessage(Attribute attribute, String lang, String key, String defaultMsg) {
    String attrKey = key + "-" + lang;
    String attrValue = attribute != null && attribute.getValue() != null ? attribute.valueAsMap().get(attrKey) : null;
  ...
  /**
   * Returns preferred language of user from the given application. Preferred language is taken from the submitted
   * application, or from the user's preferred language attribute. If none is present, 'en' is returned.
   *
  ...
  public void initialize() {
    this.sess = perun.getPerunSession(new PerunPrincipal("perunRegistrar", ExtSourcesManager.EXTSOURCE_NAME_INTERNAL,
        ExtSourcesManager.EXTSOURCE_INTERNAL), new PerunClient());
  }

  /**
   * Rejects the given application with custom message. The 'attrValueKey' represents the key of the message in the
   * applicationAutoRejectMessages attribute. If the vo/group has no such attribute set, the default message will be
   * used.
   *
   * @param application    application to be rejected
   * @param attrValueKey   key of a message in the applicationAutoRejectMessages
   * @param defaultMessage default message if the appropriate vo/group has no applicationAutoRejectMessages attribute
   *                       set
   */
  private void rejectApplication(Application application, String attrValueKey, String defaultMessage) {
    ...
    String message;
    if (group != null) {
      message = getRejectMessage(messagesAttr, lang, attrValueKey, defaultMessage).replaceAll(GROUP_PLACEHOLDER,
          group.getName());
    } else {
      message =
          getRejectMessage(messagesAttr, lang, attrValueKey, defaultMessage).replaceAll(VO_PLACEHOLDER, vo.getName());
    }
  ...
  /**
   * Compares date of last modification of application to values in expiration attribute and if finds expired
   * application, then rejects it.
   *
  ...
  /**
   * Checks all applications for given vos and if some application is expired (according to expiration rules set by VO
   * Manager), then rejects this application.
   *
