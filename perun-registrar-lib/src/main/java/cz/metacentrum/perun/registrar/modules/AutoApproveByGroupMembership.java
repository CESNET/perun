...
public class AutoApproveByGroupMembership extends DefaultRegistrarModule {
	...
	private static final String AUTOAPPROVEBYGROUPMEMBERSHIP_GROUP_ATTRNAME = AttributesManager.NS_GROUP_ATTR_DEF + ":autoApproveByGroupMembership";
	private static final String AUTOAPPROVEBYGROUPMEMBERSHIP_VO_ATTRNAME = AttributesManager.NS_VO_ATTR_DEF + ":autoApproveByGroupMembership";
	...
	public boolean autoApproveShouldBeForce(PerunSession sess, Application app) throws PerunException {
		...
		try {
			if (app.getUser() == null) {
				...
				if (user != null) {
					...
					log.error("[REGISTRAR] User is not member of VO, therefore it is not possible to auto-approve group application: " + app);
					return false;
				}
			} else {
		...
		Attribute autoApproveGroupIds = perun.getAttributesManagerBl().getAttribute(sess, app.getGroup(), AUTOAPPROVEBYGROUPMEMBERSHIP_GROUP_ATTRNAME).valueAsList() != null ?
		...
		if (autoApproveGroupIds.valueAsList() != null) {
			for (String id : autoApproveGroupIds.valueAsList()) {
				try {
					if (perun.getGroupsManagerBl().getTotalMemberGroupStatus(sess, member, perun.getGroupsManagerBl().getGroupById(sess, Integer.parseInt(id))) == MemberGroupStatus.VALID) {
						return true;
					}
				} catch (GroupNotExistsException ex) {
					if (perun.getAttributesManagerBl().getAttribute(sess, app.getGroup(), AUTOAPPROVEBYGROUPMEMBERSHIP_GROUP_ATTRNAME).valueAsList() != null) {
						log.error("[REGISTRAR] Attribute autoApproveByGroupMembership for group " + app.getGroup() + " contains invalid group ID: " + id);
					} else {
						log.error("[REGISTRAR] Attribute autoApproveByGroupMembership for VO " + app.getVo() + " contains invalid group ID: " + id);
					}
				}
			}
		}
