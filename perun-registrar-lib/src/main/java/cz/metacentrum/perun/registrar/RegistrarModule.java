...
/**
 * Interface for all registrar modules. They extend core registrar functionality and are called after same functions in
 * registrar.
 *
...
public interface RegistrarModule {
  ...
  /**
   * Custom logic for checking method before application approval from GUI. Also called if application is set to
   * auto-approval mode. In such case, if CantBeApprovedException is thrown, approval is stopped.
   *
   * @param session who approves the application
   * @param app     application
   */
  ...
  /**
   * Custom logic for processing pre-filled form item data before they are returned from Perun to GUI in order to
   * display form. It can be used to modify pre-filled data retrieved from perun or federation attributes.
   *
   * @param session   who approves the application
