...
public class PublicationSystemManagerDaoImpl implements PublicationSystemManagerDao {
  ...
  private static final String PUBLICATION_SYSTEM_SELECT_QUERY = "cabinet_publication_systems.id as ps_id, " +
                                                                "cabinet_publication_systems.friendlyName as " +
                                                                "ps_friendlyName, cabinet_publication_systems.type as" +
                                                                " ps_type, " +
                                                                " cabinet_publication_systems.url as ps_url, " +
                                                                "cabinet_publication_systems.username as ps_username," +
                                                                " cabinet_publication_systems.password as " +
                                                                "ps_password, cabinet_publication_systems" +
                                                                ".loginNamespace as ps_loginNamespace";
  private static final RowMapper<PublicationSystem> PUBLICATION_SYSTEM_ROW_MAPPER = new RowMapper<PublicationSystem>() {
    @Override
  ...
  public PublicationSystem createPublicationSystem(PerunSession session, PublicationSystem ps) {
    try {
      ...
      jdbc.update(
          "insert into cabinet_publication_systems (id, friendlyName, type, url, username, password, loginNamespace, " +
          "created_by_uid, modified_by_uid)" +
          " values (?,?,?,?,?,?,?,?,?)", newId, ps.getFriendlyName(), ps.getType(), ps.getUrl(), ps.getUsername(),
          ps.getPassword(), ps.getLoginNamespace(), session.getPerunPrincipal().getUserId(),
  ...
  public void deletePublicationSystem(PublicationSystem ps) throws CabinetException {
    try {
      ...
      if (numAffected == 0) {
        throw new CabinetException(ErrorCodes.PUBLICATION_SYSTEM_NOT_EXISTS);
      }
  ...
  public PublicationSystem getPublicationSystemById(int id) throws CabinetException {
    try {
      return jdbc.queryForObject(
          "select " + PUBLICATION_SYSTEM_SELECT_QUERY + " from cabinet_publication_systems where id=?",
          PUBLICATION_SYSTEM_ROW_MAPPER, id);
  ...
  public PublicationSystem getPublicationSystemByName(String name) throws CabinetException {
    try {
      return jdbc.queryForObject(
          "select " + PUBLICATION_SYSTEM_SELECT_QUERY + " from cabinet_publication_systems where friendlyName=?",
          PUBLICATION_SYSTEM_ROW_MAPPER, name);
    } catch (EmptyResultDataAccessException ex) {
      throw new CabinetException(ErrorCodes.PUBLICATION_SYSTEM_NOT_EXISTS, ex);
    } catch (RuntimeException err) {
      throw new InternalErrorException(err);
  ...
  public PublicationSystem getPublicationSystemByNamespace(String namespace) throws CabinetException {
    try {
      return jdbc.queryForObject(
          "select " + PUBLICATION_SYSTEM_SELECT_QUERY + " from cabinet_publication_systems where loginNamespace=?",
          PUBLICATION_SYSTEM_ROW_MAPPER, namespace);
    } catch (EmptyResultDataAccessException ex) {
      throw new CabinetException(ErrorCodes.PUBLICATION_SYSTEM_NOT_EXISTS, ex);
    } catch (RuntimeException err) {
      throw new InternalErrorException(err);
  ...
  public List<PublicationSystem> getPublicationSystems() {
    try {
      return jdbc.query("select " + PUBLICATION_SYSTEM_SELECT_QUERY + " from cabinet_publication_systems",
          PUBLICATION_SYSTEM_ROW_MAPPER);
    } catch (EmptyResultDataAccessException ex) {
  ...
  public PublicationSystem updatePublicationSystem(PerunSession session, PublicationSystem ps) throws CabinetException {
    try {
      int numAffected = jdbc.update(
          "update cabinet_publication_systems set friendlyName=?,type=?,url=?,loginNamespace=?,modified_by_uid=?" +
          " where id=?", ps.getFriendlyName(), ps.getType(), ps.getUrl(), ps.getLoginNamespace(),
          session.getPerunPrincipal().getUserId(), ps.getId());
      if (numAffected == 0) {
        throw new CabinetException(ErrorCodes.PUBLICATION_SYSTEM_NOT_EXISTS);
      }
      if (numAffected > 1) {
        throw new ConsistencyErrorException("There are multiple PS with same id: " + ps.getId());
      }
    } catch (RuntimeException err) {
      throw new InternalErrorException(err);
    }
