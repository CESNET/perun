...
public class ParserPerunDB implements Parser {
	...
	private final static Logger log = LoggerFactory.getLogger(ParserPerunDB.class);
	@Autowired
	...
	public void run() {
		...
		//String file = Thread.currentThread().getContextClassLoader().getResource("test-data-for-parser-perun-db.txt").getFile();
		InputStream in = getClass().getClassLoader().getResourceAsStream("test-data-for-parser-perun-db.txt");
