...
public class TaskStatusImplTest extends TestBase {
	private final static Logger log = LoggerFactory
			.getLogger(TaskStatusImplTest.class);
	...
	public void getWaitingDestinationsTest() throws InternalErrorException {
		...
		Assert.isTrue(
				destinations.contains(destination3)
						|| destinations.contains(destination4),
				"destination3 or destination4");
		destinations = taskStatus.getWaitingDestinations();
		Assert.isTrue(destinations.size() == 1, "size == 1");
		taskStatus.setDestinationStatus(destination3,
	...
	public void getDestinationStatusTest() throws InternalErrorException {
		TaskStatus taskStatus = new TaskStatusImpl(task1);
		Assert.isTrue(taskStatus.getDestinationStatus(destination1).equals(
	...
	public void setDestinationStatusTest() throws InternalErrorException {
		TaskStatus taskStatus = new TaskStatusImpl(task1);
		taskStatus.setDestinationStatus(destination1,
				TaskDestinationStatus.DONE);
		Assert.isTrue(taskStatus.getDestinationStatus(destination1).equals(
				TaskDestinationStatus.DONE));
	}
	...
	public void isTaskFinishedTest() throws InternalErrorException {
		...
		taskStatus.setDestinationStatus(destination3,
				TaskDestinationStatus.DONE);
		Assert.isTrue(!taskStatus.isTaskFinished());
		taskStatus.setDestinationStatus(destination2,
				TaskDestinationStatus.ERROR);
		Assert.isTrue(taskStatus.isTaskFinished());
		taskStatus.setDestinationStatus(destination4,
				TaskDestinationStatus.ERROR);
		Assert.isTrue(taskStatus.isTaskFinished());
	}
	...
	public void getTaskStatusTest() throws InternalErrorException {
		TaskStatus taskStatus = new TaskStatusImpl(task1);
		Assert.isTrue(taskStatus.getTaskStatus().equals(
				cz.metacentrum.perun.taskslib.model.Task.TaskStatus.PROCESSING));
		taskStatus.setDestinationStatus(destination1,
				TaskDestinationStatus.DONE);
		Assert.isTrue(taskStatus.getTaskStatus().equals(
				cz.metacentrum.perun.taskslib.model.Task.TaskStatus.PROCESSING));
		...
		taskStatus.setDestinationStatus(destination2,
				TaskDestinationStatus.DONE);
		Assert.isTrue(taskStatus.getTaskStatus().equals(
				cz.metacentrum.perun.taskslib.model.Task.TaskStatus.DONE));
		...
		taskStatus.setDestinationStatus(destination2,
				TaskDestinationStatus.ERROR);
		Assert.isTrue(taskStatus.getTaskStatus().equals(
				cz.metacentrum.perun.taskslib.model.Task.TaskStatus.ERROR));
