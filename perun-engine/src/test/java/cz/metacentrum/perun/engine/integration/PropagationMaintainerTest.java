...
public class PropagationMaintainerTest extends BaseTest {
	...
	public void testExecutingRealMessage() throws InterruptedException {
		...
		PropagationMaintainerWorker propagationMaintainerWorker = new PropagationMaintainerWorker();
		taskExecutor.execute(propagationMaintainerWorker);
		started = System.currentTimeMillis();
		itWentOk = false;
		while (System.currentTimeMillis() - started < TIME_OUT) {
			// SEND
		...
		if (!itWentOk) {
			fail("#marDk323 Waiting for PropagationMaintainer has timed out.");
		}
	}
	...
	private class PropagationMaintainerWorker implements Runnable {
		@Override
		public void run() {
