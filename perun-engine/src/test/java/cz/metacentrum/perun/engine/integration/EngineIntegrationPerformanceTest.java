...
public class EngineIntegrationPerformanceTest extends TestBase {
	...
	private final static Logger log = LoggerFactory
			.getLogger(EngineIntegrationPerformanceTest.class);
	...
	public void testExecutingRealMessage() throws InterruptedException,
		...
		for (num = 0; num < NUM_TASKS; num++) {
			...
			// String message =
			// "event|1|[Tue Aug 30 12:29:23 CEST 2011][clockworkorange][Member:[id='36712'] added to Group:[id='16326', name='falcon', description='null'].]";
		...
		/*
		 * EventProcessorWorker eventProcessorWorker = new
		 * EventProcessorWorker(testEvent);
		 * taskExecutorMessageProcess.execute(eventProcessorWorker); boolean
		 * itWentOk = false; long started = System.currentTimeMillis(); while
		 * (System.currentTimeMillis() - started < TIME_OUT) { if
		 * (taskScheduler.getPoolSize() >= 1) { itWentOk = true;
		 * log.debug("     #marDk323 OK, we have " + taskScheduler.getPoolSize()
		 * + " ExecService-Facility pairs in the pool."); break; } else {
		 * log.debug("     #marDk323 There are only " +
		 * taskScheduler.getPoolSize() +
		 * " ExecService-Facility pairs in the pool."); } Thread.sleep(500); }
		 * if (!itWentOk) {
		 ...
		 * taskExecutorMessageProcess.execute(taskSchedulerWorker); started =
		 * System.currentTimeMillis(); itWentOk = false; while
		 * (System.currentTimeMillis() - started < TIME_OUT) { itWentOk = true;
		 * //if (taskManager.getTask(getExecServicePasswdGenerate(),
		 * getFacility1195(),
		 * Integer.parseInt(propertiesBean.getProperty("engine.unique.id"))) ==
		 * null) { // itWentOk = false; //} log.debug("###TASK TASKS were:" +
		 * taskManager.listAllTasks(Integer.parseInt(propertiesBean.getProperty(
		 * "engine.unique.id"))).toString()); Thread.sleep(500); if (itWentOk) {
		 * break; } } if (!itWentOk) {
		...
		// ///
		/*
		 * int expectedTaskResults = 3; List<TaskResult> taskResults = null;
		 * itWentOk = false; if (taskManager == null) {
		 * fail("taskManager really shouldn't be null :-)"); }
		 * 
		 * Task taskExecService1 = task1;
		 * //taskManager.getTask(getExecServicePasswdSend(), getFacility1195(),
		 * Integer.parseInt(propertiesBean.getProperty("engine.unique.id"))); if
		 * (taskExecService1 == null) {
		 * fail("taskExecService1 really shouldn't be null :-)"); } while
		 * (System.currentTimeMillis() - started < TIME_OUT) { itWentOk = true;
		 * if (taskResultDao == null) {
		 * fail("taskResultDao really shouldn't be null :-)"); } taskResults =
		 * taskResultDao.getTaskResultsByTask(taskExecService1.getId());
		 * log.debug("TASKRESULTS:" + taskResults.size()); // Are there three of
		 * them? if (taskResults.size() != expectedTaskResults) { itWentOk =
		 * false; } else { boolean hasDestinationA = false; boolean
		 * hasDestinationB = false; boolean hasDestinationC = false; for
		 * (TaskResult taskResult : taskResults) { // DestinationA ? if
		 * (taskResult.getDestinationId() == destination1.getId()) {
		 * hasDestinationA = true; log.debug("Result " + taskResult.getId() +
		 * " hasDestinationA: TRUE"); } // DestinationB ? if
		 * (taskResult.getDestinationId() == destination2.getId()) {
		 * hasDestinationB = true; log.debug("Result " + taskResult.getId() +
		 * " hasDestinationB: TRUE"); } // DestinationC ? if
		 * (taskResult.getDestinationId() == destination3.getId()) {
		 * hasDestinationC = true; log.debug("Result " + taskResult.getId() +
		 * " hasDestinationC: TRUE"); } } log.debug("hasDestinationA: " +
		 * hasDestinationA + ", hasDestinationB: " + hasDestinationB +
		 * ", hasDestinationC: " + hasDestinationC); if (!(hasDestinationA &&
		 * hasDestinationB && hasDestinationC)) { itWentOk = false; } } if
		 * (itWentOk) { break; } Thread.sleep(100); }
		 * log.debug("\tTASKRESULTS total:" + taskResults.size()); // TODO: Put
		 * some While here... Thread.sleep(10000);
		 * 
		 * // GENERATE log.debug("\tTASKRESULTS total:" + taskResults.size());
		 * Task task = task1;
		 * //taskManager.getTask(getExecServicePasswdGenerate(),
		 * getFacility1195(),
		 * Integer.parseInt(propertiesBean.getProperty("engine.unique.id")));
		 * assertNotNull
		 * ("Task with getExecServicePasswdGenerate should not be null.", task);
		 ...
		 * , itWentOk); // TODO: Put some While here... Thread.sleep(10000);
		 */
