...
public class TaskSchedulerImpl implements TaskScheduler {
	...
	public void propagateService(ExecService execService, Date time, Facility facility) throws InternalErrorException {
		...
		if (execService.isEnabled()) {
			...
			if (!denialsResolver.isExecServiceDeniedOnFacility(execService, facility)) {
				...
				previousTask = taskManager.getTask(execService, facility, Integer.parseInt(propertiesBean.getProperty("engine.unique.id")));
				if (previousTask == null || !previousTask.getStatus().equals(TaskStatus.PROCESSING)) {
					...
					for (ExecService dependantService : dependantServices) {
						Task dependantServiceTask = taskManager.getTask(dependantService, facility, Integer.parseInt(propertiesBean.getProperty("engine.unique.id")));
					...
					if (proceed) {
						...
						if (execService.getExecServiceType().equals(ExecServiceType.SEND)) {
							...
							//
							// BEGIN-FORMER-APPROACH-WHERE-GENERATE-WAS-TREATED-DIFFERENTLY
							//     If the dependency is of type GENERATE:
							//     NONE - Schedule and Proceed
							//     PLANNED - Wait, lets put this [ExecService,Facility] pair back to the SchedulingPool for a while.
							//     PROCESSING - Wait, lets put this [ExecService,Facility] pair back to the SchedulingPool for a while.
							//     ERROR - Schedule and Proceed (ending on ERROR would cause dead-lock...)
							//     DONE - Schedule and Proceed
							//
							//     If the dependency is of type SEND:
							...
							//     DONE - Proceed
							//
							//     TaskExecutorEngine is responsible for ensuring the sequence GENERATE -> SEND.
							...
							for (ExecService dependency : dependencies) {
								Task dependencyServiceTask = taskManager.getTask(dependency, facility, Integer.parseInt(propertiesBean.getProperty("engine.unique.id")));
	...
	private void manipulateTasks(ExecService execService, Facility facility, Task task) {
		...
		if (storedTask == null) {
			try {
				log.debug("Scheduling a new Task:" + task.toString());
				taskManager.scheduleNewTask(task, Integer.parseInt(propertiesBean.getProperty("engine.unique.id")));
			} catch (InternalErrorException e) {
				log.error(e.toString(), e);
			}
		} else {
			// Clean-up old task results
			// TODO: Is this simple clean-up really OK?
			taskResultDao.clearByTask(storedTask.getId());
			// Update task
			task.setId(storedTask.getId());
			log.debug("Updating an old Task:" + task.toString());
			taskManager.updateTask(task, Integer.parseInt(propertiesBean.getProperty("engine.unique.id")));
		}
	}
