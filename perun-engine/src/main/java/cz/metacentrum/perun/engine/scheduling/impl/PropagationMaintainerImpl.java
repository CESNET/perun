  ...
  public void endStuckTasks() {
    ...
    for (Map.Entry<Future<Task>, Task> generatingTask : generatingTasks.getRunningTasks().entrySet()) {
      ...
      if (startTime == null) {
        ...
        if (!future.isCancelled()) {
          ...
          log.debug(
              "[{}] Cancelled stuck generating Future<Task> was not picked by GenCollector, forcefully removing from Engine.",
              task.getId());
    ...
    for (Map.Entry<Future<SendTask>, SendTask> sendingSendTask : sendingSendTasks.getRunningTasks().entrySet()) {
      ...
      if (startTime == null) {
        ...
        // by implementation can't happen, we set time before adding to the generatingTasksMap
        log.error(
            "[{}] SendTask in sendingSendTask has no start time for Destination {}. Shouldn't happen by implementation.",
            task.getId(), sendTask.getDestination());
        ...
        if (!future.isCancelled()) {
          ...
          log.debug(
              "[{}] Cancelled stuck sending Future<SendTask> for Destination: {} was not picked by SendCollector, forcefully removing from Engine.",
              task.getId(), sendTask.getDestination());
      ...
      switch (task.getStatus()) {
        ...
        case WAITING:
          ...
          try {
            ...
            log.error(
                "[{}] Failed during removal of WAITING Task from SchedulingPool. Such Task shouldn't be in Engine at all: {}",
                task.getId(), ex);
          }
