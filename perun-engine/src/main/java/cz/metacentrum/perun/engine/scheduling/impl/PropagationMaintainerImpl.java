...
public class PropagationMaintainerImpl implements PropagationMaintainer {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(PropagationMaintainerImpl.class);
    ...
    for (Task task : allTasks) {
      switch (task.getStatus()) {
        ...
        case PLANNED:
          ...
          Also if GenPlanner implementation fails it might take Task from the BlockingDeque but doesn't
          change
        ...
        case GENERROR, GENERATED:
          /*
          Check Tasks, which should be processed by GenCollector and taken by SendPlanner or reported as
          GENERROR to Dispatcher.
          Task must have endTime set by GenWorker, otherwise it failed completely and should be reported as
          error.
          If either of GenCollector and SendPlanner fails to process generated tasks, it's missing in
          generatedTasksQueue.
          */
          ...
          if ((genEndTime == null || howManyMinutesAgo >= rescheduleTime) &&
        ...
        case SENDING:
          // TODO - if SendPlanner fails, we might need to switch SENDING Tasks to SENDERROR and cancel any running
          //  SendTaskFutures
        ...
        case DONE:
                    ...
                    /*
                     DONE Tasks are almost immediately reported to Dispatcher by
                     schedulingPool#decreaseSendTaskCount().
                     Only way Task is stuck in DONE in scheduling pool is that implementation of removal from
                     schedulingPool fails.
                     And in such case we can't fix it here anyway and spamming Dispatcher about finished state is
                     pointless.
                     */
