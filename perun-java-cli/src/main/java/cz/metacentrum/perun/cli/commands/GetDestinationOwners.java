...
public class GetDestinationOwners extends PerunCommand {
	...
	public void addOptions(Options options) {
		options.addOption(Option.builder("d").required(true).hasArg(true).longOpt("destination").desc("destination name").build());
		options.addOption(Option.builder("c").required(false).hasArg(false).longOpt("convertToLowerCase").desc("convert destination name to lowercase").build());
	}
	...
	public void executeCommand(PerunCLI.CommandContext ctx) {
		...
		for (Facility facility : ctx.getPerunRPC().getFacilitiesManager().getFacilitiesByDestination(destination)) {
			...
			System.out.println("facility: " + facility.getId() + " " + facility.getName() + (desc != null && !desc.trim().isEmpty() ? " \"" + desc + "\"" : ""));
