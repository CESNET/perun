...
public class ApiCaller {
  ...
  public ApiCaller(ServletContext context, PerunPrincipal perunPrincipal, PerunClient client) {
    ...
    PerunPrincipal rpcPrincipal = new PerunPrincipal(RPCPRINCIPAL, ExtSourcesManager.EXTSOURCE_NAME_INTERNAL,
        ExtSourcesManager.EXTSOURCE_INTERNAL);
    this.rpcSession = perun.getPerunSession(rpcPrincipal, new PerunClient());

    // Initialize CabinetManager
    this.cabinetManager =
        WebApplicationContextUtils.getWebApplicationContext(context).getBean("cabinetManager", CabinetManager.class);
    ...
    this.notificationManager = WebApplicationContextUtils.getWebApplicationContext(context)
        .getBean("perunNotifNotificationManager", PerunNotifNotificationManager.class);

    this.integrationManagerApi = WebApplicationContextUtils.getWebApplicationContext(context)
        .getBean("integrationManagerApi", IntegrationManagerApi.class);
  ...
  public <E> List<E> getSublist(List<E> listOfObjects, int fromIndex, int toIndex) {
    ...
    if (toIndex > listOfObjects.size()) {
      toIndex = listOfObjects.size();
    }
