...
public class VirtualAttributeEventProcessor extends AbstractEventProcessor implements EventProcessor {
  ...
  protected List<Pair<PerunAttribute<User>, AttributeDefinition>> getAttributeValues(PerunBl perun, User user,
                                                                   Collection<PerunVirtualAttribute<User>> attrDefs) {
    return attrDefs.stream().map(attrDef -> {
      try {
        return new Pair<PerunAttribute<User>, AttributeDefinition>(attrDef, perun.getAttributesManagerBl()
            .getAttribute(ldapcManager.getPerunSession(), user, attrDef.getPerunAttributeName()));
    ...
    try {
      ...
      if (attrDefs != null) {
        for (User user : perun.getGroupsManagerBl().getGroupUsers(ldapcManager.getPerunSession(), beans.getGroup())) {
          ...
          Collection<Pair<PerunAttribute<User>, AttributeDefinition>> userAttrs =
              this.getAttributeValues(perun, user, attrDefs);
  ...
  public void processGroupAttributeChange(String msg, MessageBeans beans) {
    ...
    try {
      ...
      Collection<PerunVirtualAttribute<User>> attrDefs =
          dependencyManager.getAttributeDependants(beans.getAttribute().getName());
      LOG.debug("Processing group {} attribute {} change, affected attributes {}", beans.getGroup(),
          beans.getAttribute(), attrDefs);
      if (attrDefs != null) {
      ...
      LOG.error("Error processing group {} attribute {} change: {}", beans.getGroup(), beans.getAttribute(),
          e.getMessage());
    }
  }
  ...
  public void processGroupAttributeRemoval(String msg, MessageBeans beans) {
    ...
    try {
      PerunBl perun = (PerunBl) ldapcManager.getPerunBl();
      Collection<PerunVirtualAttribute<User>> attrDefs =
          dependencyManager.getAttributeDependants(beans.getAttributeDef().getName());
      LOG.debug("Processing group {} attribute {} removal, affected attributes {}", beans.getGroup(),
          beans.getAttributeDef(), attrDefs);
      if (attrDefs != null) {
        for (User user : perun.getGroupsManagerBl().getGroupUsers(ldapcManager.getPerunSession(), beans.getGroup())) {
          /* get attribute value for each dependent attribute */
          Collection<Pair<PerunAttribute<User>, AttributeDefinition>> userAttrs =
      ...
      LOG.error("Error processing group {} attribute {} removal: {}", beans.getGroup(), beans.getAttributeDef(),
          e.getMessage());
    }
  }
  ...
  public void processGroupMemberChange(String msg, MessageBeans beans) {
    ...
    try {
      ...
      List<Attribute> groupAttrs = perun.getAttributesManagerBl()
          .getAttributes(ldapcManager.getPerunSession(), beans.getGroup(), registeredAttrs);
      LOG.debug("Processing member {} status change in group {} with attributes {}", user, beans.getGroup(),
          groupAttrs);
      ...
      LOG.error("Error processing member {} change in group {}: {}", beans.getMember(), beans.getGroup(),
          e.getMessage());
    }
  }
