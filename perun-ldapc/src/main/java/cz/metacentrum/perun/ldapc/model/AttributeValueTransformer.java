 ...
 * to conform expected format in the LDAP.
 */
public interface AttributeValueTransformer {
  ...
  /**
   * TRUE if transformer prefers to convert all attribute values at once (whole attribute).
   * FALSE if transformer is applied to each attribute value.
   *
  ...
  /**
   * TRUE if transformer is reducing multi-valued input into single value (eg. Map -> JSON, List -> String).
   * FALSE if transformer keeps original single-/multi-valued nature of attribute.
   *
