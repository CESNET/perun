...
function process {
	...
	for GROUP_NAME in `ls $FROM_PERUN_DIR/` ; do
		...
		pts membership -nameorid "$GROUP_NAME" | tail -n "+2" | sed -e 's/^\s*//' | grep "@meta$" > "$AFS_GROUP_USERS_FILE" 2> "$TMP_ERROR_FILE"
		if [ -s "$TMP_ERROR_FILE" ]; then
			MSG="Command failed: pts membership -nameorid $GROUP_NAME | tail -n +2 | sed -e 's/^\s*//' | grep '@meta$' > $AFS_GROUP_USERS_FILE Reason: `cat $TMP_ERROR_FILE`"
			echo "$MSG" >&2
