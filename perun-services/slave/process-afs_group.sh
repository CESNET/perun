...
function process {
	...
	for GROUP_NAME in `ls $FROM_PERUN_DIR/` ; do
		...
		if [ -n "${AFS_MEMBERSHIP_FILTER}" ]; then	
			cat "$USERS_FROM_PERUN_FILE" | grep "$AFS_MEMBERHIP_FILTER" > "$USERS_FROM_PERUN_FILE_FILTERED" 2> "$TMP_ERROR_FILE"
		else
			cat "$USERS_FROM_PERUN_FILE" > "$USERS_FROM_PERUN_FILE_FILTERED" 2> "$TMP_ERROR_FILE"
		fi
		if [ -s "$TMP_ERROR_FILE" ]; then
			MSG="Command failed: cat $USERS_FROM_PERUN_FILE | grep $AFS_MEMBERHIP_FILTER > $USERS_FROM_PERUN_FILE_FILTERED Reason: `cat $TMP_ERROR_FILE`"
			echo "$MSG" >&2
			logger -t "${NAME}" -p daemon.error "${SERVICE}: ${MSG}" &>/dev/null
			ERROR=1
			continue
