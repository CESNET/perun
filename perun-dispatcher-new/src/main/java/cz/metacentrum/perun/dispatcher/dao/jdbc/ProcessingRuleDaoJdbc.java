...
public class ProcessingRuleDaoJdbc extends JdbcDaoSupport implements ProcessingRuleDao {
    ...
    public Map<ProcessingRule, List<ExecService>> getRules(PerunSession perunSession) throws ServiceNotExistsException, InternalErrorException, PrivilegeException {
        ...
        for (ProcessingRule processingRule : listProcessingRules()) {
            rulesExecServices.put(processingRule, getExecServices(perunSession, processingRule));
        }
    ...
    private List<ExecService> getExecServices(PerunSession perunSession, ProcessingRule processingRule) throws ServiceNotExistsException, InternalErrorException, PrivilegeException {
        ...
        for (Integer serviceId : services) {
            execServicesTiedToTheProcessingRule.addAll(generalServiceManager.listExecServices(perunSession, serviceId));
        }
