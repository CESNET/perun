...
public class GetExpirationByExtLogin extends PerunCommand {
  ...
  public void addOptions(Options options) {
    ...
    options.addOption(
        Option.builder("E").required(true).hasArg().longOpt("extSourceName").desc("extSource name").build());
    options.addOption(
        Option.builder("f").required(true).hasArg().longOpt("file").desc("name of file with user login per line")
            .build());
  }
  ...
  public void executeCommand(PerunCLI.CommandContext ctx) {
    ...
    try {
      ...
      for (String login : Files.readAllLines(Paths.get(filename))) {
        ...
        Attribute attr = perunRPC.getAttributesManager()
            .getMemberAttributeByName(member.getId(), "urn:perun:member:attribute-def:def:membershipExpiration");
