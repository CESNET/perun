...
public class GetFacilityByAttributeWithAttributes extends PerunCommand {
  ...
  public void addOptions(Options options) {
    options.addOption(
        Option.builder("a").required(true).hasArg(true).longOpt("attrName").desc("attribute name").build());
    options.addOption(
        Option.builder("v").required(true).hasArg(true).longOpt("attrValue").desc("attribute value").build());
    options.addOption(Option.builder("r").required(true).hasArg(true).longOpt("returnedAttributeNames")
        .desc("names of returned attributes").build());
  }
  ...
  public void executeCommand(PerunCLI.CommandContext ctx) {
    ...
    List<FacilityWithAttributes> facilities = ctx.getPerunRPC().getFacilitiesManager()
        .getFacilitiesByAttributeWithAttributes(attributeName, attributeValue, attrNames);
