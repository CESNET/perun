...
public class ListOfFacilityGroups extends PerunCommand {
  ...
  public void addOptions(Options options) {
    ...
    options.addOption(
        Option.builder("a").required(false).hasArgs().longOpt("attrNames").desc("multiple attribute names").build());
  }
  ...
  public void executeCommand(PerunCLI.CommandContext ctx) {
    ...
    if (optionA == null) {
      ...
      for (Group group : groups) {
        System.out.println(
            group.getId() + "\t" + group.getName() + "\t" + "\"" + group.getDescription() + "\"\t voId: " +
                group.getVoId());
      }
    } else {
      List<String> attrNames = Arrays.asList(optionA);
      List<RichGroup> groups = ctx.getPerunRPC().getFacilitiesManager()
          .getAllowedRichGroupsWithAttributes(facilityId, attrNames, voId, serviceId);
