...
public class ServicesPackage extends Auditable implements Comparable<PerunBean> {
  ...
  public ServicesPackage(int id, String description, String name, String createdAt, String createdBy, String modifiedAt,
  ...
  public int compareTo(PerunBean perunBean) {
    ...
    if (perunBean instanceof ServicesPackage) {
      ...
      int compare;
      //Compare on name
      if (this.getName() == null && servicesPackage.getName() != null) {
        compare = -1;
      } else if (servicesPackage.getName() == null && this.getName() != null) {
        compare = 1;
      } else if (this.getName() == null && servicesPackage.getName() == null) {
        compare = 0;
      } else {
        compare = this.getName().compareToIgnoreCase(servicesPackage.getName());
      }
      if (compare != 0) {
        return compare;
      }
      //Compare on description
      if (this.getDescription() == null && servicesPackage.getDescription() != null) {
        compare = -1;
      } else if (servicesPackage.getDescription() == null && this.getDescription() != null) {
        compare = 1;
      } else if (this.getDescription() == null && servicesPackage.getDescription() == null) {
        compare = 0;
      } else {
        compare = this.getDescription().compareToIgnoreCase(servicesPackage.getDescription());
      }
      if (compare != 0) {
        return compare;
      }
