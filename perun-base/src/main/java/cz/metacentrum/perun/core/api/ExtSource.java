...
public class ExtSource extends Auditable implements Comparable<PerunBean> {
  ...
  public ExtSource(int id, String name, String type, String createdAt, String createdBy, String modifiedAt,
  ...
  public int compareTo(PerunBean perunBean) {
    ...
    if (perunBean instanceof ExtSource) {
      ...
      if (this.getName() == null && extSource.getName() != null) {
        compare = -1;
      } else if (extSource.getName() == null && this.getName() != null) {
        compare = 1;
      } else if (this.getName() == null && extSource.getName() == null) {
        compare = 0;
      } else {
        compare = this.getName().compareToIgnoreCase(extSource.getName());
      }
      if (compare != 0) {
        return compare;
      }
      //Compare on type
      if (this.getType() == null && extSource.getType() != null) {
        compare = -1;
      } else if (extSource.getType() == null && this.getType() != null) {
        compare = 1;
      } else if (this.getType() == null && extSource.getType() == null) {
        compare = 0;
      } else {
        compare = this.getType().compareToIgnoreCase(extSource.getType());
      }
      if (compare != 0) {
        return compare;
      }
  ...
  public String toString() {
    ...
    return str.append(this.getClass().getSimpleName()).append(":[").append("id='").append(getId()).append('\'')
        .append(", name='").append(name).append('\'').append(", type='").append(type).append('\'').append(']')
        .toString();
  }
}
